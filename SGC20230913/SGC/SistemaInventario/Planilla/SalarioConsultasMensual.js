...
$(document).ready(function () {
    ...
    $('#MainContent_txtAñoPeriodo').blur(function () {
        ...
        if ($('#MainContent_txtAñoPeriodo').val().trim() != '') {
            ...
            $('#lblDisplayPeriodo').text("AÑO: " + SemPro.Año + ", Periodo: " + SemPro.NroPeriodo + ", FECHA: Del " + SemPro.FechaInicialStr + " al " + SemPro.FechaFinalStr);
    ...
    $('#MainContent_txtAñoPeriodoActual').css('background', '#FFFFEE');
    $('#MainContent_ddlProyecto').css('background', '#FFFFE0');
    $('#MainContent_ddlRegimenLaboral').css('background', '#FFFFE0');
    $('#MainContent_txtAñoPeriodo').css('background', '#FFFFE0');
...
$().ready(function () {
    ...
    $(document).everyTime(100, function () {
        ...
        if (DiaActual != now2) {
            DiaActual = now2;
            Planilla_Periodo = utF_Planilla_Periodo_Obtener(DiaActual);
            $('#MainContent_txtAñoPeriodoActual').val(Planilla_Periodo.AñoPeriodo1);
        };
...
function F_Buscar() {
    ...
    try {
        ...
        var objParams = {
            Filtro_CodRegimenLaboral: 2,
            Filtro_CodCategoria: 9,
            Filtro_CodPeriodo: $('#MainContent_lblCodPeriodo').text(),
...
function F_ImprimirDocumento(Fila) {
        ...
        var TipoArchivo = 'application/pdf';
        var CodMenu = '8002';
        var FormatoRPT = 'Web_rptPlanilla_Salario_Boleta_General.rpt';       
...
function F_VerPDF(){
    var CodTrabajadores = $("#MainContent_grvConsulta input:checkbox:checked").not("#MainContent_grvConsulta_ChkAll").map(function(){       
        ...
        return $(hfCodTrabajador).val()+'-'+$(idExcel).val();
    }).get();  
...
function  F_EliminarCarga(){
    var obj={};
    obj["CodPeriodo"]=Number($("#MainContent_lblCodPeriodo").text());    
    if (confirm('¿ESTA SEGURO DE ELIMINAR LA CARGA DEL EXCEL?')){
       
        $.ajax({
        type:"POST",
        contentType: "application/json; charset=utf-8",
        url:"SalarioConsultasMensual.aspx/P_Eliminar_Carga_Excel_PlanillaRG",
...
function F_ConfirmarPago(){
    var obj={};
    obj["CodPeriodo"]=Number($("#MainContent_lblCodPeriodo").text());
    if (confirm('¿ESTA SEGURO DE CONFIRMAR EL PAGO DEL PERIODO?')){
       
        $.ajax({
        type:"POST",
        contentType: "application/json; charset=utf-8",
        url:"SalarioConsultasMensual.aspx/P_Confirmar_Pago_PlanillaRG",
        data:JSON.stringify(obj),
        dataType: "json",
        success:function(dObject){
         var data = dObject.d;                                            
