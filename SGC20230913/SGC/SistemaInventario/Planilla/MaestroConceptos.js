var AppSession = "../Planilla/MaestroConceptoRemuneracion.aspx";
var CodigoMenu = 8000;
var CodigoInterno = 40;
...
function F_Controles_Inicializar() {
    ...
    utF_Conceptos_Combos('#MainContent_ddlTipo', 31);
    utF_Conceptos_Combos('#MainContent_ddlTipoEdicion', 31);

    utF_Conceptos_Combos('#MainContent_ddlClasificacion', 32);
    utF_Conceptos_Combos('#MainContent_ddlClasificacionEdicion', 32);
...
function F_ValidarGrabarDocumento() {
    try {
        ...
        if ($(Cuerpo + 'txtOrden').val() == '')
            Cadena = Cadena + '<p></p>' + 'Orden';

        if ( isNaN($(Cuerpo + 'txtOrden').val()))
            Cadena = Cadena + '<p></p>' + 'Orden';
...
function F_GrabarDocumento() {
    ...
    var objEntidad = {};
    objEntidad["CodConceptoRemuneracion"] = $("#hfCodigo").val();
    objEntidad["Descripcion"] = $('#MainContent_txtDescripcion').val().trim();
    objEntidad["Orden"] = Number($('#MainContent_txtOrden').val());
    objEntidad["CodTipo"] = Number($('#MainContent_ddlTipo').val().trim());
    objEntidad["CodEstado"] = Number($('#MainContent_ddlEstado').val());
    objEntidad["CodRegimenLaboral"] = Number($('#MainContent_ddlRegimenLaboral').val());
    objEntidad["CodConceptoRemuneracion_Referencia"] = Number(0);
    objEntidad["CodClasificacion"] = Number($('#MainContent_ddlClasificacion').val());
    objEntidad["CodNaturaleza"] = Number(1);
    objEntidad["Orden2"] = 0;
    ...
    $.ajax({
        ...
        url: '../Servicios/DatosPlanilla_ConceptoRemuneracion.asmx/F_ConceptoRemuneracion_Actualizar',
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        ...
        var objEntidad = {};
        objEntidad["CodConceptoRemuneracion"] = $(hfCodLinea).val();

        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '../Servicios/DatosPlanilla_ConceptoRemuneracion.asmx/F_ConceptoRemuneracion_Eliminar',
            data: JSON.stringify(objEntidad),
            dataType: "json",
            async: false,
            success: function (dbObject) {
...
function F_EditarRegistro(Fila) {
    try {
        ...
        var hfOrden = '#' + imgID.replace('imgEditarRegistro', 'hfOrden');
        var hfCodRegimenLaboral = '#' + imgID.replace('imgEditarRegistro', 'hfCodRegimenLaboral');
        var hfCodClasificacion = '#' + imgID.replace('imgEditarRegistro', 'hfCodClasificacion');
...
function F_EdicionRegistro() {
    ...
    objEntidad["Orden"] = Number($('#MainContent_txtOrdenEdicion').val());
    objEntidad["CodTipo"] = Number($('#MainContent_ddlTipoEdicion').val().trim());
    objEntidad["CodEstado"] = Number($('#MainContent_ddlEstadoEdicion').val());
    objEntidad["CodRegimenLaboral"] = Number($('#MainContent_ddlRegimenLaboralEdicion').val());
    objEntidad["CodConceptoRemuneracion_Referencia"] = Number(0);
    objEntidad["CodClasificacion"] = Number($('#MainContent_ddlClasificacionEdicion').val());
    objEntidad["CodNaturaleza"] = Number(1);
    objEntidad["Orden2"] = 0;
