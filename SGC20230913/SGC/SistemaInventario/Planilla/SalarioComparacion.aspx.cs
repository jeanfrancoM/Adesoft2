...
{
    public partial class SalarioComparacion : System.Web.UI.Page
    {
        ...
        protected void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (MensajeErrorCarga == "")
            {
                ...
                {
                    P_ExportarExcel(Convert.ToInt64(HiddenField1.Value));
                    HiddenField1.Value = "0";
                    FileUpload1 = new FileUpload();
                }
            }
            ...
            if (FileUpload1.HasFile)
            {
                if (FileUpload1.PostedFile.ContentType == ExcelContentType || FileUpload1.PostedFile.ContentType == Excel2010ContentType)
                {
                    ...
                    try
                    {
                        ...
                        Cadena = string.Format("Select [DNI],[DIAS_T],[HTRAB],[HFER],[DDMED],[HE60],[HE100],[BASICO],[DESM],[BUC],[BAE],[DOMIN],[FER],[HE60PORC],[HE100PORC],[BONPROD],[MOVILID],[ESCOLAR],[RESC],[ALTITUD],[ALTURA],[INDEM],[VAC],[GRATIFIC],[BONIF],[OTRAF],[OTRIN],[LIBRE_D],[LABFER],[RDIAS],[RBAE],[RH60],[RH100],[RCTSE],[TOTINGR],[ONP],[AFPPENS],[PRIMAS],[COMPORC],[CONAF],[SIND],[5TA],[DESJUD],[OTRDESC],[TOTDESC],[NETO],[ESSALUD],[SCTR_S],[AFP_JA],[MAS_VIDA],[SEGP],[TAPOR], " +
                                                HiddenField1.Value + " AS IDExcelComparacion , " +
                        ...
                        using (OleDbConnection connection = new OleDbConnection(excelConnectionString))
                        {
                            ...
                            using (DbDataReader dr = command.ExecuteReader())
                            {
                                ...
                                {
                                    bulkCopy.DestinationTableName = "Planilla_Salario_Excel_Comparacion";
                                    bulkCopy.ColumnMappings.Add("[DNI]", "DNI");
                                    bulkCopy.ColumnMappings.Add("[IDExcelComparacion]", "IDExcelComparacion");
                                    ...
                                    //--
                                    bulkCopy.ColumnMappings.Add("[DIAS_T]", "[EXCEL_DATO_DIAS_T]");
                                    bulkCopy.ColumnMappings.Add("[HTRAB]", "[EXCEL_DATO_H_TRAB]");
                                    bulkCopy.ColumnMappings.Add("[HFER]", "[EXCEL_DATO_H_FER]");
                                    bulkCopy.ColumnMappings.Add("[DDMED]", "[EXCEL_DATO_D_D_MED]");
                                    bulkCopy.ColumnMappings.Add("[HE60]", "[EXCEL_DATO_H_E_60]");
                                    bulkCopy.ColumnMappings.Add("[HE100]", "[EXCEL_DATO_H_E_100]");
                                    bulkCopy.ColumnMappings.Add("[BASICO]", "[EXCEL_CALC_BASICO]");
                                    bulkCopy.ColumnMappings.Add("[DESM]", "[EXCEL_CALC_DES_M]");
                                    bulkCopy.ColumnMappings.Add("[BUC]", "[EXCEL_CALC_B_U_C]");
                                    bulkCopy.ColumnMappings.Add("[BAE]", "[EXCEL_CALC_B_A_E]");
                                    bulkCopy.ColumnMappings.Add("[DOMIN]", "[EXCEL_CALC_DOMIN]");
                                    bulkCopy.ColumnMappings.Add("[FER]", "[EXCEL_CALC_FER]");
                                    bulkCopy.ColumnMappings.Add("[HE60PORC]", "[EXCEL_CALC_H_E_60]");
                                    bulkCopy.ColumnMappings.Add("[HE100PORC]", "[EXCEL_CALC_H_E_100]");
                                    bulkCopy.ColumnMappings.Add("[BONPROD]", "[EXCEL_CALC_BON_PROD]");
                                    bulkCopy.ColumnMappings.Add("[MOVILID]", "[EXCEL_CALC_MOVILID]");
                                    bulkCopy.ColumnMappings.Add("[ESCOLAR]", "[EXCEL_CALC_ESCOLAR]");
                                    bulkCopy.ColumnMappings.Add("[RESC]", "[EXCEL_CALC_R_ESC]");
                                    bulkCopy.ColumnMappings.Add("[ALTITUD]", "[EXCEL_CALC_ALTITUD]");
                                    bulkCopy.ColumnMappings.Add("[ALTURA]", "[EXCEL_CALC_ALTURA]");
                                    bulkCopy.ColumnMappings.Add("[INDEM]", "[EXCEL_CALC_INDEM]");
                                    bulkCopy.ColumnMappings.Add("[VAC]", "[EXCEL_CALC_VAC]");
                                    bulkCopy.ColumnMappings.Add("[GRATIFIC]", "[EXCEL_CALC_GRATIFIC]");
                                    bulkCopy.ColumnMappings.Add("[BONIF]", "[EXCEL_CALC_BONIF]");
                                    bulkCopy.ColumnMappings.Add("[OTRAF]", "[EXCEL_CALC_OTR_AF]");
                                    bulkCopy.ColumnMappings.Add("[OTRIN]", "[EXCEL_CALC_OTR_IN]");
                                    bulkCopy.ColumnMappings.Add("[LIBRE_D]", "[EXCEL_CALC_LIBRE_D]");
                                    bulkCopy.ColumnMappings.Add("[LABFER]", "[EXCEL_CALC_LAB_FER]");
                                    bulkCopy.ColumnMappings.Add("[RDIAS]", "[EXCEL_CALC_R_DIAS]");
                                    bulkCopy.ColumnMappings.Add("[RBAE]", "[EXCEL_CALC_R_BAE]");
                                    bulkCopy.ColumnMappings.Add("[RH60]", "[EXCEL_CALC_R_H60]");
                                    bulkCopy.ColumnMappings.Add("[RH100]", "[EXCEL_CALC_R_H100]");
                                    bulkCopy.ColumnMappings.Add("[RCTSE]", "[EXCEL_CALC_R_CTS_E]");
                                    bulkCopy.ColumnMappings.Add("[TOTINGR]", "[EXCEL_CALC_TOT_INGR]");
                                    bulkCopy.ColumnMappings.Add("[ONP]", "[EXCEL_CALC_O_N_P]");
                                    bulkCopy.ColumnMappings.Add("[AFPPENS]", "[EXCEL_CALC_AFP_PENS]");
                                    bulkCopy.ColumnMappings.Add("[PRIMAS]", "[EXCEL_CALC_PRIMA_S]");
                                    bulkCopy.ColumnMappings.Add("[COMPORC]", "[EXCEL_CALC_COM]");
                                    bulkCopy.ColumnMappings.Add("[CONAF]", "[EXCEL_CALC_CONAF]");
                                    bulkCopy.ColumnMappings.Add("[SIND]", "[EXCEL_CALC_SIND]");
                                    bulkCopy.ColumnMappings.Add("[5TA]", "[EXCEL_CALC_QTA]");
                                    bulkCopy.ColumnMappings.Add("[DESJUD]", "[EXCEL_CALC_DES_JUD]");
                                    bulkCopy.ColumnMappings.Add("[OTRDESC]", "[EXCEL_CALC_OTR_DESC]");
                                    bulkCopy.ColumnMappings.Add("[TOTDESC]", "[EXCEL_CALC_TOT_DESC]");
                                    bulkCopy.ColumnMappings.Add("[NETO]", "[EXCEL_CALC_NETO]");
                                    bulkCopy.ColumnMappings.Add("[ESSALUD]", "[EXCEL_CALC_ESSALUD]");
                                    bulkCopy.ColumnMappings.Add("[SCTR_S]", "[EXCEL_CALC_SCTR_S]");
                                    bulkCopy.ColumnMappings.Add("[AFP_JA]", "[EXCEL_CALC_AFP_J_A]");
                                    bulkCopy.ColumnMappings.Add("[MAS_VIDA]", "[EXCEL_CALC_MAS_VIDA]");
                                    bulkCopy.ColumnMappings.Add("[SEGP]", "[EXCEL_CALC_SEG_P]");
                                    bulkCopy.ColumnMappings.Add("[TAPOR]", "[EXCEL_CALC_T_APOR]");
                    ...
                    catch (Exception ex)
                    {
                        try
                        {
                            MensajeError = "Error en la lectura del excel. <p></p> " +
                                            ...
                                            " <b>Hoja: FORMATO</b>" +
                                            " Columnas: <p></p>" +
                                            " <b>[DNI],[DIAS_T],[HTRAB],[HFER],[DDMED],[HE60],[HE100],[BASICO],[DESM],[BUC],[BAE],[DOMIN],[FER],[HE60PORC],[HE100PORC],[BONPROD],[MOVILID],[ESCOLAR],[RESC],[ALTITUD],[ALTURA],[INDEM],[VAC],[GRATIFIC],[BONIF],[OTRAF],[OTRIN],[LIBRE_D],[LABFER],[RDIAS],[RBAE],[RH60],[RH100],[RCTSE],[TOTINGR],[ONP],[AFPPENS],[PRIMAS],[COMPORC],[CONAF],[SIND],[5TA],[DESJUD],[OTRDESC],[TOTDESC],[NETO],[ESSALUD],[SCTR_S],[AFP_JA],[MAS_VIDA],[SEGP],[TAPOR]</b>";
                ...
                else
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                MSG += "|   LA PLANILLA DEL PROYECTO Y DE LA SEMANA SELECCIONADA NO EXISTE";
                            }
        ...
        private void ProcedimientoTrasladar(ref string Mensaje, ref int CodMovimiento)
        {
            ...
            Planilla_SalarioCE objOperacion = (new Planilla_SalarioCN()).F_Planilla_Salario_Excel_Comparacion(objEntidad);
        ...
        public void P_ExportarExcel(long IdExcel)
        {
            FileInfo newFile = new FileInfo(Server.MapPath("SalarioComparacion_Analisis.xlsx").ToString());

            ExcelPackage pck = new ExcelPackage(newFile);

            var ws = pck.Workbook.Worksheets["DIFERENCIAS"];
            ...
            Planilla_SalarioCE objEntidadSalario = new Planilla_SalarioCE();
            objEntidadSalario.IDExcel = IdExcel;
            DataTable dtTabla = (new Planilla_SalarioCN()).F_Planilla_Salario_Excel_Comparacion_ConsultaExcel(objEntidadSalario);
            ...
            Response.ContentType = "application/octet-stream";
            Response.AppendHeader("Content-Disposition", "attachment; filename=" + "SalarioComparacion_Analisis.xlsx");
            Response.TransmitFile(Server.MapPath("SalarioComparacion_Analisis.xlsx"));
            Response.End();
        }
