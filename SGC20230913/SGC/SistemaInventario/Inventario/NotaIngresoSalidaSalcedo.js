var AppSession = "../Inventario/NotaIngresoSalidaSalcedo.aspx";
...
//                MostrarEspera(false);
     ...
     try 
        ...
        catch (e) 
        {
            ...
            toastr.warning("Error Detectado: " + e);
               ...
               $('#MainContent_btnNotaingreso').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
        try {
            F_Pedidos();
    ...
    $('#MainContent_txtObservacionNS').css('background', '#FFFFE0');
...
$(document).on("change", "select[id $= 'MainContent_ddlTipoDocumento']",function () {
     if ($('#MainContent_ddlTipoDocumento').val()==10)
     {
            ...
            $('#MainContent_txtSerie').prop('disabled',false);
     ...
     else
     {
            $('#MainContent_txtSerie').val('');
            $('#MainContent_txtNumero').val('');
            $('#MainContent_txtSerie').prop('disabled',true);
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        $('#hfCodAlmacen').val(result.split('~')[20]) 
    ...
    var ConexionNombre      = $("#MainContent_ddlAlmacen").val().split('~')[1];
            ...
            var objParams = {
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    $('#MainContent_txtObservacionNS').val(result.split('~')[13]);
                    ...
                    $("#hfMenorPrecioAgregar").val(0);
                     ...
                     $('#MainContent_txtSerie').prop("disabled", true);
                     $('#MainContent_txtNumero').prop("disabled", true);
                     $('#MainContent_ddlMotivoTrabajo').prop("disabled", true);
                     $('#MainContent_txtProveedor').prop("disabled", true);
                     $('#MainContent_ddlDireccion').prop("disabled", true);
                     $('#MainContent_txtObservacionNS').prop("disabled", true);
                     $('#MainContent_ddlAlmacen').prop("disabled", true);
                     $('#MainContent_btnAgregarProducto').prop("disabled", true);
                     $('#MainContent_btnEliminar').prop("disabled", true);
                     $('#MainContent_btnBuscarGuiaAlmacenExterno').prop("disabled", true);
...
function F_ValidarGrabarDocumento(){
    try 
        ...
        if ($(Cuerpo + 'lblNumRegistros').text() == '0')
            Cadena = Cadena + '<p></p>' + 'NO HA INGRESADO PRODUCTO';

               if ($('#MainContent_ddlMotivoTrabajo').val() == 15)
            Cadena = Cadena + '<p></p>' + 'NO PUEDE SER SALIDA DE MERCADERIA';
        ...
        if ($('#hfRemoto').val() === "0") {
            ...
            ConexionNombre      = "";  
                ...
                var objParams = {
                                    ...
                                    Filtro_ObservacionNS: $('#MainContent_txtObservacionNS').val(),
                                    Filtro_CodTipoDoc: $('#MainContent_ddlTipoDocumento').val(),
...
function F_Nuevo()
{
       ...
       $('#MainContent_txtObservacion').val('');
       $('#MainContent_txtResponsable').val('');
       $('#MainContent_txtObservacionNS').val('');
       $('#MainContent_txtSerie').val('');
       $('#MainContent_txtNumero').val('');
       $('#hfCodFacturaAnterior').val('0');
       $('#hfRemoto').val(0);
       ...
       try 
        {
              var objParams = {
                ...
                {                  
                     ...
                     $('#MainContent_txtSerie').prop("disabled", false);
                     $('#MainContent_txtNumero').prop("disabled", false);
                     $('#MainContent_ddlMotivoTrabajo').prop("disabled", false);
                     $('#MainContent_txtProveedor').prop("disabled", false);
                     $('#MainContent_ddlDireccion').prop("disabled", false);
                     $('#MainContent_txtObservacionNS').prop("disabled", false);
                     $('#MainContent_ddlAlmacen').prop("disabled", false);
                     $('#MainContent_btnAgregarProducto').prop("disabled", false);
                     $('#MainContent_btnEliminar').prop("disabled", false);
                     $('#MainContent_btnBuscarGuiaAlmacenExterno').prop("disabled", false);
        ...
        try {
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#ddlTipoDocumento').val(result.split('~')[7]);
                    ...
                    $('#MainContent_txtObservacionNS').val(result.split('~')[15]);
                    
                     switch(result.split('~')[14]) {
                            case '11':
                              $('#MainContent_ddlMotivoTrabajo').val(7);
                              break;
                            case '10':
                              $('#MainContent_ddlMotivoTrabajo').val(2);
                              break;
                            case '3':
...
function F_InicializarSalcedo()
{
    $('#MainContent_txtProveedor').val('CORPORACION AUTOMOTRIZ SALCEDO S.A.C.');
    $('#hfCodCtaCte').val('5');
    $('#hfProveedor').val('CORPORACION AUTOMOTRIZ SALCEDO S.A.C.');  
    F_BuscarDireccionesCliente();    
}
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'NotaingresoSalidaSalcedo.aspx/F_Consulta_Notaingreso_Pendientes_NET',
        dataType: "json",
        ...
        success: function (dbObject) {
            ...
            var data = dbObject.d;
            try {
                var lu = $('#lu_notaingreso');
                lu.empty();
                var Count = 1;
                var CountVisible = 1;
                arrNotaingreso = new Array();
                $.each(data.rows, function (index, item) {
                    var fmt2 =   ' <li style="width: 220px; height:30px; padding-top:0px; display:block"> <div class="linea-button"> ' +
                                ...
                                '             <input type="hidden" id="@ID" value="@CodMovimiento"> ' +
                                '             <input type="hidden" id="@ID_SERIE" value="@Serie"> ' +
                                '             <input type="hidden" id="@ID_NUMERO" value="@NumeroDoc"> ' +
                                ...
                                '             <input type="hidden" id="@ID_OBSERVACION" value="@Observacion"> ' +
                                '             <input type="hidden" id="@ID_ALMACENPARTIDA" value="@AlmacenOrigen"> ' +
                                '             <input type="hidden" id="@DscAlmacen" value="@DscAlmacen"> ' +
                                '       </td> ' +
								'  		<td style="width: 100;padding-left:45px"> ' +
                                '             <label style="font-weight: bold; font-size: 18px;">@NroDocumento</label> ' +
                                '       </td> ' +
								'  		<td style="padding-left:5px"> ' +
                                '             <label style="font-weight: bold; font-size: 18px;">@DscAlmacen</label> ' +
                                '       </td> ' +
								'  		<td style="width: 25"> ' +
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'NOTAINGRESO_' + Count).replace('@CodMovimiento', item.CodMovimiento)
                                        .replace('@NroDocumento', Number(item.NumeroDoc)).replace("@ElVendedor", item.Vendedor).replace('@Cliente', item.Cliente)
                                        .replace('@ID_SERIE', 'NOTAINGRESO_SERIE_' + Count).replace('@Serie', item.Serie)
                                        .replace('@ID_NUMERO', 'NOTAINGRESO_NUMERO_' + Count).replace('@NumeroDoc', item.NumeroDoc)
                                        .replace('@ID_EMISION', 'NOTAINGRESO_EMISION_' + Count).replace('@Emision', item.EmisionStr)
                                        .replace('@ID_VENCIMIENTO', 'NOTAINGRESO_VENCIMIENTO_' + Count).replace('@Vencimiento', item.VencimientoStr)
                                        .replace('@ID_CODCTACTE', 'NOTAINGRESO_CODCTACTE_' + Count).replace('@CodCtaCte', item.CodCtaCte)
                                        .replace('@ID_NRORUC', 'NOTAINGRESO_NRORUC_' + Count).replace('@NroRuc', item.NroRuc)
                                        .replace('@ID_CLIENTE', 'NOTAINGRESO_CLIENTE_' + Count).replace('@Cliente', item.CodCtaCte)
                                        .replace('@ID_CODDEPARTAMENTO', 'NOTAINGRESO_CODDEPARTAMENTO_' + Count).replace('@CodDepartamento', item.CodDepartamento)
                                        .replace('@ID_CODPROVINCIA', 'NOTAINGRESO_CODPROVINCIA_' + Count).replace('@CodProvincia', item.CodProvincia)
                                        .replace('@ID_CODDISTRITO', 'NOTAINGRESO_CODDISTRITO_' + Count).replace('@CodDistrito', item.CodDistrito)
                                        .replace('@ID_DISTRITO', 'NOTAINGRESO_DISTRITO_' + Count).replace('@Distrito', item.Distrito)
                                        .replace('@ID_DIRECCION', 'NOTAINGRESO_DIRECCION_' + Count).replace('@Direccion', item.Direccion)
                                        .replace('@ID_CODFORMAPAGO', 'NOTAINGRESO_CODFORMAPAGO_' + Count).replace('@CodFormaPago', item.CodFormaPago)
                                        .replace('@ID_CODMONEDA', 'NOTAINGRESO_CODMONEDA_' + Count).replace('@CodMoneda', item.CodMoneda)
                                        .replace('@ID_TC', 'NOTAINGRESO_TC_' + Count).replace('@TC', item.TipoCambio)
                                        .replace('@ID_SUBTOTAL', 'NOTAINGRESO_SUBTOTAL_' + Count).replace('@Subtotal', item.SubTotal)
                                        .replace('@ID_IGV', 'NOTAINGRESO_IGV_' + Count).replace('@Igv', item.Igv)
                                        .replace('@ID_TOTAL', 'NOTAINGRESO_TOTAL_' + Count).replace('@Total', item.Total)
                                        .replace('@ID_BTNAGREGARCOTIZACION', 'BTN_NOTAINGRESO_' + Count).replace('@Total', item.Total)
										.replace('@ID_FLAGIGV', 'NOTAINGRESO_FLAGIGV_' + Count).replace('@FlagIgv', item.FlagIgv)
										.replace('@ID_NROOPERACION', 'NOTAINGRESO_NROOPERACION_' + Count).replace('@NroOperacion', item.NroOperacion)
										.replace('@ID_CODEMPLEADO', 'NOTAINGRESO_CODEMPLEADO_' + Count).replace('@CodEmpleado', item.CodEmpleado)
										.replace('@ID_PLACA', 'NOTAINGRESO_PLACA_' + Count).replace('@Placa', item.Placa)
										.replace('@ID_KM', 'NOTAINGRESO_KM_' + Count).replace('@KM', item.KM)
                                        .replace('@ID_OBSERVACION', 'NOTAINGRESO_OBSERVACION_' + Count).replace('@Observacion', item.Observacion)
                                        .replace('@ID_ALMACENPARTIDA', 'NOTAINGRESO_ALMACENPARTIDA_' + Count).replace('@AlmacenOrigen', item.AlmacenOrigen)
                                        .replace('@DscAlmacen', 'ALMACEN_' + Count).replace('@DscAlmacen', item.DscAlmacen)
                                        .replace('@DscAlmacen', item.DscAlmacen)


                        var objDetalle = {
                            Indice: Count,
                            CodProforma: item.CodMovimiento,
                            Serie: item.Serie,
                            Numero: item.NumeroDoc,
                            Nro: item.Serie + '-' + item.NumeroDoc
                        };
                        arrNotaingreso.push(objDetalle);
...
function F_AgregarTemporalCTxBoton(Boton) {
    $('#MainContent_txtSerie').val($('#NOTAINGRESO_SERIE_' + Boton.split('_')[2]).val());
    $('#MainContent_txtNumero').val($('#NOTAINGRESO_NUMERO_' + Boton.split('_')[2]).val());
    $('#MainContent_ddlAlmacen').val($('#NOTAINGRESO_ALMACENPARTIDA_' + Boton.split('_')[2]).val());
    $('#MainContent_txtObservacionNS').val($('#OBSERVACION' + Boton.split('_')[2]).val());
    F_Obtener_Guia_AlmacenExterno();
}
