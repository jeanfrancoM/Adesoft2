var AppSession = "../Inventario/GuiaRemisionAlvarado.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_txtProveedor').autocomplete({
        ...
        select: function (e, i) {
            ...
            F_BuscarDireccionesClienteConexion();
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, 2000506, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try {
            F_BuscarNotaIngreso();
     ...
     $('#MainContent_btnFacturarCompra').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
        try {
            F_AgregarTemporalCTxBoton();
            return false;
        }
...
function F_CamposAlternativas(Campos) {
    ...
    switch (Campos) {
        ...
        case "MainContent_txtCodCotizacion":
            ...
            break;
        case "MainContent_txtIDCompra":
            if ($("#MainContent_txtIDCompra").val().trim() != "") {
                if (isNaN($("#MainContent_txtIDCompra").val().trim()))
                    $("#MainContent_txtIDCompra").val('');
                else
                    $('#MainContent_btnFacturarCompra').click();
            }
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#hfNroRuc').val(result.split('~')[18]);
                        $('#NombreClaveAzure').val(result.split('~')[19]);
        ...
        var tasaigv;
          ...
          if ($('#MainContent_chKConIgv').is(':checked'))
                ...
                var objParams = {
                                        ...
                                        Filtro_CodAlmacenFisicoDesde:0, //$('#MainContent_ddlPartida').val(),
                                        Filtro_CodAlmacenFisicoHasta:0, //$('#MainContent_ddlDestino').val(),
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        var Peso = 0;
        var NroBultos = 0;
        var FlagMotorizado = 0;
        var Destino =$('#MainContent_ddlDestino').val().split('-')[0];
        var Direccion =$('#MainContent_ddlDireccion').val().split('-')[0];
        ...
        var objParams = {
                ...
                Filtro_CodAlmacenLlegada: Destino,
                ...
                Filtro_Observacion: $('#MainContent_txtMensaje').val(),
...
function F_Nuevo(){
        ...
        $('#MainContent_ddlDireccionTransportista').val('0');
          if ($('#MainContent_ddlTipoDocumento option').length ==1)
        ...
        $("#MainContent_txtIDCompra").prop('disabled', false);
                ...
                {                  
                 ...
                 F_BUSCAR_ALMACEN($('#MainContent_ddlEmpresa').val());
                   ...
                   $("#MainContent_ddlTipoDocumento").prop('disabled', false);
                       $("#MainContent_ddlSerie").prop('disabled', false);
                       ...
                       $("#MainContent_btnBuscar").prop('disabled', false);
...
function F_BuscarDireccionesClienteConexion() {
    if (!F_SesionRedireccionar(AppSession)) return false;
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '../Servicios/Servicios.asmx/F_TCDireccion_ListarXCliente_AutoComplete_Conexion',
        data: "{'CodCtaCte':'" + $('#hfCodCtaCte').val() + "'}",
        dataType: "json",
        async: false,
        success: function (dbObject) {
            ...
            try {
                ...
                var CodDireccionPrimera = 0; var ccc = 0;
                $.each(data.rows, function (index, item) {
                    $('#MainContent_ddlDireccion').append($("<option></option>").val(item.CodDireccionConexion).html(item.Direccion));
                    $('#MainContent_ddlDestino').append($("<option></option>").val(item.CodDireccionConexion).html(item.Direccion));
                    if (ccc === 0) CodDireccionPrimera = item.CodDireccion;  ccc++;
                });
        ...
        try {
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    //$('#MainContent_ddlDestino').val(result.split('~')[17]);
                   // $('#MainContent_ddlDireccion').val(result.split('~')[17]);
                   $('#MainContent_ddlDireccion').val().split('-')[0]=result.split('~')[17];
                   $('#MainContent_ddlDestino').val().split('-')[0]=result.split('~')[17];
                    $('#MainContent_ddlEmpresa').val(result.split('~')[18]);
                    $('#MainContent_txtMensaje').val(result.split('~')[19]);

                    if (result.split('~')[20] == 1)
    ...
    try {    
        ...
        arrDetalle.push(objDetalle);
                ...
                var objParams = {
                                        ...
                                        Filtro_TasaIgvDscto: 0,
                                        Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
                                        Filtro_CodAlmacenFisicoDesde: 0,
                                        Filtro_CodAlmacenFisicoHasta: 0,
                                        Filtro_FlagFormulario: 1,
                                        Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle)
                               };
...
var cotFlagIgv = '';
function F_BuscarNotaIngreso(Fila) {
 var Conexion =$('#MainContent_ddlDireccion').val().split('-')[1];
 var Ruc =$('#MainContent_txtProveedor').val().split('-')[0];
 var Destino =$('#MainContent_ddlDestino').val().split('-')[2];

  if($('#MainContent_ddlMotivoTrabajo').val() == 14)
 {
 toastr.warning("OPERACION NO VALIDA");
 return false;
 }

  if($.trim($('#MainContent_txtProveedor').val().split('-')[0]) == $.trim($('#hfNroRuc').val()) & $('#MainContent_ddlMotivoTrabajo').val() != 10)
 {
 toastr.warning("SI EL ORIGEN ES LA DE LA MISMA EMPRESA LA OPERACION DEBE SER ENTRE ESTAB. DE LA MISMA EMPRESA");
 return false;
 }

   if($.trim($('#MainContent_txtProveedor').val().split('-')[0]) != $.trim($('#hfNroRuc').val()) & $('#MainContent_ddlMotivoTrabajo').val() != 9)
 {
 toastr.warning("SI EL ORIGEN ES DISTINTO DE LA EMPRESA LA OPERACION DEBE SER CONSIGNACION");
 return false;
 }

  if($.trim($('#MainContent_ddlDireccion').val().split('-')[1]) == $.trim($('#NombreClaveAzure').val()))
 {
 toastr.warning("NO SE PUEDE BUSCAR DE LA MISMA SUCURSAL");
 return false;
 }

 if($('#MainContent_txtProveedor').val().split('-')[0] == "20602525920 " || $('#MainContent_txtProveedor').val().split('-')[0] == "20524261112 " | $('#MainContent_txtProveedor').val().split('-')[0] == "20605607501 " ||$('#MainContent_txtProveedor').val().split('-')[0] == "20605156372 " )
 {
 MostrarEspera(true);
    $.ajax({
        async: true,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionAlvarado.aspx/F_ObtenerNotaIngreso_PorID_Net',
        dataType: "json",
        data: "{'Serie':'" + $('#MainContent_ddlSerie option:selected').text()  + "','Numero':'" + $('#MainContent_txtNumero').val()  + "','Ruc':'" + Ruc  + "','Destino':'" + Destino  + "','CodTipoDoc':'" + $('#MainContent_ddlTipoDocumento').val()  + "','Conexion':'" + Conexion  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            
            var data = dbObject.d;
            try {
            if(data.lNotaingresoDet.length>0)
            {
                    var arrDetalle = new Array();
                    var Contenedor = '#MainContent_';
                    var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);

                    var CountAgregados = 0;
                    //Array de Items
                    $.each(data.lNotaingresoDet, function (index, item) {

                            var objDetalle = {
                                 CodArticulo:  item.CodProducto,
                    ...
                    var objParams = {
                                        Filtro_CodTipoDoc: "2",
                                        Filtro_SerieDoc:  data.SerieDocSust,
                                        Filtro_NumeroDoc: data.NumeroDocSust,
                                        Filtro_FechaEmision:  $('#MainContent_txtEmision').val(),
                                        Filtro_Vencimiento: $('#MainContent_txtEmision').val(),
                                        Filtro_CodCliente: 119,
                                        Filtro_CodFormaPago: 1,
                                        ...
                                        Filtro_TasaIgvDscto: 0,
                                        Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
                                        Filtro_CodAlmacenFisicoDesde: data.CodAlmacenOrigen,
                                        Filtro_CodAlmacenFisicoHasta: data.CodAlmacenDestino,
                                        Filtro_FlagFormulario: 1,
                                        Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle)
                    };
                ...
                F_AgregarTemporal_Servidor_NET(arg, function (result) {
                    var str_resultado_operacion = "";
                    var str_mensaje_operacion = "";
                ...
                if (str_resultado_operacion == "1") 
                {
                       ...
                       $('#MainContent_txtSubTotal').val(result.split('~')[7]);
                       $('#MainContent_txtDsctoTotal').val(result.split('~')[8]);
                       $('#MainContent_txtMensaje').val(data.Observacion2);
                       ...
                       $("#MainContent_ddlSerie").prop('disabled', true);
                       ...
                       $("#MainContent_btnBuscar").prop('disabled', true);
                       $("#MainContent_ddlAlmacen").prop('disabled', true);
                       $("#MainContent_txtEmision").prop('disabled', true);
                       $("#MainContent_txtIDCompra").prop('disabled', true);
                       $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                             chkDel = '#' + this.id;
                             txtCantidad = chkDel.replace('chkEliminar', 'txtCantidad');
            ...
            catch (x) {  toastr.warning("ERROR DETECTADO: " + x)}
            MostrarEspera(false);
        },
        error: function (xhr, ajaxOptions, thrownError) {
        toastr.warning("ERROR DETECTADO: " + xhr.responseJSON.Message)
          $('#MainContent_txtMensaje').val(xhr.responseJSON.Message);
          MostrarEspera(false);
        }
    });
     }else{
      toastr.warning("SOLO SE PUEDE BUSCAR DE EMPRESAS DE ALVARADO")
 return false
 }
 }
...
function F_Inicializar_TextBox() {  
    ...
    $('#MainContent_txtIDCompra').css('background', '#FFFFE0');  
...
function F_BuscarFacturaCompra(Fila) {
 ...
 MostrarEspera(true);
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'GuiaRemisionAlvarado.aspx/F_ObtenerFacturaCompra_PorID_Net',
        dataType: "json",
        data: "{'ID':'" + $('#MainContent_txtIDCompra').val() + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
