var AppSession = "../Inventario/NotaIngresoSalidaAraSac.aspx";
...
//                MostrarEspera(false);
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, 2000708, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try {
            F_BuscarNotaIngreso();
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        $('#hfNroRuc').val(result.split('~')[21]);
                        $('#NombreClaveAzure').val(result.split('~')[22]);
...
//            Cadena=Cadena + '<p></p>' + 'Numero';
        ...
        if ($(Cuerpo + 'txtObservacion').val() + $(Cuerpo + 'txtMensaje').val() =='')
            Cadena=Cadena + '<p></p>' + 'Observacion';

            if (($(Cuerpo + 'txtSerie').val()=='0250' | $(Cuerpo + 'txtSerie').val()=='0211' | $(Cuerpo + 'txtSerie').val()=='0173' | $(Cuerpo + 'txtSerie').val()=='0270') && $(Cuerpo + 'txtMensaje').val()=='')
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        if ($('#MainContent_chkImportacion').is(':checked'))
        FlagImportacion = 1;
                    CodAlmacenRemoto    = "";
            ConexionNombre      = "";  
                ...
                var objParams = {
                                    ...
                                    Filtro_Observacion: $('#MainContent_txtObservacion').val()+ ' ' +  $('#MainContent_txtMensaje').val(),
                                    Filtro_CodTipoDoc: $('#MainContent_ddlTipoDocumento').val(),
                                    ...
                                    Filtro_FlagImportacion: FlagImportacion,
        ...
        try {
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    if (result.split('~')[16] == 1)
                        $('#MainContent_chkImportacion').prop('checked',true);
                    else
                        $('#MainContent_chkImportacion').prop('checked',false);

                    if (result.split('~')[17] == 1)
                        $('#MainContent_chkMotorizado').prop('checked',true);
                    else
                        $('#MainContent_chkMotorizado').prop('checked',false);

                    $('#MainContent_ddlDireccion').val().split('-')[0]=result.split('~')[11];
...
function F_BuscarNotaIngreso(Fila) {
 ...
 var Destino =$('#MainContent_ddlDireccion').val().split('-')[2];

   if($.trim($('#MainContent_txtProveedor').val().split('-')[0]) == $.trim($('#hfNroRuc').val()))
 {
 toastr.warning("NO SE PUEDE BUSCAR DE LA MISMA EMPRESA");
 return false;
 }

  if($.trim($('#MainContent_ddlDireccion').val().split('-')[1]) == $.trim($('#NombreClaveAzure').val()))
 {
 toastr.warning("NO SE PUEDE BUSCAR DE LA MISMA BASE DE DATOS");
 return false;
 }
 ...
 MostrarEspera(true);
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'NotaIngresoSalidaAraSac.aspx/F_ObtenerTraslados_PorID_Net',
        dataType: "json",
        data: "{'Serie':'" + $('#MainContent_txtSerie').val()  + "','Numero':'" + $('#MainContent_txtNumero').val()  + "','Ruc':'" + $('#hfNroRuc').val()  + "','Destino':'" + Destino  + "','CodTipoDoc':'" + $('#MainContent_ddlTipoDocumento').val()  + "','Conexion':'" + Conexion  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            
            var data = dbObject.d;
            try {
            if(data.ltrasladosDet.length>0)
            {
                    var arrDetalle = new Array();
                    var Contenedor = '#MainContent_';
                    var tasaigv =1.18;
                    ...
                    //Array de Items
                    $.each(data.ltrasladosDet, function (index, item) {
                            var objDetalle = {
                ...
                if (str_resultado_operacion == "1") 
                {
                    ...
                    $("#hfMenorPrecioAgregar").val(0);
                      ...
                      // $("#MainContent_txtObservacion").prop('disabled', true);
