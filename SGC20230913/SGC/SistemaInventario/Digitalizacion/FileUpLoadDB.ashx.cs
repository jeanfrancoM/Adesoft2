...
namespace SistemaInventario.Digitalizacion
{
    ...
    public class FileUpLoadDB : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (Convert.ToInt32(context.Request.Params["Flag"]) == 1)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < dtb.Rows.Count; i++)
                    {
                        ...
                        nombre_doc = f["CodProducto"].ToString() + "-" + f["IdImagen"].ToString() + DateTime.Now.ToString("yyyyMMddhhmmss") + ".jpg";
            ...
            //Logo Empresa
            if (Convert.ToInt32(context.Request.Params["Flag"]) == 3)
            {
                ...
                objEntidad.CodEmpresa = Convert.ToInt32(context.Request["IdFile"]);
                dtb = objOperacion.F_DescargarImagen_CodEmpresa(objEntidad);
                ...
                List<TCEmpresaCE> objListEmpresa = new List<TCEmpresaCE>();
                ...
                try
                {
                    ...
                    for (int i = 0; i < dtb.Rows.Count; i++)
                    {
                        ...
                        nombre_doc = "ImagenEmpresa.jpg"; // dtb.Rows[i][1].ToString();
                        nombre_doc = f["NroRuc"].ToString() + DateTime.Now.ToString("ddMMyyyyhhmmss") + ".jpg";
                        ...
                        objListEmpresa.Add(new TCEmpresaCE()
                        {
                            T_NombreArchivo = nombre_doc,
                            T_Ruta = sFile,
                            T_Preview = vPrevia,
                ...
                JavaScriptSerializer oSerializer = new JavaScriptSerializer();
                string sJSON = oSerializer.Serialize(objListEmpresa);
                context.Response.Write(sJSON);
            }
