    ...
    public class FileDocDB : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (Convert.ToInt32(context.Request.Params["Flag"]) == 1)
            {
                ...
                objEntidad.CodProducto = Convert.ToInt32(context.Request["IdFile"]);
                    ...
                    switch (tipo)
                    {
                        case 1: //forma de visualizador de windows
                            try
                            {
                                foreach (DataRow img in dtb.Rows)
                                {
                                    byte[] bits = (byte[])img["B_LogoEmpresa"];
                                    string nombre_doc = img["NroRuc"].ToString() + "-" + img["CodEmpresa"].ToString() + DateTime.Now.ToString("ddMMyyyyhhmmss") + ".jpg";
                                    string sFile = System.Web.HttpContext.Current.Server.MapPath(@"..\files\temp\") + nombre_doc;
                                    string vPrevia = "";
                                    vPrevia = @"../../files/temp/" + nombre_doc; //DESARROLLO
                                    //sFile = @"../files/download/" + nombre_doc; //PUBLICACION
                                    using (FileStream fs = new FileStream(sFile, FileMode.Create))
                                    {
                                        fs.Write(bits, 0, Convert.ToInt32(bits.Length));
                                        fs.Close();
