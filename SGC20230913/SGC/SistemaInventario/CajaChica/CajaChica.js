    ...
    if ($(chkok_grilla).is(':checked')) {
       $('#MainContent_grvDetalleLiquidacion .detallesart').each(function () {
                   ...
                   // ASIGNO VALOR CERO A LOS DOCUMENTOS NO FORMALES
                    if (($(lblDocumento).text().indexOf('F') == -1 & $(lblDocumento).text().indexOf('B') == -1) | ($(hfcodtipodoc).val() == 2 & parseFloat($(hfSolesAplicado).val())< 0 ) )
                    {
                         $(txtTotalSoles).val('0');
...
function F_ValidarLiquidacionaplicar() {
    ...
    try {
        ...
        $('#MainContent_grvDetalleLiquidacion .detallesart').each(function () {
            ...
            if (($(hfcodtipodoc).val()!=16||$(hfcodtipodoc).val()!=13||$(hfcodtipodoc).val()!=8)  &&( parseFloat($(txtTotalSoles).val())>0|| parseFloat( $(txtTotalDolares).val())>0)) {
                x = x+1;

            }
            else if (($(hfcodtipodoc).val()==16||$(hfcodtipodoc).val()==13||$(hfcodtipodoc).val()==8) &&( parseFloat($(txtTotalSoles).val())>0|| parseFloat( $(txtTotalDolares).val())>0)){
            y=y+1
            }
            c=c+1;
        });
