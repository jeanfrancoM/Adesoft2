...
$().ready(function () {
    ...
    $(document).everyTime(600000, function () {
        ...
        $.ajax({
            type: "POST",
            url: '../Ventas/Cotizacion.aspx/KeepActiveSession',
            data: {},
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
...
function F_AnularRegistro(Fila) {
 try 
    ...
    if(!confirm("ESTA SEGURO DE ELIMINAR LA GUIA DE COMPRA : " + $(lblnumero_grilla).text() + "\nDEL PROVEEDOR : " +  $(lblcliente_grilla).text()))
    return false;
        ...
        try {
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#MainContent_txtProveedor').val(result.split('~')[7]);
                    $('#MainContent_ddlMoneda').val(result.split('~')[8]);
                    $('#MainContent_ddlFormaPago').val(result.split('~')[9]);
                    $('#hfCodCtaCte').val(result.split('~')[10]);
                    $('#hfCodFacturaAnterior').val(result.split('~')[11]);
                    $('#MainContent_txtPeriodo').val(result.split('~')[12]);
                    $('#MainContent_txtFechaIngreso').val(result.split('~')[13]);
                    $("#MainContent_txtSerie").val(result.split('~')[14]);
                    $('#MainContent_txtNumero').val(result.split('~')[15]);
                    $('#MainContent_txtEmision').val(result.split('~')[16]);
                    $('#MainContent_txtVencimiento').val(result.split('~')[17]);
                    $('#MainContent_lblTC').text(result.split('~')[18]);
                    $('.ccsestilo').css('background', '#FFFFE0');
