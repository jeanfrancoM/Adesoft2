var AppSession = "../Compras/RegistroFacturaCompraNuevoSalcedo.aspx";
...
//    }
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, 3000204, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
     try 
        {
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
    if (F_PermisoOpcion(CodigoMenu, 3000201, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
     try 
        {
          F_FacturacionOC();
          return false;
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
     try 
        {
            if(!F_ValidarDevolucion("SELECCIONE UN PRODUCTO PARA DEVOLVER"))
              return false;
    ...
    $('#MainContent_btnVentas').click(function () {
       if (!F_SesionRedireccionar(AppSession)) return false;
       if (F_PermisoOpcion(CodigoMenu, 3000202, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    ...
    $('#MainContent_ddlPrecio').css('background', '#FFFFE0');
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    hfMargen2_grilla = imgAgregar_grilla.replace('imgAgregar', 'hfMargen2');
    ...
    $('#MainContent_txtMargen').val($(hfMargen_grilla).val());
    $('#MainContent_txtMargen2').val($(hfMargen2_grilla).val());
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_SerieDocVentas', result.split('~')[15]);
 ...
 try 
        ...
        if ($("#MainContent_txtCantidad").val() == '')
            ...
            cadena = cadena + "LA CANTIDAD NO PUEDE SER NEGATIVO<p></p> ";
              ...
              if ($("#MainContent_txtPrecioDisplay").val() == '0' | $("#MainContent_txtPrecioDisplay").val() == '')
            ...
            if ($("#MainContent_txtMargen2").val() == '0' | $("#MainContent_txtMargen2").val() == '')
            cadena = cadena + "INGRESE  M. MIN. <p></p> ";

             if (parseFloat($("#MainContent_txtMargen2").val()) < '0')
            cadena = cadena + "EL M. MIN. NO PUEDE SER NEGATIVO<p></p> ";

             if ($("#MainContent_txtMargen").val() == '0' | $("#MainContent_txtMargen").val() == '')
            cadena = cadena + "INGRESE M. GEN. <p></p> ";

             if (parseFloat($("#MainContent_txtMargen").val()) < '0')
            cadena = cadena + "EL M. GEN. NO PUEDE SER NEGATIVO<p></p> ";

             if (parseFloat($("#MainContent_txtMargen2").val()) > parseFloat($("#MainContent_txtMargen").val()))
            cadena = cadena + "EL M. GEN. NO PUEDE SER MENOR QUE EL M. MIN.<p></p> ";
        ...
        if ($('#MainContent_chkConIgvMaestro').is(':checked'))
             ...
             tasaigv=1;
                ...
                var objDetalle = {
                     ...
                     CantidadOC:0,
                     Margen:$('#MainContent_txtMargen').val(),
                     Margen2:$('#MainContent_txtMargen2').val()
                };                                               
            ...
            CodTipoCliente=1;
                ...
                {                  
                    if (str_mensaje_operacion=='Se grabo correctamente')
                    {
                        ...
                        F_MostrarNumero(result.split('~')[2]);
                        F_Nuevo();
                    }
                    else
                    {
...
function F_EliminarRegistro(Fila) {
 ...
 try 
    ...
    if(!confirm("ESTA SEGURO DE ELIMINAR LA FACTURA : " + $(lblNumero).text() + "\n" + "DEL PROVEEDOR : " +  $(lblCliente).text()))
    return false;
                  ...
                  if ($(chkSi).is(':checked')) 
                    {
                       if (parseFloat($(hfStockActual).val()) < parseFloat ($(txtCantidadEntregada).val()) )
                        ...
                        var objDetalle = {
                        CodDetalle: $(hfCodDetalle).val(),
                        CodMovimiento: $(hfCodMovimiento).val(),
...
function F_ValidarStockGrillaOC(ControlID) {
    ...
    hfCantidadTotal = txtcantidad_Grilla.replace('txtCantidadEntregada', 'hfCantidadTotal');
    hfCantidadFacturada = txtcantidad_Grilla.replace('txtCantidadEntregada', 'hfCantidadFacturada');

    
    boolEstado = $(chkok_grilla).is(':checked');

    if (boolEstado &&  parseFloat($(lblstock).text()) > parseFloat($(hfCantidadTotal).val())- parseFloat($(hfCantidadFacturada).val())  ||  parseFloat($(txtcantidad_Grilla).val()) > parseFloat($(lblstock).text())) {
            alertify.log("Stock insuficiente");
            $(txtcantidad_Grilla).val($(lblstock).text());
            return false;
    }
     if (boolEstado &&  parseFloat($(txtcantidad_Grilla).val()) <= 0) {
            alertify.log("Corregir Cantidad");
            $(txtcantidad_Grilla).val($(lblstock).text());
            return false;
    }
        ...
        if ($('#MainContent_chkConIgvMaestro').is(':checked')) {
            ...
            FlagIgv = 1;
                ...
                $('#MainContent_grvDetalleOC .chkDelete :checkbox').each(function () {
                    ...
                    hfCodDetDocumentoVenta = chkSi.replace('chkEliminar', 'hfCodDetDocumentoVenta');
                    hfMargen1 = chkSi.replace('chkEliminar', 'hfMargen1');
                    hfMargen2 = chkSi.replace('chkEliminar', 'hfMargen2');
                    hfCodDetDocumentoVenta = chkSi.replace('chkEliminar', 'hfCodDetDocumentoVenta');
                  ...
                  if ($(chkSi).is(':checked')) 
                    {
                        ...
                        CodDetDocumentoVenta:$(hfCodDetDocumentoVenta).val(),
                        Margen:$(hfMargen1).val(),
                        Margen2:$(hfMargen2).val()
                        };
 ...
 try 
        {
             ...
             if (parseInt($(hfCodDetalleOC).val())> 0 & parseFloat($(txtPrecio).val())!=parseFloat($(hfPrecio).val()) )
            {
             ...
             alertify.log("EL PRECIO NO PUEDE SER DIFERENTE AL REGISTRADO EN LA OC");              
            return false;
            }
         ...
         return false;
            ...
            var objParams = {
                              ...
                              Filtro_Tasa: $('#MainContent_lblTC').text(),
                              Filtro_CodMoneda: $('#MainContent_ddlMoneda').val(),
                               Filtro_Margen: $(txtMargen).val(),
                               Filtro_Margen2: $(hfMargen2).val(),
            };
 ...
 try 
        {
            ...
            var hfMargen = txtCantidad.replace('txtCantidad', 'hfMargen');
            var hfCantidad = txtCantidad.replace('txtCantidad', 'hfCantidad');
            var txtPrecio = txtCantidad.replace('txtCantidad', 'txtPrecio');
            var txtMargen = txtCantidad.replace('txtCantidad', 'txtMargen');
            var txtMargen2 = txtCantidad.replace('txtCantidad', 'txtMargen2');
            var lblPrecio = txtCantidad.replace('txtCantidad', 'lblPrecio');
            var lblProducto = txtCantidad.replace('txtCantidad', 'lblProducto');
            var hfCantidadOC = txtCantidad.replace('txtCantidad', 'hfCantidadOC');
            var tasaigv;
            var FlagIgv = 0;
            var Contenedor = '#MainContent_';    
            if(parseFloat($(txtPrecio).val())==parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val())==parseFloat($(hfCantidad).val()))
            ...
            if (parseInt($(hfCodDetalleOC).val())> 0 & parseFloat($(txtCantidad).val())>parseFloat($(hfCantidadOC).val()) )
            {
             $(txtCantidad).val(parseFloat($(hfCantidad).val()).toFixed(2));
             alertify.log("LA CANTIDAD NO PUEDE SER SUPERIOR A LA CANTIDAD REGISTRADA EN LA OC");              
            return false;
            }
...
function F_ActualizarMargen(Fila) {
 try 
        {
            var txtMargen =  '#' + Fila;           
            var hfCodDetalle = txtMargen.replace('txtMargen', 'hfCodDetalle');
            var hfCodDetalleOC = txtMargen.replace('txtMargen', 'hfCodDetalleOC');
            var hfPrecio = txtMargen.replace('txtMargen', 'hfPrecio');
            var hfCantidad = txtMargen.replace('txtMargen', 'hfCantidad');
            var hfMargen = txtMargen.replace('txtMargen', 'hfMargen');
            var hfMargen2 = txtMargen.replace('txtMargen', 'hfMargen2');
            var txtCantidad = txtMargen.replace('txtMargen', 'txtCantidad');
            var txtPrecio = txtMargen.replace('txtMargen', 'txtPrecio');
            var lblProducto = txtMargen.replace('txtMargen', 'lblProducto');
            var tasaigv;
            var FlagIgv = 0;
            var Contenedor = '#MainContent_';    
//            if(parseFloat($(txtPrecio).val())==parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val())==parseFloat($(hfCantidad).val()))
...
//            }
            ...
            return false;
                ...
                if (parseInt($(txtMargen).val())<= 0 | $(txtMargen).val()== '' )
            {
             $(txtMargen).val(parseFloat($(hfMargen).val()).toFixed(2));
             alertify.log("EL MARGEN GENERAL NO PUEDE SER MENOR O IGUAL A 0");              
            return false;
            }

             if(parseFloat($(txtMargen).val())==parseFloat($(hfMargen).val()) )
            {
            $(txtMargen).val(parseFloat($(txtMargen).val()).toFixed(2));
            return false;
            }

               if(parseFloat($(txtMargen).val())<=parseFloat($(hfMargen2).val()) )
            {
            $(txtMargen).val(parseFloat($(hfMargen).val()).toFixed(2));
             alertify.log("EL MARGEN GENERAL NO PUEDE SER MENOR AL MARGEN MINIMO"); 
            return false;
            }
...
function F_ActualizarMargen2(Fila) {
 try 
        {
            var txtMargen =  '#' + Fila;           
            var hfCodDetalle = txtMargen.replace('txtMargen2', 'hfCodDetalle');
            var hfCodDetalleOC = txtMargen.replace('txtMargen2', 'hfCodDetalleOC');
            var hfPrecio = txtMargen.replace('txtMargen2', 'hfPrecio');
            var hfCantidad = txtMargen.replace('txtMargen2', 'hfCantidad');
            var hfMargen = txtMargen.replace('txtMargen2', 'hfMargen');
            var hfMargen2 = txtMargen.replace('txtMargen2', 'hfMargen2');
            var txtCantidad = txtMargen.replace('txtMargen2', 'txtCantidad');
            var txtPrecio = txtMargen.replace('txtMargen2', 'txtPrecio');
            var lblProducto = txtMargen.replace('txtMargen2', 'lblProducto');
            var tasaigv;
            var FlagIgv = 0;
            var Contenedor = '#MainContent_';    
//            if(parseFloat($(txtPrecio).val())==parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val())==parseFloat($(hfCantidad).val()))
...
//            }
            ...
            return false;
                ...
                if (parseInt($(txtMargen).val())<= 0 |$(txtMargen).val()== '' )
            {
             $(txtMargen).val(parseFloat($(hfMargen2).val()).toFixed(2));
             alertify.log("EL MARGEN MINIMO NO PUEDE SER MENOR O IGUAL A 0");              
            return false;
            }
            ...
            return false;
               ...
               if(parseFloat($(hfMargen).val())<=parseFloat($(txtMargen).val()) )
            {
            $(txtMargen).val(parseFloat($(hfMargen2).val()).toFixed(2));
             alertify.log("EL MARGEN GENERAL NO PUEDE SER MENOR AL MARGEN MINIMO"); 
            return false;
            ...
            var objParams = {
                              ...
                              Filtro_Tasa: $('#MainContent_lblTC').text(),
                              Filtro_CodMoneda: $('#MainContent_ddlMoneda').val(),
                               Filtro_Margen: $(hfMargen).val(),
                               Filtro_Margen2: $(txtMargen).val(),
            };
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            {
               alertify.log("LA FACTURA " + $(lblNumero).text() + " NO SE PUEDE MODIFICAR, SE ENCUENTRA CANCELADO");
               return false;
            }
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (result.split('~')[33]!=0)
                {
                    alertify.log("LA FACTURA TIENE UNA NOTA DE CREDITO ASOCIADA");
                    return false;
                }                   
                    ...
                    $('#MainContent_txtFechaIngreso').val(result.split('~')[35]);
                    ...
                    $('#MainContent_ddlCajaFisica').val(result.split('~')[32]);

                    if (result.split('~')[36] == '1')
...
function F_FacturacionVentas() {
    ...
    if ($('#MainContent_chkNumeroVentas').is(':checked'))
        chkNumero='1';
    
    if ($('#MainContent_chkFechaVentas').is(':checked'))
        chkFecha='1';
...
function F_Reporte_Excel(CodMenu, Fila) {
if (F_PermisoOpcion(CodigoMenu, 3000205, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    var imgID = Fila.id;
    var lblCodigo = '#' + imgID.replace('imgImprimirExcel', 'lblCodigo');
    var NombreHoja = "Factura";
    var NombreArchivo = "Xls_FacturaCompra.xlsx";
    ...
    rptURL = rptURL + 'CodMovimiento=' + $(lblCodigo).val() + '&';
