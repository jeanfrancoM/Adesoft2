var AppSession = "../Ventas/RegistroCotizacionesTEK.aspx";
...
if (!F_SesionRedireccionar(AppSession)) return false;
    $.ajax({
        type: "GET",
        //contentType: "application/json; charset=utf-8",
        url: 'http://localhost:1428/api/Ubigeo_API_RUC_Controller/UBIGEO_LISTAR?nubigeo=' + $('#hfUbigeo').val() + "'",
//        data: "{'Ubigeo':'" + $('#hfUbigeo').val() + "'}",
       ...
       success: function (dbObject) {
             var data = dbObject.d;   
              $('#td_loading').css('display', 'none');
                    $('#hfCodDepartamento').val(dbObject.split(':')[1].split(',')[0]);
                        $('#hfCodProvincia').val(dbObject.split(':')[2].split(',')[0]);
                        $('#hfCodDistrito').val(dbObject.split(':')[3].split(',')[0]);
                    return true;
...
//        $("#hfMenorPrecioAgregar").val(Precio1);
//    }
...
//    $('#MainContent_txtPrecioDisplay').val(Precio1);
...
if (F_PermisoOpcion(CodigoMenu, 4000102, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
  var imgID = Fila.id;
        ...
        var TipoArchivo = 'application/pdf';
        var CodMenu = '209';
        var NombreArchivo = 'Web_Reporte_Ventas_rptCotizacion_Povis_A4.rpt';
