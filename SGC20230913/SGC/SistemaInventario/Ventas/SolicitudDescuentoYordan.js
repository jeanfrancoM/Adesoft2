var AppSession = "../Ventas/SolicitudDescuentoYordan.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_btnCancelarSD').click(function () {
     if (!F_SesionRedireccionar(AppSession)) return false;
        try {
            $('#div_ConfirmacionCambioPrecio').dialog('close');
        }
        catch (e) {
    ...
    $('#MainContent_btnRechazarSD').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
     try 
        {


        if (!F_ValirdarRechazar())
            return false;

            


        if (confirm("¿RECHAZAR SOLICITUD DE DESCUENTO POR " + $("#MainContent_txtPrecioSD").val() + " Y DEJAR PRECIO DE " + $("#MainContent_txtPrecioPermitidoSD").val() + "?"))    
        F_GuardarRechazo();
    ...
    $('#MainContent_btnAprobarSD').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
     try 
        {          

        if (confirm("¿APROBAR SOLICITUD DE DESCUENTO POR " + $("#MainContent_txtPrecioSDACEPTAR").val() + "?"))    
            F_GuardarAprobacion();
    ...
    $("#MainContent_txtFechaSD").css('background', '#FFFFE0');

    $("#MainContent_txtAlmacenSD").css('background', '#FFFFE0');

    $("#MainContent_txtUsuarioSD").css('background', '#FFFFE0');

    $("#MainContent_txtCantidadSD").css('background', '#FFFFE0');

    $("#MainContent_txtPrecioSD").css('background', '#FFFFE0');

    $("#MainContent_txtCodigoSD").css('background', '#FFFFE0');

    $("#MainContent_txtDescripcionSD").css('background', '#FFFFE0');

    $("#MainContent_txtCantidadSD").css('background', '#FFFFE0');

    $("#MainContent_txtImporteSD").css('background', '#FFFFE0');

    $("#MainContent_txtPrecioPermitidoSD").css('background', '#24C567');
    
    


    $("#MainContent_txtFechaSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtAlmacenSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtUsuarioSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtCantidadSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtPrecioSDACEPTAR").css('background', '#24C567');

    $("#MainContent_txtCodigoSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtDescripcionSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtCantidadSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtImporteSDACEPTAR").css('background', '#FFFFE0');

    $("#MainContent_txtPrecioPermitidoSDACEPTAR").css('background', '#FFFFE0');
    ...
    $("#div_ConfirmacionCambioPrecio").dialog({
        resizable: false,
        modal: true,
    ...
    $("#div_ConfirmacionCambioPrecioACEPTAR").dialog({
        resizable: false,
        modal: true,
        title: "Solicitud de Descuento",
        title_html: true,
    ...
    $('#div_ConfirmacionCambioPrecioACEPTAR').dialog('close');
...
$.each(Parametros, function (index, item) {
    ...
    switch(item.Parametro) {
        ...
        case "P_CANTIDAD_MINIMA_PRECIO2" :
            P_CANTIDAD_MINIMA_PRECIO2 = item.Valor;
            break;

        case "P_FILTRA_SERVICIOS" :
            P_FILTRA_SERVICIOS = item.Valor;
            break;
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    //  en el caso de yordan no habrá precio3, habrán precio1 y 2 para lista de precios 1, precio 3 y 4 para lista de precios 2
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var FlagIgv = 0;
        var SolicitarDescuento = 0;
        FilaActualizarValidaPermiso = "#" + Fila;
        PrecioActualizarValidarPermiso = $(hfPrecio).val();
        
        //si el usuario no esta autorizado para modificar precios
        if (UsuarioIniciado_PermisoCambioPrecios === '0') {


            if (Number($(txtCantidad).val()) < Number(P_CANTIDAD_MINIMA_PRECIO2)) {
                if (Number($(txtPrecio).val()) < Number($(hfP1).val())) {
                    $(txtPrecio).val((Number($(hfP1).val())).toFixed(2));
                    alertify.log("SOLO SE PUEDE DISMINUIR EL PRECIO A PARTIR DE " + P_CANTIDAD_MINIMA_PRECIO2 + " DEL PRODUCTO");
                    return false;                
                }
            } 

            if (Number($(txtCantidad).val()) >= Number(P_CANTIDAD_MINIMA_PRECIO2)) {
                if (Number($(txtPrecio).val()) < Number($(hfP2).val())) {
                    SolicitarDescuento = 1;
                    //alertify.log("SE HA ENVIADO SOLICITUD DE DESCUENTO AL SUPERVISOR, SI NO LO DESEA, VUELVA AL PRECIO ANTERIOR");
                }
            }
        }
        ...
        var objParams = {
            ...
            Filtro_Flag: 0,
            Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
            Filtro_SolicitarDescuento: SolicitarDescuento,
        };
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var hfP1 = txtCantidad.replace('txtCantidad', 'hfP1');
        var hfP2 = txtCantidad.replace('txtCantidad', 'hfP2');
...
var objParams = {
                ...
                MostrarEspera(true);
                F_BuscarSolicitudes_NET(arg, function (result) {
...
function F_AgregarTemporalCTxNumero() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleYordan.aspx/F_ObtenerCotizacion_PorNumero_Net',
        dataType: "json",
...
//            return false;
        ...
        $.ajax({
            ...
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            url: 'RegistroFacturaMultipleYordan.aspx/F_Listar_Detalle_Proforma_NET',
            dataType: "json",
...
function F_Inicializar_Tabla_Almacenes_Stocks() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleYordan.aspx/F_Inicializar_Tabla_Almacenes_Stocks_NET',
        dataType: "json",
...
return false;
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleYordan.aspx/F_Consulta_Cotizaciones_Pendientes_NET',
        dataType: "json",
...
function F_RechazarSolicitud (Control) {
    var TipoAccion = 'Rechazar';
    $('#lbTipoAccion').text("Rechazar solicitud de descuento ");
    var hfFechaSolicitud = '#' + Control.replace('imgRechazar', 'hfFechaSolicitud');
    var hfAlmacen = '#' + Control.replace('imgRechazar', 'hfAlmacen');
    var hfSolicitante = '#' + Control.replace('imgRechazar', 'hfSolicitante');
    var hfCodigoProducto = '#' + Control.replace('imgRechazar', 'hfCodigoProducto');
    var hfDescripcion = '#' + Control.replace('imgRechazar', 'hfDescripcion');
    var hfCantidad = '#' + Control.replace('imgRechazar', 'hfCantidad');
    var hfPrecio = '#' + Control.replace('imgRechazar', 'hfPrecio');
    var hfP1 = '#' + Control.replace('imgRechazar', 'hfP1');
    var hfP2 = '#' + Control.replace('imgRechazar', 'hfP2');
    var hfCodDetalle = '#' + Control.replace('imgRechazar', 'hfCodDetalle');
    var hfCodMoneda = '#' + Control.replace('imgRechazar', 'hfCodMoneda');
    var lblimporte = '#' + Control.replace('imgRechazar', 'lblimporte');

    CodDetalle = $(hfCodDetalle).val();
    Cantidad = $(hfCantidad).val();
    PrecioSistema = $(hfP2).val();
    CodMoneda = $(hfCodMoneda).val();

    $("#MainContent_txtFechaSD").val($(hfFechaSolicitud).val());
    $("#MainContent_txtAlmacenSD").val($(hfAlmacen).val());
    $("#MainContent_txtUsuarioSD").val($(hfSolicitante).val());
    $("#MainContent_txtCodigoSD").val($(hfCodigoProducto).val());
    $("#MainContent_txtDescripcionSD").val($(hfDescripcion).val());
    
    $("#MainContent_txtCantidadSD").val($(hfCantidad).val());
    $("#MainContent_txtImporteSD").val($(lblimporte).text());

    $("#MainContent_txtPrecioSD").val($(hfP2).val());
    $("#MainContent_txtPrecioPermitidoSD").val($(hfP2).val());
    



    //$('#div_ConfirmacionCambioPrecio').dialog('open');

    F_GuardarRechazo();
return true
}

function F_ValirdarRechazar () {
    var retorno = true;

    if ( isNaN( $('#MainContent_txtPrecioPermitidoSD').val() )) {
        alertify.log("NO ES UN PRECIO VALIDO");
        $('#MainContent_txtPrecioPermitidoSD').val(PrecioSistema);
        retorno = false;
    }


    if ( Number($('#MainContent_txtPrecioPermitidoSD').val() ) < Number(PrecioSistema) ) {
        if (confirm("EL PRECIO ESPECIFICADO "  + $('#MainContent_txtPrecioPermitidoSD').val() + " ES MENOR " +
            "AL PRECIO 2 DEL SISTEMA : " + PrecioSistema + " ¿SEGURO DE PROCESAR?"))    {
...
function F_GuardarRechazo() {
    ...
    //cuando se rechaza en su totalidad y se mantiene el precio 2
    var TipoAccion = 'R';

    //cuando lo rechaza pero modifica el precio y este es menor al precio 2
    if (Number($("#MainContent_txtPrecioPermitidoSD").val()) < Number(PrecioSistema))
        TipoAccion = 'M'; //entonces es una modificacion
    

        var objParams = {
            Filtro_Precio: Number($("#MainContent_txtPrecioPermitidoSD").val()) / tasaigv,
            Filtro_Cantidad: Cantidad,
            Filtro_CodDetDocumentoVenta: CodDetalle,
            Filtro_CodMoneda: CodMoneda,
            Filtro_Descripcion: $("#MainContent_txtDescripcionSD").val(),
            Filtro_TasaIgv: tasaigv,
            Filtro_NotaPedido: 0,
            Filtro_FlagIgv: FlagIgv,
            Filtro_Flag: 0,
            Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
            Filtro_TipoAccion: TipoAccion,
        };

        var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
        MostrarEspera(true);
        F_GestionarSolicitud_Net(arg, function (result) {
...
var Cantidad;
var CodMoneda;
function F_AceptarSolicitud (Control) {
    var TipoAccion = 'Aprobar';
    $('#lbTipoAccionACEPTAR').text("ACEPTAR solicitud de descuento ");
    var hfFechaSolicitud = '#' + Control.replace('imgAprobar', 'hfFechaSolicitud');
    var hfAlmacen = '#' + Control.replace('imgAprobar', 'hfAlmacen');
    var hfSolicitante = '#' + Control.replace('imgAprobar', 'hfSolicitante');
    var hfCodigoProducto = '#' + Control.replace('imgAprobar', 'hfCodigoProducto');
    var hfDescripcion = '#' + Control.replace('imgAprobar', 'hfDescripcion');
    var hfCantidad = '#' + Control.replace('imgAprobar', 'hfCantidad');
    var hfPrecio = '#' + Control.replace('imgAprobar', 'hfPrecio');
    var hfP1 = '#' + Control.replace('imgAprobar', 'hfP1');
    var hfP2 = '#' + Control.replace('imgAprobar', 'hfP2');
    var hfCodDetalle = '#' + Control.replace('imgAprobar', 'hfCodDetalle');
    var hfCodMoneda = '#' + Control.replace('imgAprobar', 'hfCodMoneda');
    var lblimporte = '#' + Control.replace('imgAprobar', 'lblimporte');
    var txtPrecio = '#' + Control.replace('imgAprobar', 'txtPrecio');
    ...
    if (isNaN($(txtPrecio).val()) || $(txtPrecio).val().trim() === "") {
        alertify.log("PRECIO INCORRECTO");
        $(txtPrecio).val($(hfPrecio).val());
        return true;
    }

    $("#MainContent_txtFechaSDACEPTAR").val($(hfFechaSolicitud).val());
    $("#MainContent_txtAlmacenSDACEPTAR").val($(hfAlmacen).val());
    $("#MainContent_txtUsuarioSDACEPTAR").val($(hfSolicitante).val());
    $("#MainContent_txtCodigoSDACEPTAR").val($(hfCodigoProducto).val());
    $("#MainContent_txtDescripcionSDACEPTAR").val($(hfDescripcion).val());

    $("#MainContent_txtCantidadSDACEPTAR").val($(hfCantidad).val());
    $("#MainContent_txtImporteSDACEPTAR").val($(lblimporte).text());

    $("#MainContent_txtPrecioSDACEPTAR").val($(txtPrecio).val());
    $("#MainContent_txtPrecioPermitidoSDACEPTAR").val($(hfP2).val());

    //$('#div_ConfirmacionCambioPrecioACEPTAR').dialog('open');
...
function F_GuardarAprobacion() {
    ...
    var TipoAccion = 'A';  
        ...
        var objParams = {
            Filtro_Precio: Number($("#MainContent_txtPrecioSDACEPTAR").val()) / tasaigv,
            Filtro_Cantidad: Cantidad,
            Filtro_CodDetDocumentoVenta: CodDetalle,
            Filtro_CodMoneda: CodMoneda,
            Filtro_Descripcion: $("#MainContent_txtDescripcionSD").val(),
