...
//    });
    ...
    $("#MainContent_txtCliente").bind("propertychange change keyup paste input", function(){
        if ($("#MainContent_txtCliente").val() != $("#hfCliente").val() & $("#hfCodCtaCte").val() != '0' & $("#MainContent_txtCliente").val() === '---NUEVO CLIENTE---')
        {
            ...
            {
                toastr.warning('NO SE PUEDE MODIFICAR CLIENTES REGISTRADOS');
...
//           $('.Jq-ui-dtp').datepicker('setDate', new Date());
    ...
    if ($('#MainContent_ddlTipoDoc').val() != 16) {
        toastr.warning('SOLO ES PUEDEN AGREGAR PRODUCTOS EN LAS PROFORMAS');
        return false;
    }
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
        try 
        {
            ...
            if ($.isNumeric($('#MainContent_txtDescuentoPorcentaje').val()))
            {
                var Dcto = $('#MainContent_txtDescuentoPorcentaje').val();
                if (Dcto > 0 & Dcto < 100)
                {
                ...
                {
                    toastr.warning('AGREGE UN PORCENTAJE DE DESCUENTO VALIDO');
                    $('#MainContent_txtDescuentoPorcentaje').val('0.00');
                    $('#MainContent_txtDescuentoPorcentaje').focus();
                }
            }
            else
            {
                toastr.warning('INGRESE UN PORCENTAJE DE DESCUENTO VALIDO');
                $('#MainContent_txtDescuentoPorcentaje').val('0.00');
                $('#MainContent_txtDescuentoPorcentaje').focus();
            }
        }
    ...
    $('#MainContent_btnNotaVenta').click(function () {
        ...
        if ($('#MainContent_ddlTipoDoc').val() == "16") {
                toastr.warning("opcion valida solo para Facturas y boletas");
                return false;
            }
...
//            $('#div_ImpresorasNotaPedido').dialog('close');
    ...
    $('#MainContent_txtDescuentoPorcentaje').css('background', '#FFFFE0');
    ...
    $("#MainContent_txtSerieOCEdicion").css('background', '#FFFFE0');
 ...
 try 
        ...
        if (P_VALIDASTOCK == "1") {
            if ((Number($("#MainContent_txtStockAgregar").val()) -  Number($("#MainContent_txtCantidad").val())) < Number(P_VALIDASTOCK_MONTO_MINIMO)) {
                toastr.warning("LA CANTIDAD SUPERA AL MONTO DE VALIDACION MINIMO");
                return false;
            }
        }
            ...
            try {
                ...
                if (data.MsgError === "") 
                {
                    ...
                    if (TienePermiso === '1')
                    {
                        if (data.ClavePrecio.trim() === $("#MainContent_txtContraseñaPrecio").val().trim()) 
                        {
                            ...
                            if (FilaActualizarValidaPermiso != '')
                                ...
                                toastr.warning('NO POSEE CLAVE PARA CAMBIO DE PRECIO');   
                            }
                            else {
                            if (FilaActualizarValidaPermiso != '')
                                $(FilaActualizarValidaPermiso).val(PrecioActualizarValidarPermiso);
                    ...
                    else
                    {
                        if (FilaActualizarValidaPermiso != '')
                            $(FilaActualizarValidaPermiso).val(PrecioActualizarValidarPermiso);
                        toastr.warning('USUARIO SIN PERMISO PARA AUTORIZAR CAMBIOS DE PRECIOS');
                    }
...
function F_GuardarCambiosFactura() {
 try 
    ...
    var objParams = {
                      ...
                      Filtro_MarcaGuia:     $('#MainContent_txtMarcaGuiaEdicion').val(),
...
function F_ActualizarPrecio(Fila, campo) {
    try {
        ...
        var hfP1                = txtPrecio.replace(campo, 'hfP1');
        ...
        var txtPrecio           = txtPrecio.replace(campo, 'txtPrecio');
        var FlagIgv = 0;
...
//        }
        ...
        var Descuento = Number($(txtDescuento).val()); 
