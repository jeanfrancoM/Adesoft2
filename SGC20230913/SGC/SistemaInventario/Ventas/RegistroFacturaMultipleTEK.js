var AppSession = "../Ventas/RegistroFacturaMultipleTEK.aspx";
    ...
    $('#MainContent_txtConductorDNI').autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                url: '../Servicios/Servicios.asmx/F_ListarClientes_AutoComplete',
                data: "{'NroRuc':'" + "" + "','RazonSocial':'" + request.term + "','CodTipoCtaCte':'" + 4 + "','CodTipoCliente':'" + 1 + "'}",
                dataType: "json",
                async: true,
                success: function (data) {
                    response($.map(data.d, function (item) {
...
//            $('#div_ImpresorasNotaPedido').dialog('close');
    ...
    return false;
        var id='0000' + $('#MainContent_txtSerieOC').val();
            $('#MainContent_txtSerieOC').val(id.substr(id.length - 4));   
       return false;
    });
    ...
    return false;
        var id='00000000' + $('#MainContent_txtNumeroOC').val();
            $('#MainContent_txtNumeroOC').val(id.substr(id.length - 8));   
       return false;
    });
...
//      $("#MainContent_grvConsultaArticulo tbody tr:nth-child(2n)").addClass("alt");

 //     $('.MainContent_grvConsultaArticulo tr:nth-child(odd)').addClass('alt');
//      $("table#MainContent_grvConsultaArticulo tr:even").css("background-color", "#F4F4F8");
//  $("table#MainContent_grvConsultaArticulo tr:odd").css("background-color", "#EFF1F1");
//         
});
        ...
        {        
            ...
            var Cadena = 'Ingresar los sgtes. Datos:'; 
             ...
             if ($(Cuerpo + 'txtCliente').val()=='' && $('#hfCodCtaCte').val()==0 && $("#MainContent_ddlTipoDoc").val() == 1)
                    ...
                    if ($(Cuerpo + 'txtCliente').val()=='---NUEVO CLIENTE---')
                    Cadena=Cadena + '<p></p>' + 'Cliente';
...
if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    var lblSaldo = '#' + imgID.replace('imgAnularDocumento', 'lblSaldo');
        ...
        if ($(lblEstado).text() != "PENDIENTE") {
            alertify.log("SOLO SE PUEDE ANULAR CUANDO ESTE PENDIENTE EL DOCUMENTO");
            return false;
        }

          if (parseFloat($(lblTotal).text()) != parseFloat($(lblSaldo).text())) { 
            alertify.log("ELIMINE LA COBRANZA REGISTRADA, PARA ANULAR EL DOCUMENTO");
            return false;
        }
     ...
     $('#lblfactanu').text($(lblnumero_grilla).text());

        $('#div_Anulacion').dialog({
               resizable: false,
               modal: true,
               title: "Anulacion de Factura de Venta",
               title_html: true,
               height: 190,
               width: 470,
               autoOpen: false
