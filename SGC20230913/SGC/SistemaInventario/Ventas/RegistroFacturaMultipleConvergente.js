var AppSession = "../Ventas/RegistroFacturaMultipleConvergente.aspx";
          ...
          var tasaigv = 1;
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                        ...
                        $('#hfCodigoTemporal').val(result.split('~')[3]);
                        var SubTotal = parseFloat(result.split('~')[6]) + parseFloat(result.split('~')[7]);
                        $('#MainContent_txtTotal').val(parseFloat(SubTotal).toFixed(2));
                        $('#MainContent_txtIgv').val(parseFloat(result.split('~')[6]).toFixed(2));
                        $('#MainContent_txtSubTotal').val(parseFloat(result.split('~')[7]).toFixed(2));
                        $('#MainContent_txtAcuentaNV').val(parseFloat(result.split('~')[8]).toFixed(2));
...
function F_VerPDF(){
    var CodFacturas = $("#MainContent_grvConsulta input:checkbox:checked").not("#MainContent_grvConsulta_ChkAll").map(function(){       
        var fila = this.id;
        var lblCodigo = "#"+fila.replace('chkPdf','lblCodigo');
        return $(lblCodigo).val();
    }).get();
    if (CodFacturas.length!=0){
      var json={
        Codigos:CodFacturas.toString(),
        CodTipoDoc:1
      }
      $.ajax({
        type:"POST",
