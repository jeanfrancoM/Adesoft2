var AppSession = "../Ventas/RegistroCotizacionesReeim.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_btnDescuentoTotal').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
        try 
        {
...
function F_GrabarDocumento(){
  try 
        ...
        var FlagConCodigo = 0; if ($('#MainContent_chkConCodigo').is(':checked')) { FlagConCodigo = 1;};        
            ...
            Filtro_FlagComisionable: FlagComisionable
                ...
                {                    
                    if (str_mensaje_operacion=='Se Grabo Correctamente')
                    {
                        ...
                        F_VistaPreliminar(result.split('~')[2]);                     
                        F_Nuevo();                   
                    }
                    else
                    {
...
function F_Nuevo(){
            ...
            $('.ccsestilo').css('background', '#FFFFE0'); 
            $('#MainContent_txtNroRuc').focus();
            $('#MainContent_txtDescuentoTotal').val('0.00');
...
function getContentTab(){
    $('#MainContent_txtDesde').val(Date_AddDays($('#MainContent_txtHasta').val(), -15));
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var lblPrecioOriginal = txtPrecio.replace('txtPrecio', 'lblPrecioOriginal');
        var FlagIgv = 0;
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var lblPrecioOriginal = txtCantidad.replace('txtCantidad', 'lblPrecioOriginal');
        var Flag = 0;
        var FlagIgv = 1;
...
//        }
        ...
        var objParams = {
            ...
            Filtro_PrecioOriginal: $(lblPrecioOriginal).text() / tasaigv,
            Filtro_Cantidad: $(txtCantidad).val(),
            Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
            Filtro_Descripcion: $(txtDescripcion).val(),
            Filtro_CodDetDocumentoVenta: $(lblCodDetalle).val(),
...
function F_ImprimirDocumento(codigo,Fila,rplc,TipoImp,TipoDoc) {
        ...
        F_VistaPreliminar($(lblCodigo).val());
        return false;
}
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
    ...
    if ($(lblEstado).text() == 'ANULADO')
    {
        alertify.log('NO SE PUEDE MODIFICAR UNA COTIZACION ANULADA');
        return false;
    }
            
    //if ($(lblEstado).text() == 'ANULADO' | $(lblEstado).text() == 'FACTURADO' | $(lblEstado).text() == 'GENERADO')
    //{
    //    alertify.log('NO SE PUEDE MODIFICAR UNA COTIZACION ANULADA');
    //    return false;
    //}
    ...
    $.ajax({
        ...
        success: function (dbObject) {
            ...
            try {
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {                        
                        ...
                        $('#MainContent_txtReferencia').val(data.Referencia);
                        $('#MainContent_txtObservacion').val(data.Observacion);
                        $('#MainContent_txtNumero').val(data.Numero);
                        $('#MainContent_txtVigencia').val(data.VencimientoStr);
            ...
            catch (x) { alertify.log("ERROR DETECTADO: " + e); }
            MostrarEspera(false);
        },
        error: function (xhr, ajaxOptions, thrownError) {
...
function F_VistaPreliminar(Codigo) {
...
function F_InicializarCajaText()
{
    $("#MainContent_txtNumeroConsulta").ForceNumericOnly();
    ...
    $('#MainContent_txtDescuentoTotal').css('background', '#FFFFE0');

    return false;
}

function F_ActualizarDescuento() {
    try {
        var tasaigv = 1;
        ...
        var objParams = {
            Filtro_Descuento: (1-(parseFloat($('#MainContent_txtDescuentoTotal').val())/100)),      
            Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
            Filtro_CodMoneda: $('#MainContent_ddlMoneda').val(),
            Filtro_TasaIgv: tasaigv,
            Filtro_NotaPedido: 0,
        ...
        var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
        MostrarEspera(true);
        F_ActualizarDescuento_Net(arg, function (result) {
