var AppSession = "../Ventas/RegistroFacturaMultipleFundicion.aspx";
...
//            if ($('#MainContent_txtDestino').val().trim() == ',') $('#MainContent_txtDestino').val('');
    ...
    $('#MainContent_btnNotaVenta').click(function () {
        ...
        Anticipo = 0;
        F_AbrirPanelNV('Facturacion Nota de Venta');
        $('#MainContent_btnNotaVenta').focus();
        return false;
    });

    $('#MainContent_btnAnticipo').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if (F_PermisoOpcion(CodigoMenu, 4000202, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        if (!($('#MainContent_ddlTipoDoc').val()=="1" | $('#MainContent_ddlTipoDoc').val()=="2")) return false;
        $('#MainContent_txtDesdeNV').val(Date_AddDays($('#MainContent_txtHastaNV').val(), -7));
        Anticipo = 1;
        F_AbrirPanelNV('ANTICIPO');
        $('#MainContent_btnNotaVenta').focus();
        return false;
    });
 ...
 try 
        ...
        if ($("#MainContent_txtCantidad").val() == '')
            cadena = cadena + "CAMPO CANTIDAD <p></p> ";
              ...
              if ($("#MainContent_txtPrecioDisplay").val() == '0')
            cadena = cadena + "INGRESE PRECIO <p></p> ";

             if (parseFloat($("#MainContent_txtPrecioDisplay").val()) < '0')
            cadena = cadena + "EL PRECIO NO PUEDE SER NEGATIVO<p></p> ";

            if ($("#MainContent_txtCantidad").val() == '0')
            cadena = cadena + "INGRESE CANTIDAD <p></p> ";

             if (parseFloat($("#MainContent_txtCantidad").val()) < '0')
            cadena = cadena + "LA CANTIDAD NO PUEDE SER NEGATIVO<p></p> ";
...
function F_ActualizarPrecio(Fila) {
    try {
       ...
       if (parseInt($(hfCodTipoDoc).val())==16 & parseFloat($(txtPrecio).val())!=parseFloat($(hfPrecio).val()) )
           ...
           if (parseFloat($(txtPrecio).val()) <= '0' | $(txtPrecio).val() == ''){
        
            $(txtPrecio).val(parseFloat($(hfPrecio).val()).toFixed(2));
             alertify.log("EL PRECIO NO PUEDE SER MENOR O IGUAL A CERO"); 
                return false;
        }
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        if (parseFloat($(txtPrecio).val()) == parseFloat($(hfPrecio).val()) & parseFloat($(txtCantidad).val()) == parseFloat($(hfCantidad).val())) {
         ...
         if (parseFloat($(txtCantidad).val()) <= '0' | $(txtCantidad).val() == ''){
        
            $(txtCantidad).val(parseFloat($(hfCantidad).val()).toFixed(2));
             alertify.log("LA CANTIDAD NO PUEDE SER MENOR O IGUAL A CERO"); 
                return false;
        }
...
function F_ValidarCheckSinIgvMaestro(ControlID) {
    ...
    if ($(chkok_grilla).is(':checked')) {
        ...
        $('#MainContent_chkSinIgv').prop('checked', true);
            ...
            $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
            chkSi = '#' + this.id;
            txtCantidad = chkSi.replace('chkEliminar', 'txtCantidad');
...
function F_AbrirPanelNV(CADENA) {
...
function F_AgregarTemporalNV() {
    try {
        ...
        $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            ...
            hfCodMovimiento = chkSi.replace('chkEliminar', 'hfCodMovimiento');
            ...
            if ($(chkSi).is(':checked')) {
                ...
                var objDetalle = {
                    CodDocumentoVentaAnticipo: $(hfCodMovimiento).val(),
                    CodArticulo: $(lblcodproducto_grilla).val(),
                    Cantidad: $(txtcantidad_grilla).val(),
