var AppSession = "../Ventas/RegistroNotaDebitoAra.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_btnGrabar').click(function () {
        ...
        try {
            ...
            if (confirm("ESTA SEGURO DE GRABAR LA NOTA DE DEBITO"))
                F_GrabarDocumento();
    ...
    $('#MainContent_btnEliminar').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if (F_PermisoOpcion(CodigoMenu, 4000902, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try {
    ...
    $('#MainContent_btnFactura').click(function () {
        if (!F_SesionRedireccionar(AppSession)) return false;
        if (F_PermisoOpcion(CodigoMenu, 4000901, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        $('#div_Factura').dialog({
            resizable: false,
            modal: true,
            title: "Factura",
            ...
            if (str_resultado_operacion == "1") {
                if (str_mensaje_operacion == 'Se grabo correctamente') {
                    if ($('#MainContent_chkImpresion').is(':checked')) {
                        var p1 = ''; var ti = 'IMP';
                        F_ImprimirDocumento(result.split('~')[2], '', p1, ti, 3);
                    }
...
function F_EditarRegistro(Fila) {
    ...
    try {
        ...
        if ($(Contenedor + 'ddlTipoOperaciones').val() == 10) {
            $(Contenedor + 'txtPrecio').prop('readonly', false);
            $(Contenedor + 'txtCantidad').prop('readonly', true);
        }
        else if ($(Contenedor + 'ddlTipoOperaciones').val() == 11) {
            $(Contenedor + 'txtPrecio').prop('readonly', false);
            $(Contenedor + 'txtCantidad').prop('readonly', true);
        }
        else if ($(Contenedor + 'ddlTipoOperaciones').val() == 12) {
            $(Contenedor + 'txtPrecio').prop('readonly', false);
            $(Contenedor + 'txtCantidad').prop('readonly', true);
        }
        else {
...
function F_ValidarEdicionItem() {
    ...
    try {
        ...
        if (parseFloat($('#MainContent_txtPrecio').val()) < parseFloat($('#hfPrecio').val()) & $('#MainContent_ddlTipoOperaciones').val() != 3)
            Cadena = Cadena + '<p></p>' + 'El Precio no debe ser menor al Precio original';
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        ...
        if ($(lblEstado).text() == "CANCELADO PARCIAL") {
            alertify.log("Este documento se encuentra cancelado parcial; primero elimine la cobranza y luego anule la nota de debito");
            return false;
        }

        if ($(lblEstado).text() == "CANCELADO TOTAL") {
            alertify.log("Este documento se encuentra cancelado total; primero elimine la cobranza y luego anule la nota de debito");
            return false;
        }

        if (!confirm("ESTA SEGURO DE ELIMINAR LA NOTA DE DEBITO : " + $(lblNumero).text() + "\nDEL CLIENTE : " + $(lblCliente).text()))
            return false;
...
function F_AnularRegistro(Fila) {
    ...
    try {
        ...
        if (!confirm("ESTA SEGURO DE ANULAR LA NOTA DE DEBITO : " + $('#hfNumeroAnulacion').val() + "\nDEL CLIENTE : " + $('#hfClienteAnulacion').val()))
            return false;
...
function F_AnularPopUP(Fila) {
    ...
    if ($(lblEstado).val() == "ANULADO") {
        alertify.log("LA NOTA DE DEBITO SE ENCUENTRA ANULADA");
        return false;
    }

    if ($(lblEstado).val() == "CANCELADO TOTAL") {
        alertify.log("ESTA NOTA DE DEBITO SE ENCUENTRA CANCELADA TOTAL; PRIMERO ELIMINE LA COBRANZA Y LUEGO ANULE LA NOTA DE DEBITO");
        return false;
    }
    ...
    $('#div_Anulacion').dialog({
        resizable: false,
        modal: true,
        title: "Anulacion de Nota de Debito",
        title_html: true,
        height: 190,
        width: 470,
        autoOpen: false
