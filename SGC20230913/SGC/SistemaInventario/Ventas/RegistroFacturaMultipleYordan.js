var AppSession = "../Ventas/RegistroFacturaMultipleYordan.aspx";
...
//         
    ...
    //invisibilizar
    var PermisoVerStock = F_PermisoOpcion_SinAviso(CodigoMenu, '777003', '');
    if (PermisoVerStock == '0') {
        $('#MainContent_txtStockAgregar').css('display', 'none');
        $('#tdStock').css('display', 'none');
    }
...
function F_Controles_Inicializar() {
    ...
    if (P_FILTRA_SERVICIOS == "0") {
        $('#MainContent_chkServicios').text("Descripcion");
        $('#MainContent_chkServicios').css("display", "none");
        }        
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        $('#MainContent_ddlTipoDoc').val(2);
                        $('#MainContent_ddlTipoDoc2').val(2);
                        $('#MainContent_ddlTipoDoc').css('background', '#FFFFE0');
                        $('#MainContent_ddlTipoDoc2').css('background', '#FFFFE0');
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    txtStockAgregar = $(lblStock_grilla).val();
    ...
    if ($(hfPrecio2_grilla).val() != '0.00') {
        ...
        if (Number($(hfPrecio2_grilla).val()) < Number($("#hfMenorPrecioAgregar").val()) & Number($(hfPrecio2_grilla).val()) > 0) 
        {
            ...
            $("#hfMayorPrecioAgregar").val($(hfPrecio2_grilla).val());
            $("#hfMenorPrecioAgregar").val($(hfPrecio1_grilla).val());        
        }
    }
...
return true;
 ...
 try 
        ...
        if (P_VALIDASTOCK == "1") {
            if ((Number(txtStockAgregar) -  Number($("#MainContent_txtCantidad").val())) < Number(P_VALIDASTOCK_MONTO_MINIMO)) {
                alertify.log("STOCK INSUFICIENTE");
                return false;
            }
        }
        ...
        if (UsuarioIniciado_PermisoCambioPrecios === '0') {
            ...
            if (Number($("#MainContent_txtCantidad").val()) < Number(P_CANTIDAD_MINIMA_PRECIO2)) {
                if (Number($("#MainContent_txtPrecioDisplay").val()) < Number($("#hfMayorPrecioAgregar").val())) {
                    $("#MainContent_txtPrecioDisplay").val((Number($("#hfMayorPrecioAgregar").val())).toFixed(2));
...
function F_AgregarTemporal() {
    ...
    try {
        ...
        var xSolicitarDescuento = 0;
        if (Number($("#MainContent_txtCantidad").val()) >= Number(P_CANTIDAD_MINIMA_PRECIO2)) {
            if (Number($("#MainContent_txtPrecioDisplay").val()) < Number($("#hfMenorPrecioAgregar").val())) {
                xSolicitarDescuento = 1;
                alertify.log("SE HA ENVIADO SOLICITUD DE DESCUENTO AL SUPERVISOR, SI NO LO DESEA, VUELVA AL PRECIO ANTERIOR");
            }
        }
        ...
        var objParams = {
            ...
            Filtro_Servicio: chkServicio,
            Filtro_NotaPedido: chkNotaPedido,
            Filtro_SolicitarDescuento: xSolicitarDescuento,
            Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle)
        };
...
//            }
           ...
           if (!F_ValidarCorreo($(Cuerpo + 'txtCorreo').val()))
            ...
            $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                ...
                var hfSolicitudDescuento = chkSi.replace('chkEliminar', 'hfSolicitudDescuento');
                var hfDescuentoAprobado = chkSi.replace('chkEliminar', 'hfDescuentoAprobado');

                if ($(hfSolicitudDescuento).val() === "1") {
                    if ($(hfDescuentoAprobado).val() === "" | $(hfDescuentoAprobado).val() === "0") {
                        PorProbar++;
                    }
                }
            });
            ...
            if (PorProbar > 0)
                Cadena=Cadena + '<p></p>' + 'SOLICITUDES DE DESCUENTO EN ESPERA' + '<p></p>';
...
function F_SoloActualizarDetalle(){
  try 
        {
        if($('#MainContent_txtTotal').val()=='0.00')
        return;
        ...
        if ($('#MainContent_chkConIgvMaestro').is(':checked')) { 
             ...
             FlagIgv = 1;         
                ...
                MostrarEspera(true);
                F_SoloActualizarDetalle_NET(arg, function (result) {
...
function F_Inicializar_Tabla_Almacenes_Stocks() {
    ...
    $.ajax({
        ...
        success: function (dbObject) {
            ...
            try {
                $.each(data.rows, function (index, item) {
                    Cuerpo += Row.replace("@Almacen", item.DscAlmacen)
...
function F_VerArchivoCDR (Control) {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleYordan.aspx/F_ObtenerArchivoCDR_NET',
        dataType: "json",
