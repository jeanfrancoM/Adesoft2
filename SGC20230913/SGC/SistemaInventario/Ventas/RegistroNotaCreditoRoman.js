...
/*function F_AnularRegistro() {
    if (F_PermisoO
pcion(CodigoMenu, CodigoInterno, 'Consultar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
    try {

        if (confirm("Esta seguro de anular la cobranza de la factura : " + "\n" + $(lblNumero).text() + " del cliente " + $('#hfClienteAnulacion').val()) == false)
            return false;
...
function F_AnularPopUP(Fila) {
    ...
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Anular') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
    var imgID = Fila.id;
    var hfCodigo = '#' + imgID.replace('imgAnularDocumento', 'lblCodigo');
    ...
    $('#div_Anulacion').dialog({
        resizable: false,
        modal: true,
        title: "Eliminacion de Nota de Credito de Venta",
        title_html: true,
        height: 190,
        width: 470,
        autoOpen: false
