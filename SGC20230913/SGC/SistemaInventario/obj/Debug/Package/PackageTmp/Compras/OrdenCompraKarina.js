var AppSession = "../Compras/OrdenCompraKarina.aspx";
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if ($(lblEstado).text()=='FACTURADO')
            {
               alertify.log("LA ORDEN DE COMPRA " + $(lblNumero).text() + " NO SE PUEDE MODIFICAR, SE ENCUENTRA FACTURADO");
               return false;
            }

               if ($(lblEstado).text()=='CANCELADO TOTAL')
            {
               alertify.log("LA ORDEN DE COMPRA " + $(lblNumero).text() + " NO SE PUEDE MODIFICAR, SE ENCUENTRA CANCELADO");
               return false;
            }

             if ($(lblEstado).text()=='ANULADO')
            {
               alertify.log("LA ORDEN DE COMPRA " + $(lblNumero).text() + " NO SE PUEDE MODIFICAR, SE ENCUENTRA ANULADO");
               return false;
            }
...
if (!F_SesionRedireccionar(AppSession)) return false;
       ...
       var lblcliente_grilla = '#' + imgID.replace('imgAnularDocumento', 'lblcliente');
        ...
        if ($(lblEstado).text()=="ANULADO") 
        {alertify.log("ESTE ORDEN DE COMPRA YA SE ENCUENTRA ANULADA");
        return false;}
