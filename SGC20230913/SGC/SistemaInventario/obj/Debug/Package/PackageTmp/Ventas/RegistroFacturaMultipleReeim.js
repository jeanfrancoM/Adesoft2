var AppSession = "../Ventas/RegistroFacturaMultipleReeim.aspx";
...
var P_FACTURA_ITEMS_MAXIMO_DE_COTIZACION;
...
//    });
    ...
    k.down("enter", function () {
            ...
            if (idx == inputs.length - 1) {
                ...
                if (inputs[idx].id.indexOf("MainContent_grvDetalleArticulo_txtDescripcion") > 0 |
                        inputs[idx].id.indexOf("MainContent_grvDetalleArticulo_txtCantidad") > 0 |
                        inputs[idx].id.indexOf("MainContent_grvDetalleArticulo_txtPrecio") > 0)
                        {
                        return true;
                        }
...
function F_Inicializar_Parametros() {
    ...
    P_CantidadPlacas = "1";
    P_FACTURA_ITEMS_MAXIMO_DE_COTIZACION = "20";
...
$.each(Parametros, function (index, item) {
    ...
    switch(item.Parametro) {
        ...
        case "P_CANTIDADPLACAS" :
            ...
            break;
        case "P_FACTURA_ITEMS_MAXIMO_DE_COTIZACION" :
            P_FACTURA_ITEMS_MAXIMO_DE_COTIZACION = item.Valor;
            break;
    };
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    lblPrecioSoles   = imgAgregar_grilla.replace('imgAgregar', 'hfPrecioSoles');
    lblPrecioDolares = imgAgregar_grilla.replace('imgAgregar', 'hfPrecioDolares');
    ...
    if ($('#MainContent_ddlMoneda').val()==1)
        $('#MainContent_txtPrecioDisplay').val($(lblPrecioSoles).val());
    else
        $('#MainContent_txtPrecioDisplay').val($(lblPrecioDolares).val());
    ...
    if ($('#MainContent_ddlFormaPago').val() != "11" & $('#MainContent_ddlTipoDoc').val() == "5") {
     ...
     switch (CodFormaPago)
     {
            ...
            case "14": //
               ...
               break;
            case "15":
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),40));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "16": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),50));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "17": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),0));
            ...
            case "19": //
               ...
               break;
            case "20": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),2));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "21": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),5));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "22": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),10));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "23": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),12));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "24": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),20));
                       $('#MainContent_txtNroOperacion').val('');
               break;
            case "25": //
                       $('#MainContent_txtVencimiento').val(Date_AddDays($('#MainContent_txtEmision').val(),25));
                       $('#MainContent_txtNroOperacion').val('');
               break;
     }
    }
...
function F_MostrarTotales() {
    ...
    $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
        ...
        hfPrecioNormal = chkDel.replace('chkEliminar', 'hfPrecioNormal');
        Total += parseFloat($(txtCantidad).val())*parseFloat($(hfPrecioNormal).val());
    });

    if(isNaN(Total))
      Total = 0;

    $(Cuerpo + 'txtTotal').val(parseFloat(Total).toFixed(2));
    $(Cuerpo + 'txtSubTotal').val(parseFloat(parseFloat(Total)/(parseFloat($("#MainContent_ddlIgv option:selected").text())+1)).toFixed(2));
    $(Cuerpo + 'txtIgv').val(parseFloat(parseFloat(parseFloat(Total)/(parseFloat($("#MainContent_ddlIgv option:selected").text())+1)) * parseFloat($("#MainContent_ddlIgv option:selected").text())).toFixed(2));
            ...
            Filtro_FlagComisionable: FlagComisionable
                ...
                if (str_resultado_operacion == "1") 
                {
                    if (str_mensaje_operacion=='Se Grabo Correctamente')
                    {
                        ...
                        F_ImprimirDespacho(result.split('~')[2]);
                        F_Nuevo();                   
                    }
                    else
                    {
...
//        }
        ...
        if (parseFloat($(txtPrecio).val()) == parseFloat($(hfPrecio).val())) {
            $(txtPrecio).val(parseFloat($(txtPrecio).val()).toFixed(4));
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var hfPrecioNormal = txtCantidad.replace('txtCantidad', 'hfPrecioNormal');
        var Flag = 0;
        var FlagIgv = 0;
...
function F_AgregarTemporalCTxNumero() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleReeim.aspx/F_ObtenerCotizacion_PorNumero_Net',
        dataType: "json",
        ...
        var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                    ...
                    $.each(data.lProformaDet, function (index, item) {
                        ...
                        if (CountAgregados <= Number(P_FACTURA_ITEMS_MAXIMO_DE_COTIZACION) & item.CantidadEntregada < item.Cantidad) {
                            var objDetalle = {
...
function F_ActualizarDescripcion(Fila) {
    try {
        ...
        var objParams = {
            Filtro_Precio: $(hfPrecioNormal).val() / tasaigv,
            Filtro_Cantidad: $(txtCantidad).val(),
            Filtro_Descripcion: $(txtDescripcion).val(),
            Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
            Filtro_CodDetDocumentoVenta: $(hfCodDetalle).val(),
            ...
            try {
                ...
                $.each(data.rows, function (index, item) {
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'COTIZACION_CODPROFORMA_' + Count).replace('@CodProforma', item.CodProforma)
                                        ...
                                        .replace('@ID_OBSERVACION', 'COTIZACION_OBSERVACION_' + Count).replace('@Observacion', item.Observacion)
...
function F_ImprimirDespacho(Codigo) {
        var rptURL = '';
        var Params = 'width=' + (screen.width * 0.48) + ', height=' + (screen.height * 0.40) + ', top=0, left=0, directories=no, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=yes, titlebar=yes';
        var CodMenu = '210';
        ...
        rptURL = rptURL + 'Codigo=' + Codigo + '&';
        rptURL = rptURL + 'NombreArchivo=' + '' + '&';
        rptURL = rptURL + 'NombreTabla=' + 'Electronica' + '&';
...
function F_ImprimirDespachoFila(Fila) {
        var imgID = Fila.id;
        var lblCodigo = '#' + imgID.replace('imgDSP', 'lblCodigo');

        F_ImprimirDespacho($(lblCodigo).val());
