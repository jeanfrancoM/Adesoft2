...
function F_AgregarTemporal() {
    ...
    try {
        ...
        Precio: parseFloat(($('#MainContent_txtPrecioDisplay').val() * (1- (parseFloat($('#MainContent_txtDescuentoProducto').val())/100) )) ).toFixed(2) / tasaigv,
        PrecioDscto: parseFloat(($('#MainContent_txtPrecioDisplay').val() * (1- (parseFloat($('#MainContent_txtDescuentoProducto').val())/100) )) ).toFixed(2) / tasaigv,
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Insertar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
  try 
        ...
        var objParams = {
            ...
            Filtro_FechaSalida: $(Contenedor + 'txtEmision').val()
                            };
...
function F_VerPDF(){
    var CodCotizaciones = $("#MainContent_grvConsulta input:checkbox:checked").not("#MainContent_grvConsulta_ChkAll").map(function(){       
        var fila = this.id;
        var lblCodigo = "#"+fila.replace('chkPdf','lblCodigo');
        return $(lblCodigo).val();
    }).get();

    if (CodCotizaciones.length!=0){
      var json={
        Codigos:CodCotizaciones.toString(),
        CodTipoDoc:15
      }
      $.ajax({
        type:"POST",
