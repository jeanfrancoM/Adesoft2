var AppSession = "../Ventas/RegistroFacturaMultipleKarina.aspx";
...
//            $('#MainContent_chkSinIgvMaestro').prop('disabled',true);
    ...
    {   F_AnularRegistroOC($("#hfimgID").val());
        return true;
    }
    ...
    if ($('#hfCodTipoDoc_grilla').val() == '1' | $('#hfCodTipoDoc_grilla').val() == '2')
    {   
    F_AnularRegistro($("#hfimgID").val());
        return true;
    }
                return false;
            }
...
//            $('#div_ImpresorasNotaPedido').dialog('close');
    ...
    $('#MainContent_txtDireccion').css('background', '#FFFFE0');
      ...
      $('#MainContent_txtobservacionEditar').css('background', '#FFFFE0');
...
function F_Controles_Inicializar() {
    ...
    var arg;
    var Adicional = F_PermisoOpcion(CodigoMenu, 777013, '');
...
//                        P_CodMoneda_Inicial = result.split('~')[25];
//                        P_CantidadPlacas = result.split('~')[15];
                        ...
                        F_Update_Division_HTML('div_SerieCotizacion', result.split('~')[26]);
                        $('#MainContent_ddlSerieCotizacion').css('background', '#FFFFE0');

                        if (Adicional==0)
                            $('#MainContent_chkServicios').prop('disabled',true);     
...
//                    }
           ...
           if (parseFloat(Acuenta)==parseFloat($(Cuerpo + 'txtTotal').val()) & ($("#MainContent_ddlFormaPago").val() != 1 & $("#MainContent_ddlFormaPago").val() != 5 & $("#MainContent_ddlFormaPago").val() != 6))
...
//valida si es deposito o amerita nro Operacion
function F_ES_FormaPago_Deposito() {
    //CODPRINCIPAL 5 DE TCCONCEPTOSDET

    var valor = false;
    var FormaPago = Number($('#MainContent_ddlFormaPago').val());

    switch(FormaPago) {
        case 5:      valor = true; break; //CHEQUE
        case 10:     valor = true; break; //VISA POKET
        case 6:      valor = true; break; //DEPOSITO BCP SOLES
        case 17:     valor = true; break; //DEPOSITO BCP DOLARES
        case 18:     valor = true; break; //DEPOSITO BBVA SOLES
        case 19:     valor = true; break; //DEPOSITO BBVA DOLARES
    }
...
//valida si es deposito o amerita nro Operacion
function F_ES_FormaPago_Contado() {
    //CODPRINCIPAL 5 DE TCCONCEPTOSDET
    ...
    switch(FormaPago) {
        case 1:      valor = true; break; //CONTADO
        case 10:     valor = true; break; //VISA POKET
        case 6:      valor = true; break; //DEPOSITO BCP SOLES
        case 17:     valor = true; break; //DEPOSITO BCP DOLARES
        case 18:     valor = true; break; //DEPOSITO BBVA SOLES
...
//valida si es deposito o amerita nro Operacion
function F_ES_FormaPago_Credito() {
    //CODPRINCIPAL 5 DE TCCONCEPTOSDET
    ...
    switch(FormaPago) {
        case 2:      valor = true; break; //CREDITO
        case 3:      valor = true; break; //CREDITO
        case 4:      valor = true; break; //CREDITO
        case 8:      valor = true; break; //CREDITO
        case 9:      valor = true; break; //CREDITO
        case 11:     valor = true; break; //CREDITO
        case 13:     valor = true; break; //CREDITO
        case 14:     valor = true; break; //CREDITO
        case 15:     valor = true; break; //CREDITO
        case 16:     valor = true; break; //CREDITO
        case 20:     valor = true; break; //CREDITO
        case 21:     valor = true; break; //CREDITO
        case 22:     valor = true; break; //CREDITO
        case 23:     valor = true; break; //CREDITO
        case 24:     valor = true; break; //CREDITO
        case 25:     valor = true; break; //CREDITO
    }
...
var nroDocumentoGenerado = '';
var totalFacturaGenerado = '';
var fechaFacturaGenerado = '';
function F_GrabarDocumento(){
  try 
        ...
        nroDocumentoGenerado = tipoft + ': ' + Serie + '-' + Numero;
        totalFacturaGenerado = $(Contenedor + 'txtTotal').val();
        fechaFacturaGenerado = $(Contenedor + 'txtEmision').val();
...
function F_EditarRegistro(Fila) {
 ...
 try 
    ...
    F_DatosFactura_NET(arg, function (result) {
                    ...
                    if (result.split('~')[21] == "1")
                        $('#MainContent_chkComisionableEdicion').prop('checked',true);
                    else
                        $('#MainContent_chkComisionableEdicion').prop('checked',false);

                    $('#MainContent_txtNroOperacionEdicion').val(result.split('~')[25]);
                    $('#MainContent_txtobservacionEditar').val(result.split('~')[26]);
                    $('#hfCodCtaCteEdicion').val(result.split('~')[27]);
...
function F_GuardarCambiosFactura() {
 try 
    ...
    var objParams = {
                      ...
                      Filtro_Observacion: $('#MainContent_txtobservacionEditar').val()
                    };
...
function F_AgregarTemporalOC() {
    try {
        ...
        $('#MainContent_grvDetalleOC .chkDelete :checkbox').each(function () {
            ...
            if ($(chkSi).is(':checked')) {
            hfobsevacioncliente = chkSi.replace('chkEliminar', 'Hfobsevacioncliente');
                var objDetalle = {
        ...
        var objParams = {
            ...
            Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle),
            Filtro_FlagIgv: 1,
            Filtro_Observacion:$(hfobsevacioncliente).val()
        };
...
function F_EliminarRegistro(Fila) {
 ...
 try 
    ...
    var hfCodTipoDoc = '#' + imgID.replace('imgEliminarDocumento', 'hfCodTipoDoc');

    if ($(hfCodTipoDoc).val() == 1 || $(hfCodTipoDoc).val() == 2)
    {   
        alertify.log("LAS FACTURAS O BOLETAS NO SE PUEDEN ELIMINAR");
        return false;
    }

    if ($(hfCodTipoDoc).val() == 5)
...
function F_AgregarTemporalCTxNumero() {
        ...
        var Serie = $('#MainContent_ddlSerieCotizacion option:selected').text()

    $.ajax({
        async: true,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleKarina.aspx/F_ObtenerCotizacion_PorNumero_Net',
        dataType: "json",
        data: "{'NumeroCotizacion':'" + $(Contenedor + 'txtCodCotizacion').val()  + "','Serie':'" + Serie + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            var data = dbObject.d;
            try {
...
try {
    esPedidoApp = data.CodigoPedidoApp;    
} catch (e) {
        ...
        var tasaigv = parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1);
                    ...
                    var objParams = {
                        ...
                        Filtro_Observacion:data.observacion
                    };
...
//            return false;
        ...
        $.ajax({
            ...
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            url: 'RegistroFacturaMultipleKarina.aspx/F_Listar_Detalle_Proforma_NET',
            dataType: "json",
    ...
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if ($(lblTipoDoc).val() != '5') {
                ...
                alertify.log('SOLO SE PUEDEN MODIFICAR LAS OC O LA NV');
                return false;
                }
            }
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#MainContent_txtObservacion').val(result.split('~')[36]);
                    $('.ccsestilo').css('background', '#FFFFE0');
...
function F_Inicializar_Tabla_Almacenes_Stocks() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleKarina.aspx/F_Inicializar_Tabla_Almacenes_Stocks_NET',
        dataType: "json",
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleKarina.aspx/F_Consulta_Cotizaciones_Pendientes_NET',
        dataType: "json",
...
function F_DetalleMultiple(Control) {
    Ctlgv = Control;
    var Src = $(Control).attr('src');
    ...
    if (Src.indexOf('plus') >= 0) {
        ...
        var grid = $(Control).next();
        F_ConsultarDetalleMultiple(grid.attr('id'));
    }
    else {
        $(Control).attr("src", "../Asset/images/plus.gif");
        $(Control).closest("tr").next().remove();
...
var grvNombre1 = '';
var grvNombre2 = '';
function F_ConsultarDetalleMultiple(Fila){
  try 
        {             
                var nmrow = 'MainContent_grvConsultaArticulo_pnlOrdersKardex_0';
                var Col = Fila.split('_')[3];
                var CodProducto = $('#' + Fila.replace('pnlOrdersKardex', 'lblcodproducto')).val();      
                grvNombre1 = 'MainContent_grvConsultaArticulo_grvDetalleKardex_' + Col;
                grvNombre2 = 'MainContent_grvConsultaArticulo_grvDetalleProducto_' + Col;
                Hfgv = '#' + Fila.replace('pnlOrders', 'hfDetalleCargado');
                ...
                {                                                                                                                                                                                                                        {
                        ...
                        {                          
                            Filtro_CodProducto: CodProducto,
                             Filtro_grvNombre1: grvNombre1,                      
                             Filtro_grvNombre2: grvNombre2,                      
                        };

                        var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);

                        $(Ctlgv).attr('src', '../Asset/images/loading.gif');
                        F_ConsultarDetalleMultiple_NET(arg, function (result) {
                        ...
                        {
try {
                                var p = $('#' + grvNombre1).parent();
                            $(p).attr('id', grvNombre1.replace('MainContent', 'div')); $(p).empty();
...
try {
                                var p = $('#' + grvNombre2).parent();
                            $(p).attr('id', grvNombre2.replace('MainContent', 'div')); $(p).empty();

                            F_Update_Division_HTML($(p).attr('id'), result.split('~')[3]);
...
function F_ActualizarEnApp(){
       
    var actPedido = {};
    actPedido["Factura"] = nroDocumentoGenerado;
    actPedido["Estado"] = "FACTURADO";
    actPedido["FechaFacturacion"] = fechaFacturaGenerado;
    actPedido["TotalFactura"] =  Number(totalFacturaGenerado);
    
    var actualizacion = dbFirestore.collection("Pedidos").doc("20547868839").collection("Pedidos").doc(esPedidoApp);

    // Set the "capital" field of the city 'DC'
    return actualizacion.update(actPedido)
    .then(() => {

    alertify.log("SE ACTUALIZO EL PEDIDO EN LA APP");
    return false;
    ...
    .catch((error) => {
        ...
        MostrarEspera(false);
        alertify.log("Error updating document: ", error);
    });
