var AppSession = "../Ventas/RegistroFacturaMultipleClever.aspx";
...
if (F_PermisoOpcion(CodigoMenu, 4000203, '') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
        try 
        {
                ...
                $('#MainContent_chkServicioMaestro').prop('checked',false);
                $('#MainContent_txtCodigoProductoAgregar').val('');
                $('#MainContent_txtStockAgregar').val('');
                $('#MainContent_txtUMAgregar').val('');
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Eliminar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    ...
    $('#MainContent_btnNuevos').click(function () {
    if (!F_SesionRedireccionar(AppSession)) return false;
     try 
        {
          F_Nuevo();
...
function F_AgregarTemporal() {
    ...
    try {
        ...
        Codigo:      $("#MainContent_txtCodigoProductoAgregar").val().toUpperCase(), 
        CodDetalle: 0,
        Acuenta: 0,
        CodTipoDoc: 0,
        Filtro_FlagIgv: FlagIgv,
            ...
            var Cadena = 'Ingresar los sgtes. Datos:'; 
             ...
             if ($(Cuerpo + 'txtEmision').val()=='' & $("#MainContent_ddlTipoDoc").val() != '5')
                    ...
                    Cadena=Cadena + '<p></p>' + 'CONDICION PAGO'; 
                                 ...
                                 if ($(Cuerpo + 'chkPlaca').is(':checked') && $.trim($(Cuerpo + 'txtPlaca').val())=='')
                    Cadena=Cadena + '<p></p>' + 'Placa';

             if ($(Cuerpo + 'chkKM').is(':checked') && $.trim($(Cuerpo + 'txtKM').val())=='')
                    Cadena=Cadena + '<p></p>' + 'KM';   
                    
                       if ($(Cuerpo + 'chkOC').is(':checked') && $.trim($(Cuerpo + 'txtNroOC').val())=='')
                    Cadena=Cadena + '<p></p>' + 'OC';   
...
//                    Cadena=Cadena + '<p></p>' + 'No ha ingresado ningun producto';
           ...
           if (!F_ValidarCorreo($(Cuerpo + 'txtCorreo').val()))
            ...
            Cadena=Cadena + '<p></p>' + 'LA NOTA DE VENTA NO PUEDE TENER GUIA ELECTRONICA';  
...
function F_GrabarDocumento(){
  try 
        ...
        if ($(Contenedor + 'chkPlaca').prop('checked') === true){
        FlagPlaca=1;
        Placa = $( '#MainContent_txtPlaca').val(); 
        }

         if ($(Contenedor + 'chkKM').prop('checked') === true){
        FlagKM=1;
        Km = $( Contenedor + 'txtKM').val(); 
        }

         if ($(Contenedor + 'chkOC').prop('checked') === true){
        FlagOC=1;
        OC = $( '#MainContent_txtNroOC').val(); 
        }
          
        if ($(Contenedor + 'ddlTipoDoc').val() === '5') {
        ...
        var objParams = {
            ...
            Filtro_ObservacionesCliente: $('#MainContent_txtObservacion').val(),
            ...
            Filtro_FlagPlaca: FlagPlaca, 
            Filtro_FlagKM: FlagKM,
            ...
            Filtro_NombreConductor: $('#MainContent_txtConductorRazonSocial').val(),
               ...
               Filtro_UsuarioPermisoS: UsuarioPermiso,
...
function F_Nuevo(){
       ...
       $('#MainContent_chkKM').prop('checked', false);
       $('#MainContent_chkPlaca').prop('checked', false);
       $('#MainContent_txtNroRuc').val('');
       $('#MainContent_txtNroOperacion').val('');
       $('#MainContent_txtCliente').focus();
       $('#MainContent_txtAtencion').val('');
...
function F_EditarRegistro(Fila) {
 ...
 try 
    ...
    F_DatosFactura_NET(arg, function (result) {
                    ...
                    $('#MainContent_txtObservacionEdicion').val(result.split('~')[28]);
                    F_Update_Division_HTML('div_serieguiaEdicion', result.split('~')[23]);                      
                    $('#MainContent_ddlSerieGuiaEdicion').css('background', '#FFFFE0');  
                    $('#hfCodCtaCteEdicion').val(result.split('~')[29]);
                    $('#MainContent_ddlEmpresaEdicion').val(result.split('~')[24]);

                    $('#MainContent_txtDistritoTransportistaEdicion').val(result.split('~')[31]);
                    $('#MainContent_txtPlacaTrasladoEdicion').val(result.split('~')[32]);
                    $('#MainContent_txtMarcaGuiaEdicion').val(result.split('~')[33]);
                    $('#MainContent_txtLicenciaGuiaEdicion').val(result.split('~')[34]);
                    $('#MainContent_txtNuBultosEdicion').val(result.split('~')[35]);
                    $('#MainContent_txtPesoEdicion').val(result.split('~')[36]);
                    ...
                    $('#hfCodConductor').val(result.split('~')[39]);
                      ...
                      $('#MainContent_txtNombreAgencia').val(result.split('~')[40]);
                      $('#MainContent_txtGuiaAgencia').val(result.split('~')[41]);
                      $('#MainContent_txtClaveAgencia').val(result.split('~')[42]);


                  
                   $('#MainContent_ddlVendedorEdicion').val(result.split('~')[30]);

                    if (result.split('~')[43] == "1")
                    ...
                    if (result.split('~')[26] == "1")
                        $('#MainContent_chkPlacaEdicion').prop('checked',true);
                    else
                        $('#MainContent_chkPlacaEdicion').prop('checked',false);

                    if (result.split('~')[27] == "1")
                        $('#MainContent_chkKMEdicion').prop('checked',true);
                    else
                        $('#MainContent_chkKMEdicion').prop('checked',false);

                    if (result.split('~')[25] == "1")
                        $('#MainContent_chkOCEdicion').prop('checked',true);
                    else
                        $('#MainContent_chkOCEdicion').prop('checked',false);
...
function F_GuardarCambiosFactura() {
 ...
 try 
    ...
    var FlagGuia = 0; if ($('#MainContent_chkGuiaEdicion').is(':checked')) {FlagGuia = 1; };
        ...
        if ($('#MainContent_chkPlacaEdicion').is(':checked'))
            FlagPlaca=1;

        if ($('#MainContent_chkKMEdicion').is(':checked'))
            FlagKM=1;

        if ($('#MainContent_chkOCEdicion').is(':checked'))
            FlagOC=1;
    ...
    var objParams = {
                      ...
                      Filtro_ObservacionCliente: $('#MainContent_txtObservacionEdicion').val(),
...
function F_AgregarTemporalCTxNumero() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleClever.aspx/F_ObtenerCotizacion_PorNumero_Net',
        dataType: "json",
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                        ...
                        if (data.FlagPlaca == 0)                           
                            ...
                            $('#MainContent_chkPlaca').prop('checked', true);  

                        if (data.FlagKM == 0)                           
                            $('#MainContent_chkKM').prop('checked', false);                       
                        else
                            $('#MainContent_chkKM').prop('checked', true);
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            if ($(lblTipoDoc).val() != '5') {
                alertify.log('SOLO SE PUEDEN MODIFICAR LAS OC');
                return false;
            }
...
function F_LlenarGridDetalle(Fila){
  try 
        {             
                ...
                var CodUsuario = $('#hfCodUsuario').val();
