var AppSession = "../Ventas/RegistroFacturaMultipleFundidora.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_txtNroRuc').autocomplete(   
    {
        ...
        $('#hfCodTransportista').val(i.item.val);
            ...
            $('#hfCodCategoria').val(i.item.CodCategoria);
    ...
    $('#MainContent_txtClienteConsulta').autocomplete({
        ...
        $('#MainContent_txtClienteEditar').autocomplete({
        source: function (request, response) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
        ...
        select: function (e, i) {
            ...
            $('#MainContent_txtDestinoEdicion').val(i.item.Direccion + ' ' + i.item.Distrito);
        },
        minLength: 3
    });
    ...
    $('#MainContent_txtDistritoTransportista').autocomplete({
        source: function (request, response) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
        ...
        select: function (e, i) {
            $('#hfCodDepartamentoTransportista').val(i.item.val);
            $('#hfCodProvinciaTransportista').val(i.item.CodProvincia);
            $('#hfCodDistritoTransportista').val(i.item.CodDistrito);
            $('#hfDistritoTransportista').val(i.item.label);
            F_BuscarDireccionesTransportista();
        },
        minLength: 3
    });
    ...
    $("#MainContent_txtPrecioDisplay").bind("propertychange change keyup paste input", function() {
    if($('#hfCodCategoria').val()==1 && ($('#MainContent_ddlPrecio').val()==null || $('#MainContent_ddlPrecio').val()=='0')){
    $("#MainContent_txtDescuento2").val('0');
    $("#MainContent_txtDescuento2").prop("disabled", true);
    
    }else if ($('#hfCodCategoria').val()==2 && ($('#MainContent_ddlPrecio').val()==null || $('#MainContent_ddlPrecio').val()=='0')) {
    $("#MainContent_txtDescuento2").val('0');
    $("#MainContent_txtDescuento2").prop("disabled", true); 
    }else{
        try {
//DESCUENTO
        var PrecioDisplay = Number($("#MainContent_txtPrecioDisplay").val());
        var PrecioMaximoDescuento = Number($("#MainContent_txtPrecioDisplay").val());
        if ($('#hfPrecioOriginal').val()!=''){
        var PrecioOriginal = Number($('#hfPrecioOriginal').val());
        var PorcFinal = 100 - ((PrecioDisplay / PrecioOriginal) * 100);
       // $("#MainContent_txtDescuentoFamiliaMarca").val(PorcFinal.toFixed(2));
        if (PrecioOriginal==0)
           PorcFinal=0;
         $("#MainContent_txtDescuento2").val(PorcFinal.toFixed(2));
         $("#MainContent_txtDescuento2").prop("disabled", false);
        }else{
        $("#MainContent_txtDescuento2").val('');
         $("#MainContent_txtDescuento2").prop("disabled", false);
    ...
    $("#MainContent_txtDescuento2").bind("propertychange change keyup paste input", function() {

    if($("#MainContent_txtDescuento2").val()<100 && $("#MainContent_txtDescuento2").val()>0){
        try {

        var PrecioDisplay = Number($("#MainContent_txtDescuento2").val());
        var PrecioMaximoDescuento = Number($("#MainContent_txtDescuento2").val());
       // $("#MainContent_txtDescuentoFamiliaMarca").val(PrecioDisplay.toFixed(2));
        var PrecioOriginal = Number($('#hfPrecioOriginal').val());

        var PorcFinal=((100-PrecioDisplay)/100)*PrecioOriginal;
         $("#MainContent_txtPrecioDisplay").val(PorcFinal.toFixed(2));

        } catch (e) {
    
        }
        }else{
        alertify.log("Agregar un Descuento Valido");
        }
...
//            if ($('#MainContent_txtDestino').val().trim() == ',') $('#MainContent_txtDestino').val('');
    ...
    $('#MainContent_txtConductorDNI').blur(function () {

        var ConductorEncontrado = false;
        if ($('#hfDniConductor').val() != "0" & $('#hfNombreConductor').val() == "" & $('#MainContent_txtConductorRazonSocial').val() == "" &
                $('#MainContent_txtConductorDNI').val().length > 8 & $('#MainContent_txtConductorDNI').val() == $('#hfDniConductor').val()) 
        {
            ...
            if (Index == -1) { } else {
                ...
                var Datos = $('#MainContent_txtConductorDNI').val().split('-');

                $('#hfDniConductor').val(Datos[0].trim());
                $('#MainContent_txtConductorDNI').val(Datos[0].trim());
                $('#hfNombreConductor').val(Datos[1].trim());
                $('#MainContent_txtConductorRazonSocial').val(Datos[1].trim());
        ...
        if ($('#MainContent_txtConductorDNI').val().trim() != "")
        if (!F_ValidarDniConductor()) {
            $('#hfCodConductor').val('0');
            $('#hfDniConductor').val('');
            $('#MainContent_txtConductorDNI').select;
            $('#hfNombreConductor').val('');
            $('#MainContent_txtConductorRazonSocial').val('');
        }
        return false;
    ...
    {
    alertify.log('Debe Ingresar Cliente Para Agregar Productos');
    return false; 
    }
...
//            $('#div_ImpresorasNotaPedido').dialog('close');
    ...
    if (!F_SesionRedireccionar(AppSession)) return false;
        try {
            ...
            if ($('#MainContent_ddlTipoDocumentoJalar').val() === "15")
                F_AgregarTemporalCTxNumero();
            else
                F_AgregarTemporalCTxNumero_PEDIDO();
    ...
    {   F_AnularRegistroOC('#hfimgID');
        return true;
    }

    if ($('#hfCodTipoDoc_grilla').val() == '16')
    {   F_AnularRegistroNV($('#hfimgID').val());
        return true;
    }
    ...
    $('#MainContent_ddlUM').css('background', '#FFFFE0');
    ...
    $('#MainContent_ddlTipoDocumentoJalar').css('background', '#FFFFE0');

    $('#MainContent_txtNroRucTransportista').css('background', '#FFFFE0');

    $('#MainContent_ddlTipoBusquedaArticulo').css('background', '#FFFFE0');
...
function F_PrecioDisplayUp()
{
  ...
  $('#hfPrecioOriginal').val($("#MainContent_ddlPrecio option:selected").text());
}
...
if (!F_SesionRedireccionar(AppSession)) return false;
    $.ajax({
        ...
        success: function (dbObject) {
            ...
            catch (x) { alertify.log('HA OCURRIDO UN ERROR'); }
            MostrarEspera(false);
        },
        complete: function () {
            if (($('#hfCodDireccion').val() == '' | $('#hfCodDireccion').val() == '0') && $('#hfCodCtaCte').val() != 0)
    ...
    $.ajax({
        ...
        success: function (dbObject) {
            ...
            try {
                ...
                if (data.rows.length > 0) {
                    if ($('#hfCodDireccionDefecto').val() != '0') {
                        $('#MainContent_dllDireccionTransportista').val($('#hfCodDireccionDefecto').val());
                    }
...
$(document).on("click", "select[id $= 'MainContent_btnAgregarProducto']",function () {
    $("#MainContent_txtPrecioDisplay").val($("#MainContent_ddlPrecio option:selected").val());
    $('#hfPrecioOriginal').val($("#MainContent_ddlPrecio option:selected").text());
 });

$(document).on("change", "select[id $= 'MainContent_ddlUM']",function () {
    
    var preciosUnidadSELECCIONADO = "";

    $.each(preciosPresentaciones, function (index, item) {
        if (Number(item.CodProductoPresentacion) === Number($('#MainContent_ddlUM').val())) {
            preciosUnidadSELECCIONADO = item;
        }
    });

    if (preciosUnidadSELECCIONADO != "") {

        $('#hfCodUmAgregar').val(preciosUnidadSELECCIONADO.CodUnidadMedida);

        if (Number(CodUnidOriginal) === preciosUnidadSELECCIONADO.CodUnidadMedida) {
            F_AsignacionPreciosDll(
                Precio1Original, 
                Precio2Original, 
                Precio3Original,
                Precio4Original,
            ...
            F_AsignacionPreciosDll(
                preciosUnidadSELECCIONADO.Precio1, 
                preciosUnidadSELECCIONADO.Precio2, 
                preciosUnidadSELECCIONADO.Precio3,
                preciosUnidadSELECCIONADO.Precio4,
                preciosUnidadSELECCIONADO.Precio5,
                preciosUnidadSELECCIONADO.Precio6
                );
        }     
    }
} );
        ...
        var objParams =
            {
                Filtro_Fecha: $('#MainContent_txtEmision').val(),
                Filtro_CodDoc: $("#MainContent_ddlTipoDoc2").val(),
                Filtro_CodEmpresa: $("#MainContent_ddEmpresaConsulta").val()
            };
...
if (!F_SesionRedireccionar(AppSession)) return false;
    ...
    if ($('#MainContent_txtNroRuc').val()=='9')
    ...
    $('#MainContent_txtNroRuc').val(Cliente.split('-')[0].trim());
        ...
        else
        {
            ...
            {
                //CLIENTE VARIOS
                if (($('#MainContent_txtNroRuc').val() == '1' | $('#MainContent_txtNroRuc').val() == '9' )& $('#MainContent_ddlTipoDoc').val() != '1')
                {
                    if ($('#MainContent_txtNroRuc').val() == '1')
                        $('#MainContent_txtNroRuc').val('11111111');
                    if ($('#MainContent_txtNroRuc').val() == '9')
                        $('#MainContent_txtNroRuc').val('99999999');
                ...
                else
                {
                    ...
                    F_LimpiarCampos();
                    $('#MainContent_txtNroRuc').focus();
                    $('#MainContent_btnAgregarProducto').prop("disabled", true);
                    return true;
                }
            }
        }
            ...
            $.ajax({
                ...
                try {
                    ...
                    $('#hfCodDireccionDefecto').val(data[0].split(',')[14]);
                       ...
                       $('#lblNombreComercial').text(data[0].split(',')[16]);          
             if(data[0].split(',')[15]==1) 
             {
                   $('#lblTipoCliente').text('CALLE');
                   $('#lblTipoClienteDetalle').text('CALLE');
             }                
        ...
        $('#MainContent_txtNroRucTransportista').val(NroRuc);
            ...
            $.ajax({
                ...
                try {
                    ...
                    //alertify.log(x);
                    alertify.log('Por alguna razon el transportista no fue encontrado');
                    $('#td_loadingTransportista').css('display', 'none');
                    ConsultandoPadron = false;
                }
            },
            ...
            $.ajax({
                ...
                if (data.length > 0)
                {
                    try {
                            ...
                            if ($('#MainContent_txtNroRuc').val() === '11111111' | $('#MainContent_txtNroRuc').val() === '99999999')
                                $('#MainContent_txtCliente').prop('readonly', false);                                
                            
                            $('#hfCodCategoria').val(data[0].split(',')[12]);

                            if(data[0].split(',')[12]==1) 
                            {
                                  $('#lblTipoCliente').text('CALLE');
                                  $('#lblTipoClienteDetalle').text('CALLE');
                            }                
                             ...
                             $('#lblNombreComercial').text(data[0].split(',')[13]);
                            F_BuscarDireccionesCliente(); 
                            return true;
                    }
...
//            $('#MainContent_txtCliente').prop('disabled', false);
            ...
            //Valores por Defecto
            $('#hfCodCtaCte').val(4505);
            $('#hfCodDireccion').val(4838);
            $('#MainContent_txtNroRuc').val('11111111');
            $('#MainContent_txtCliente').val('CLIENTE VARIOS');
            $('#hfCliente').val('CLIENTE VARIOS');
            $('#MainContent_txtNombres').val('');
            ...
            $('#MainContent_txtDestino').val('LIMA');
...
function F_FormatosImpresion() {
    utF_FormatoImpresionCombo("#MainContent_ddlFormatoImpresion", $("#MainContent_ddlTipoDoc").val() , $("#MainContent_ddlSerie option:selected").text());
    
}

function F_FormatosImpresion2() {
     var CodTipoDoc =$('#MainContent_ddlTipoDoc').val();
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '../Servicios/Servicios.asmx/F_FormatoImpresion_Listar',
        data: "{'CodTipoDoc':'" + CodTipoDoc + "','SerieDoc':'" + SerieDoc+ "'}", //agregar parametro ms de FlagUsuariosInactivos
        dataType: "json",
        async: true,
        success: function (dbObject) {
            var data = dbObject.d;
            try {
                ...
                $("#MainContent_ddlFormatoImpresion").val(2);
            }
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_VendedorEditar', result.split('~')[29]);
                        F_Update_Division_HTML('div_EmpleadoConsulta', result.split('~')[24]);
                        F_Update_Division_HTML('div_Empresa', result.split('~')[25]);
                        F_Update_Division_HTML('div_EmpresaConsulta', result.split('~')[26]);
                        F_Update_Division_HTML('div_EmpresaEdicion', result.split('~')[27]);
                        $('#hfCodEmpresa').val(result.split('~')[28]);     
                        ...
                        $('#MainContent_dllvendedorEditor').css('background', '#FFFFE0');
                        ...
                        $('#MainContent_txtConductorRazonSocial').prop("disabled", true);
                        $('#MainContent_chkImpresion').prop('checked', false);
                        if (result.split('~')[28]==2)
                        
                        Impresion=false;

                        //Placas
                        if (Number(P_CantidadPlacas) == 0)
                        {
                        ...
                        if (Number(P_CantidadPlacas) < 2)
                            $("#div_Placa2").prop("style").display="none";
                        if (Number(P_CantidadPlacas) < 3)
                            $("#div_Placa3").prop("style").display="none";
                        if (Number(P_CantidadPlacas) < 4)
                        ...
                        $('.ccsestilo').css('background', '#FFFFE0');               
                        F_Mostrar_CorrelativoVarios_Inicio(1);                     
                        F_Inicializar_Tabla_Almacenes_Stocks();
                        F_VerificarUsuarioIniciado_PermisoCambioPrecios();
                       // F_Cotizaciones();                                                
                        F_FormatosImpresion2();                      
                        
                        $("#MainContent_ddlTipoDocumentoJalar").val(23);
                        $("#MainContent_ddlEmpleadoConsulta").val(0);
                    }
                    else {
...
function F_Mostrar_CorrelativoVarios_Inicio(CodDoc) {
    var arg;
...
function F_Buscar_Productos() {
    var arg;
    var CodTipoProducto = $('#MainContent_ddlTipoBusquedaArticulo').val();
    var chkNotaPedido = 0;
    var chkServicio = 0;
    try {
        if ($('#MainContent_chkServicios').is(':checked'))
        ...
        var objParams =
            {
                ...
                Filtro_TipoBusquedaArticulo: $('#MainContent_ddlTipoBusquedaArticulo').val(),
            };
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                       ...
                       // $('#MainContent_grvConsultaArticulo tr:nth-child(odd)').addClass('alt');
//                        $("#MainContent_grvConsultaArticulo tr:even").css("background-color", "#dedede");
//                        $("#MainContent_grvConsultaArticulo tr:odd").css("background-color", "#ffffff");

//                        $("#MainContent_grvConsultaArticulo tbody tr:even").css("background-color", "#F4F4F8");
//                        $("#MainContent_grvConsultaArticulo tbody tr:odd").css("background-color", "#EFF1F1");
//                        $("#MainContent_grvConsultaArticulo tr").css("background-color", function(index) {
//    return index%2==0?"red":"lightblue";
//});
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    var lblCosto_grilla = '';
    var lblCodUm_grilla = '';
    var txtDescuentoFamiliaMarca = '';
    var hfCodCategoriaCliente = '';
    var boolEstado = false;
    ...
    txtDescuentoFamiliaMarca = imgAgregar_grilla.replace('imgAgregar', 'hfDescuento');
    hfCodCategoriaCliente = imgAgregar_grilla.replace('imgAgregar', 'hfCodCategoriaCliente');
    lblCodProducto_grilla = imgAgregar_grilla.replace('imgAgregar', 'lblcodproducto');
    ...
    F_LGProductosPresentaciones_Listar_UM_Ventas($('#MainContent_ddlUM'), $(lblCodProducto_grilla).val());
    $('#MainContent_txtStockAgregar').val($(lblStock_grilla).text());
    $('#MainContent_lblMonedaAgregar').text($(lblMoneda_grilla).val());
    $('#MainContent_txtDescuento2').val('');
    $('#MainContent_txtDescuentoFamiliaMarca').val($(txtDescuentoFamiliaMarca).val());
    $('#hfDescuentoFamiliaMarca').val($(txtDescuentoFamiliaMarca).val());
    $('#hfCodCategoriaCliente').val($(hfCodCategoriaCliente).val());
    $('#MainContent_txtArticuloAgregar').val($(hfArticulo_grilla).text());
    $('#MainContent_txtCantidad').val(1);
    $("#hfMenorPrecioAgregar").val(0);
    $("#MainContent_ddlPrecio").empty();

    Precio1Original = $(hfPrecio1_grilla).val();
    Precio2Original = $(hfPrecio2_grilla).val();
    Precio3Original = $(hfPrecio3_grilla).val()
    Precio4Original = 0;
    Precio5Original = 0;
    Precio6Original = 0;
    CodUnidOriginal = $(lblCodUm_grilla).val();


    F_AsignacionPreciosDll(
        $(hfPrecio1_grilla).val(), 
        $(hfPrecio2_grilla).val(), 
        $(hfPrecio3_grilla).val(),
        '', '', '');
...
var Precio1Original = 0;
var Precio2Original = 0;
var Precio3Original = 0;
var Precio4Original = 0;
var Precio5Original = 0;
var Precio6Original = 0;
var CodUnidOriginal = 0;
function F_AsignacionPreciosDll(Precio1, Precio2, Precio3, Precio4, Precio5, Precio6) {

    $("#MainContent_ddlPrecio").empty();
    var PrecioFinal = "";

    if (Number(Precio1) === 0) Precio1 = '';
    if (Number(Precio2) === 0) Precio2 = '';
    if (Number(Precio3) === 0) Precio3 = '';
    if (Number(Precio4) === 0) Precio4 = '';
    if (Number(Precio5) === 0) Precio5 = '';
    if (Number(Precio6) === 0) Precio6 = '';

    //si es calle va calle
    if (Precio1 != '' && $('#hfCodCategoriaCliente').val() === "1")
    {
        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio1).html(Precio1));
        PrecioFinal = Precio1;
    }


    //si es minorista va minorista, pero si es calle tambien va minorista en el combo 
    //precio calle y precio minorista
    if (Precio2 != '' && $('#hfCodCategoriaCliente').val() === "1") {
        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio2).html(Precio2));

        if (Number(Precio2) < Number($("#hfMenorPrecioAgregar").val()) & Number(Precio2) > 0)
            $("#hfMenorPrecioAgregar").val(Precio2);

        if ($('#hfCodCategoriaCliente').val() === "1")
            PrecioFinal = Precio2;
    }

        if (Precio2 != '' && $('#hfCodCategoriaCliente').val() === "2") {
        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio2).html(Precio2));

        if (Number(Precio2) < Number($("#hfMenorPrecioAgregar").val()) & Number(Precio2) > 0)
            $("#hfMenorPrecioAgregar").val(Precio2);

//        if ($('#hfCodCategoriaCliente').val() === "2")
            PrecioFinal = Precio2;
    }


    if (Precio3 != '' && $('#hfCodCategoriaCliente').val() === "1") {
        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio3).html(Precio3));

        if (Number(Precio3) < Number($("#hfminimoPrecioAgregar").val()) & Number(Precio3) > 0)
            $("#hfminimoPrecioAgregar").val(Precio3);

     if ($('#hfCodCategoriaCliente').val() === "1")
        PrecioFinal = Precio3;
    }

        if (Precio3 != '' && $('#hfCodCategoriaCliente').val() === "2") {
        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio3).html(Precio3));

        if (Number(Precio3) < Number($("#hfminimoPrecioAgregar").val()) & Number(Precio3) > 0)
            $("#hfminimorPrecioAgregar").val(Precio3);
...
//    if (Precio4 != '') {
//        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio4).html(Precio4));

//        if (Number(Precio4) < Number($("#hfMenorPrecioAgregar").val()) & Number(Precio4) > 0)
//            $("#hfMenorPrecioAgregar").val(Precio4);
...
//    if (Precio5 != '') {
//        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio5).html(Precio5));

//        if (Number(Precio5) < Number($("#hfMenorPrecioAgregar").val()) & Number(Precio5) > 0)
//            $("#hfMenorPrecioAgregar").val(Precio5);
...
//    if (Precio6 != '') {
//        $("#MainContent_ddlPrecio").append($("<option></option>").val(Precio6).html(Precio6));

//        if (Number(Precio6) < Number($("#hfMenorPrecioAgregar").val()) & Number(Precio6) > 0)
//            $("#hfMenorPrecioAgregar").val(Precio6);

//        PrecioFinal = Precio6;
//    }

    $('#MainContent_txtPrecioDisplay').val(PrecioFinal);
    $('#hfPrecioOriginal').val(PrecioFinal);

    if(  $('#hfCodCategoria').val()=='2'){
    var PMADF = Number(PrecioFinal); //PrecioMenorAgregarDescuentoFamilia
    

    //SI HAY DESCUENTO POR FAMILIA/MARCA
    if (Number($("#hfDescuentoFamiliaMarca").val()) != 0) {
        var dsc = Number($("#hfDescuentoFamiliaMarca").val());
        PMADF = Number((PMADF-(PMADF*(dsc/100))).toFixed(2));    
    }

    $("#hfMenorPrecioAgregar").val(PMADF);

    $("#MainContent_ddlPrecio").append($("<option></option>").val(PMADF).html(PMADF));
    }
return true;
}
 ...
 try 
        ...
        if (P_VALIDASTOCK == "1") {
            if ((Number($("#MainContent_txtStockAgregar").val()) -  Number($("#MainContent_txtCantidad").val())) < Number(P_VALIDASTOCK_MONTO_MINIMO)) {
                alertify.log("LA CANTIDAD SUPERA AL MONTO DE VALIDACION MINIMO");
                return false;
            }
        }
    ...
    try {
        ...
        if ($('#MainContent_chkConIgvMaestro').is(':checked')) {
            ...
            FlagIgv = 1;
              ...
              if( $('#hfPrecioOriginal').val()=='0' ||  $('#hfPrecioOriginal').val()=='')
               $('#hfPrecioOriginal').val($('#MainContent_txtPrecioDisplay').val() ) ;
        ...
        Precio: $('#hfPrecioOriginal').val() / tasaigv,
        PrecioDscto: $('#MainContent_txtPrecioDisplay').val() / tasaigv,
        DescuentoPorcMax: $('#hfDescuentoFamiliaMarca').val(),
        ...
        Filtro_TasaIgv: tasaigv,
        Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
        CodProductoPresentacion: $('#MainContent_ddlUM').val()  
        };
        arrDetalle.push(objDetalle);
...
function F_ValidarGrabarDocumento(){
    try 
        {        
            ...
            var Cadena = 'Ingresar los sgtes. Datos:'; 
             ...
             if ($(Cuerpo + 'chkGuia').is(':checked')) {
                ...
                if ($(Cuerpo + 'txtPeso').val() == '' && $(Cuerpo + 'ddlSerieGuia option:selected').text().substr(0, 1) == "T")
                        Cadena = Cadena + '<p></p>' + 'Peso en KG.';   
                        

                 var SerieGuia = $('#MainContent_ddlSerieGuia option:selected').text().substring(0, 1);
                
                //si es electronica
                //-----------------
                if (($(Cuerpo + 'chkGuia').is(':checked')) && ($(Cuerpo + 'txtNuBultos').val() === "") && SerieGuia === "T")
                    Cadena = Cadena + '<p></p>' + 'Nro. Bultos';                   

                if ($('#MainContent_txtConductorDNI').val() != ""  && SerieGuia === "T") {
                    if (!F_ValidarDniConductor()) {
                        toastr.warning('SI VA A ESPECIFICAR CONDUCTOR, SU DNI Y NOMBRE DEBEN SER CORRECTOS');
                        Cadena = Cadena + '<p></p>' + 'DNI';
                        return false;
                    } else {
                        if ($('#MainContent_txtConductorRazonSocial').val().length <= 3) {
                        toastr.warning('SI VA A ESPECIFICAR CONDUCTOR, SU DNI Y NOMBRE DEBEN SER CORRECTOS');
                        Cadena = Cadena + '<p></p>' + 'NOMBRE CONDUCTOR';                
                        }
                    }
                }
        ...
        if ($('#MainContent_txtConductorDNI').val() != "" ) {
            if (!F_ValidarDniConductor()) {
                alertify.log('SI VA A ESPECIFICAR CONDUCTOR, SU DNI Y NOMBRE DEBEN SER CORRECTOS');
                Cadena = Cadena + '<p></p>' + 'DNI';
                return false;
            } else {
                if ($('#MainContent_txtConductorRazonSocial').val().length <= 3) {
...
function F_GrabarDocumento(){
  try 
        ...
        var objParams = {
            ...
            Filtro_DniConductor: $('#MainContent_txtConductorDNI').val(),
            Filtro_NombreConductor: $('#MainContent_txtConductorRazonSocial').val(),
            ...
            Filtro_DepartamentoTransportista: "LIMA",
            Filtro_ProvinciaTransportista: "LIMA",
            Filtro_DistritoTransportista: "LIMA",
...
function F_Nuevo(){
       ...
       $('#MainContent_chkImpresionTicket').prop('checked', true);
        $('#MainContent_chkImpresion').prop('checked', Impresion);
       ...
       $('#MainContent_chkNotaVentaANTIGUA').prop('checked', false);
       $('#lblTipoCliente').text('');
       $('#lblTipoClienteDetalle').text('');
       $("#lblNombreComercial").text('');
       $("#lblTipoCliente").text('');
...
//        }
    ...
    if(!confirm("ESTA SEGURO DE ANULAR LA "  + $("#MainContent_ddlTipoDoc2 option:selected").text() + " : " +  $('#hflblNumero').val() + "\n" + "DEL CLIENTE : " +  $('#hfClienteAnulacion').val()))
    return false;
...
function F_EditarRegistro(Fila) {
 ...
 try 
    ...
    var hfCodTipoDoc = '#' + imgID.replace('imgEditarDocumento', 'hfCodTipoDoc');
    var hfCodEmpresa = '#' + imgID.replace('imgEditarDocumento', 'hfCodEmpresa');
    ...
    F_DatosFactura_NET(arg, function (result) {
                    ...
                    F_Update_Division_HTML('div_serieguiaEdicion', result.split('~')[23]);                      
                    $('#MainContent_ddlSerieGuiaEdicion').css('background', '#FFFFE0');  
                    //$("#MainContent_ddlSerieGuiaEdicion option:selected").text(result.split('~')[10]);
                    $('#MainContent_ddlEmpresaEdicion').val(result.split('~')[24]);
                    var vendedor =result.split('~')[25];
                    $('#hfCodCtaCteEdicion').val(result.split('~')[26]);
                    $('#MainContent_txtObservacionEdicion').val(result.split('~')[27]);
                    $('#MainContent_dllvendedorEditor').val(vendedor);
                    ...
                    if ($(hfCodTipoDoc).val() == "16")
                        $('#MainContent_txtClienteEditar').prop('readonly',false);
                    else
                        $('#MainContent_txtClienteEditar').prop('readonly',true);
...
function F_GuardarCambiosFactura() {
 try 
    ...
    var FlagGuia = 0; if ($('#MainContent_chkGuiaEdicion').is(':checked')) {FlagGuia = 1; };
     var vendedor=$('#MainContent_dllvendedorEditor').val()  ;
    ...
    var objParams = {
                      ...
                      Filtro_ObservacionesClientes: $('#MainContent_txtObservacionEdicion').val()
                    };
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var hfAcuenta = txtPrecio.replace('txtPrecio', 'hfAcuenta');
        var hfCodTipoDoc = txtPrecio.replace('txtPrecio', 'hfCodTipoDoc');
        var FlagIgv = 0;
        FilaActualizarValidaPermiso = "#" + Fila;
        PrecioActualizarValidarPermiso = $(hfPrecio).val();
        var hfPrecioOriginal = txtPrecio.replace('txtPrecio', 'hfPrecioOriginal');
        var hfDescuentoPorcMax = txtPrecio.replace('txtPrecio', 'hfDescuentoPorcMax');

       if (parseInt($(hfCodTipoDoc).val())==16 & parseFloat($(txtPrecio).val())!=parseFloat($(hfPrecio).val()) )
            {
             $(txtPrecio).val(parseFloat($(hfPrecio).val()).toFixed(2));
             alertify.log("EL PRECIO NO PUEDE SER DIFERENTE AL REGISTRADO EN LA NV");              
            return false;
            }

        if (parseInt($(hfCodDetalleOC).val())> 0 & parseFloat($(txtPrecio).val())!=parseFloat($(hfPrecio).val()) )
        {
         $(txtPrecio).val(parseFloat($(hfPrecio).val()).toFixed(2));
         alertify.log("EL PRECIO NO PUEDE SER DIFERENTE AL REGISTRADO EN LA OP");              
        return false;
        }

        var PMADF = Number($(hfPrecioOriginal).val()); //PrecioMenorAgregarDescuentoFamilia


        //SI HAY DESCUENTO POR FAMILIA/MARCA
        if (Number($(hfPrecioOriginal).val()) > 0) {
            var dsc = Number($(hfDescuentoPorcMax).val());
            PMADF = Number((PMADF-(PMADF*(dsc/100))).toFixed(2));    
        }
        

        if (UsuarioIniciado_PermisoCambioPrecios === '0') {
            if (Number($(txtPrecio).val()) < PMADF & PMADF > 0 & PermisoCambio == false) 
                            //& F_VerificarTipoCliente() == false)
            {
                $("#MainContent_txtUsuarioPrecio").val('');
                $("#MainContent_txtContraseñaPrecio").val('');
        ...
        var objParams = {
           ...
           // Filtro_PrecioOriginal: Number($(hfPrecioOriginal).val()) / tasaigv,
            Filtro_PrecioOriginal: $(txtPrecio).val() / tasaigv,
            Filtro_DescuentoPorcMax: $(hfDescuentoPorcMax).val(),
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var hfCodDetalleOC = txtCantidad.replace('txtCantidad', 'hfCodDetalleOC');
         var hfCodTipoDoc = txtPrecio.replace('txtCantidad', 'hfCodTipoDoc');
        var hfAcuenta = txtCantidad.replace('txtCantidad', 'hfAcuenta');
        var Flag = 0;
        var FlagIgv = 0;

        
       if (parseInt($(hfCodTipoDoc).val())==16 & parseFloat($(txtCantidad).val())!=parseFloat($(hfCantidad).val()) )
            {
        ...
        if (parseInt($(hfCodDetalleOC).val())> 0 & parseFloat($(txtCantidad).val())!=parseFloat($(hfCantidad).val()) )
        {
         $(txtCantidad).val(parseFloat($(hfCantidad).val()).toFixed(2));
         alertify.log("EL PRECIO NO PUEDE SER DIFERENTE AL REGISTRADO EN LA OP");              
        return false;
        ...
        $('#MainContent_grvDetalleNV .chkDelete :checkbox').each(function () {
            ...
            if ($(chkSi).is(':checked')) {
                ...
                var objDetalle = {
                    ...
                    Acuenta: parseFloat(acuenta)  / parseFloat( $(lblCantidad).text()) * parseFloat( $(txtcantidad_grilla).val()),
                    CodTipoDoc: 16,
                    Fecha: $(hfFechaAnexo).val()
                };
                arrDetalle.push(objDetalle);
...
function F_AgregarTemporalCTxNumero() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleFundidora.aspx/F_ObtenerCotizacion_PorNumero_Net',
        dataType: "json",
                  ...
                  $('#MainContent_chkComisionable').prop('checked', false);   
                    ...
                    $.each(data.lProformaDet, function (index, item) {
                        var objDetalle = {
                            ...
                            Fecha: '',
                            DescuentoPorcMax:0,
                            CodProductoPresentacion: item.CodProductoPresentacion
                        };
                        arrDetalle.push(objDetalle);
...
function F_AgregarTemporalCTxBoton_PEDIDO(Boton) {
    $('#MainContent_txtCodCotizacion').val($('#PEDIDO_NUMERO_' + Boton.split('_')[2]).val());
    F_Nuevo();
    F_AgregarTemporalCTxNumero_PEDIDO();
}

function F_AgregarTemporalCTxNumero_PEDIDO() {
        var Contenedor = '#MainContent_';
        var Mensaje = "Ingrese los sgtes datos: ";
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultipleFundidora.aspx/F_ObtenerOrdenPedido_PorNumero_Net',
        dataType: "json",
        data: "{'NumeroOrdenPedido':'" + $(Contenedor + 'txtCodCotizacion').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            var data = dbObject.d;
            try {
        ...
        if (data.lOrdenPedidoDet == null)
        {
        alertify.log('EL PEDIDO AUN NO HA SIDO APROBADO');
                   return false;
        }
                    //Array de Items
                    $.each(data.lOrdenPedidoDet, function (index, item) {
                        var objDetalle = {
                            CodArticulo: item.CodArticulo,
                            Cantidad: item.Cantidad,
                            Precio: item.Precio / tasaigv,
                            ...
                            CostoUnitario: 0,
                            Dscto: 0,
                            CodDetalle: item.CodDetalleOrdenPedido,
                            OC: '',
                            Descripcion: item.Descripcion,
                            Acuenta: 0,
                            CodTipoDoc: 23,
                            Fecha: '',
                            CodProductoPresentacion: item.CodProductoPresentacion,
                            DescuentoPorcMax: item.DescuentoPorcMax,
                            PEstimado: item.PEstimado,
                            PxKilo: item.PxKilo,
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                        ...
                        $("#MainContent_ddlTipoDoc").val(data.CodTipoDoc);
...
function F_AgregarTemporalCTxID(CodProforma, NroLista) {
    ...
    try {
        ...
        var nro = $("#PEDIDO_SERIE_" + NroLista).val() + '-' + $("#PEDIDO_NUMERO_" + NroLista).val();
        ...
        $.ajax({
            ...
            success: function (dbObject) {
                ...
                try {
                    ...
                    cotFlagIgv = $("#PEDIDO_FLAGIGV_" + NroLista).val();
                    var objParams = {
                        Filtro_CodTipoDoc: 15,
                        Filtro_SerieDoc: $("#PEDIDO_SERIE_" + NroLista).val(),
                        Filtro_NumeroDoc: $("#PEDIDO_NUMERO_" + NroLista).val(),
                        Filtro_FechaEmision: $("#PEDIDO_EMISION_" + NroLista).val(),
                        Filtro_Vencimiento: $("#PEDIDO_VENCIMIENTO_" + NroLista).val(),
                        Filtro_CodCliente: $("#PEDIDO_CODCTACTE_" + NroLista).val(),
                        Filtro_CodFormaPago: $("#PEDIDO_CODFORMAPAGO_" + NroLista).val(),
                        Filtro_CodMoneda: $("#PEDIDO_CODMONEDA_" + NroLista).val(),
                        Filtro_TipoCambio: $("#PEDIDO_TC_" + NroLista).val(),
                        Filtro_SubTotal: $("#PEDIDO_SUBTOTAL_" + NroLista).val(),
                        Filtro_CodProforma: "0",
                        Filtro_Igv: $("#PEDIDO_IGV_" + NroLista).val(),
                        Filtro_Total: $("#PEDIDO_TOTAL_" + NroLista).val(),
                        Filtro_CodGuia: "0",
                        Filtro_Descuento: "0",
                        Filtro_TasaIgv: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
                        Filtro_TasaIgvDscto: tasaigv,
                        Filtro_CodigoTemporal: $('#hfCodigoTemporal').val(),
                        Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle),
                        Filtro_FlagIgv: $("#PEDIDO_FLAGIGV_" + NroLista).val()
                    };
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {
                        ...
                        $('#MainContent_txtNroRuc').val($("#PEDIDO_NRORUC_" + NroLista).val());
                        $('#hfNroRuc').val($("#PEDIDO_NRORUC_" + NroLista).val());
                        $('#MainContent_txtCliente').val($("#PEDIDO_CLIENTE_" + NroLista).val());
                        $('#hfCliente').val($("#PEDIDO_CLIENTE_" + NroLista).val());

                        $('#hfCodCtaCte').val($("#PEDIDO_CODCTACTE_" + NroLista).val());
                        $('#hfCodDepartamento').val($("#PEDIDO_CODDEPARTAMENTO_" + NroLista).val());
                        $('#hfCodProvincia').val($("#PEDIDO_CODPROVINCIA_" + NroLista).val());
                        $('#hfCodDistrito').val($("#PEDIDO_CODDISTRITO_" + NroLista).val());
                        $('#MainContent_txtDistrito').val($("#PEDIDO_DISTRITO_" + NroLista).val());
                        $('#MainContent_txtDireccion').val($("#PEDIDO_DIRECCION_" + NroLista).val());
                        $('#MainContent_txtNroOperacion').val($("#PEDIDO_NROOPERACION_" + NroLista).val());
                        $('#MainContent_ddlVendedor').val($("#PEDIDO_CODEMPLEADO_" + NroLista).val());
                        $('#MainContent_txtPlaca').val($("#PEDIDO_PLACA_" + NroLista).val());
                        $('#MainContent_ddlKM').val($("#PEDIDO_KM_" + NroLista).val());
...
function F_ReemplazarDocumento(Fila) {
    if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            var lblEstado = '#' + imgID.replace('imgReemplazar', 'hfEstado');
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#hfCodDireccion').val(result.split('~')[38]);
                    $('#hfCodTransportista').val(result.split('~')[19]);
                    $('#MainContent_txtTransportista').val(result.split('~')[20]);
                    $('#hfCodAlmacen').val(result.split('~')[36]);
                    $('#hfCodCategoria').val(result.split('~')[37]);
                    
                    $('#MainContent_txtObservacion').val(result.split('~')[39]);

                    if((result.split('~')[37])==1) 
             {
                   $('#lblTipoCliente').text('CALLE');
                   $('#lblTipoClienteDetalle').text('CALLE');
             }                
...
function F_Cotizaciones() {
    CotizacionesCargados = false;
    $.ajax({
        async: true,
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultiplePovis.aspx/F_Consulta_Cotizaciones_Pendientes_NET',
        dataType: "json",
...
function F_Pedidos() {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: 'RegistroFacturaMultiplePovis.aspx/F_Consulta_Pedidos_Pendientes_NET',
        dataType: "json",
        data: "{'CodAlmacen':'" + $('#hfCodSede').val()  + "'}",
        success: function (dbObject) {
            MostrarEspera(true);
            ...
            try {
                ...
                $.each(data.rows, function (index, item) {
                    var fmt2 =   ' <li style="width: 220px; height:30px; padding-top:0px; display:block"> <div class="linea-button"> ' +
								...
								'  		<td style="width: 25"> ' +
                                '             <button type="button" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" Font-Names="Arial" Font-Bold="True" Width="25"  height:"25" ' + 
                                '                   id="@ID_BTNAGREGARCOTIZACION" onclick="F_AgregarTemporalCTxBoton_PEDIDO(this.id); return false;"> ' +
                                '             <img src="../Asset/images/ok.gif"> </button>  ' +
                                '       </td> ' +
                                '     </tr> ' +
                                '     <table> ' +
                    ...
                    var li = fmt2.replace('@Nro', Count).replace('@ID', 'PEDIDO_CODPROFORMA_' + Count).replace('@CodProforma', item.CodOrdenPedido)
                                        .replace('@NroDocumento', Number(item.Numero)).replace("@ElVendedor", item.Vendedor).replace('@Cliente', item.Cliente)
                                        .replace('@ID_SERIE', 'PEDIDO_SERIE_' + Count).replace('@Serie', item.Serie)
                                        .replace('@ID_NUMERO', 'PEDIDO_NUMERO_' + Count).replace('@Numero', item.Numero)
                                        .replace('@ID_EMISION', 'PEDIDO_EMISION_' + Count).replace('@Emision', item.EmisionStr)
                                        .replace('@ID_VENCIMIENTO', 'PEDIDO_VENCIMIENTO_' + Count).replace('@Vencimiento', item.VencimientoStr)
                                        .replace('@ID_CODCTACTE', 'PEDIDO_CODCTACTE_' + Count).replace('@CodCtaCte', item.CodCtaCte)
                                        .replace('@ID_NRORUC', 'PEDIDO_NRORUC_' + Count).replace('@NroRuc', item.NroRuc)
                                        .replace('@ID_CLIENTE', 'PEDIDO_CLIENTE_' + Count).replace('@Cliente', item.CodCtaCte)
                                        .replace('@ID_CODDEPARTAMENTO', 'PEDIDO_CODDEPARTAMENTO_' + Count).replace('@CodDepartamento', item.CodDepartamento)
                                        .replace('@ID_CODPROVINCIA', 'PEDIDO_CODPROVINCIA_' + Count).replace('@CodProvincia', item.CodProvincia)
                                        .replace('@ID_CODDISTRITO', 'PEDIDO_CODDISTRITO_' + Count).replace('@CodDistrito', item.CodDistrito)
                                        .replace('@ID_DISTRITO', 'PEDIDO_DISTRITO_' + Count).replace('@Distrito', item.Distrito)
                                        .replace('@ID_DIRECCION', 'PEDIDO_DIRECCION_' + Count).replace('@Direccion', item.Direccion)
                                        .replace('@ID_CODFORMAPAGO', 'PEDIDO_CODFORMAPAGO_' + Count).replace('@CodFormaPago', item.CodFormaPago)
                                        .replace('@ID_CODMONEDA', 'PEDIDO_CODMONEDA_' + Count).replace('@CodMoneda', item.CodMoneda)
                                        .replace('@ID_TC', 'PEDIDO_TC_' + Count).replace('@TC', item.TipoCambio)
                                        .replace('@ID_SUBTOTAL', 'PEDIDO_SUBTOTAL_' + Count).replace('@Subtotal', item.SubTotal)
                                        .replace('@ID_IGV', 'PEDIDO_IGV_' + Count).replace('@Igv', item.Igv)
                                        .replace('@ID_TOTAL', 'PEDIDO_TOTAL_' + Count).replace('@Total', item.Total)
                                        .replace('@ID_BTNAGREGARCOTIZACION', 'BTN_PEDIDO_' + Count).replace('@Total', item.Total)
										.replace('@ID_FLAGIGV', 'PEDIDO_FLAGIGV_' + Count).replace('@FlagIgv', item.FlagIgv)
										.replace('@ID_NROOPERACION', 'PEDIDO_NROOPERACION_' + Count).replace('@NroOperacion', item.NroOperacion)
										.replace('@ID_CODEMPLEADO', 'PEDIDO_CODEMPLEADO_' + Count).replace('@CodEmpleado', item.CodEmpleado)
										.replace('@ID_PLACA', 'PEDIDO_PLACA_' + Count).replace('@Placa', item.Placa)
										.replace('@ID_KM', 'PEDIDO_KM_' + Count).replace('@KM', item.KM)
        ...
        F_Series_Guia_NET
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        F_Update_Division_HTML('div_serieguia', result.split('~')[2]);                 
                        $('#MainContent_ddlSerieGuia').css('background', '#FFFFE0');  
                    }
                    else {
...
function F_ValidarDniConductor() {
    var retorno = true;
    ...
    if ($('#MainContent_txtConductorDNI').val().length > 8) {
        ...
        if ( Index ==-1 ) {} else {
            ...
            $('#MainContent_txtConductorRazonSocial').val(Cliente.split('-')[1].trim());
        }
    }


        if ($('#MainContent_txtConductorDNI').val().length != 8 && $('#MainContent_txtConductorDNI').val().length > 1) {
            alertify.log("AL ESPECIFICAR UN CONDUCTOR SU RUC DEBE SER CORRECTO");
            $('#MainContent_txtConductorRazonSocial').val('');
            $('#MainContent_txtConductorDNI').select();
            retorno = false;
            return false;
        } else {
            if (isNaN($('#MainContent_txtConductorDNI').val())) {
                alertify.log("AL ESPECIFICAR UN CONDUCTOR SU RUC DEBE SER CORRECTO");
                $('#MainContent_txtConductorRazonSocial').val('');
                $('#MainContent_txtConductorDNI').select();
                retorno = false;
            ...
            if (!ValidarRuc($('#MainContent_txtConductorDNI').val())) {
                alertify.log("AL ESPECIFICAR UN CONDUCTOR SU RUC DEBE SER CORRECTO");
                $('#MainContent_txtConductorRazonSocial').val('');
                $('#MainContent_txtConductorDNI').select();
                retorno = false;
...
//Presentaciones
var combito;
var preciosPresentaciones;
var preciosUnidadSELECCIONADO;
function F_LGProductosPresentaciones_Listar_UM_Ventas(Combo, CodProducto) {
    ...
    $.ajax({
        type: "POST",
        url: '../Servicios/DatosLGProductosPresentaciones.asmx/F_LGProductosPresentaciones_Listar_UM_Ventas?time=' + timex,
        data: "{'CodProducto':'" + CodProducto + "','CodMoneda':'" + $('#MainContent_ddlMoneda').val() + "', 'TC':'" + $('#MainContent_lblTC').text() + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        success: function (obj, status) {

        preciosPresentaciones = obj.d;

            $(combito).empty();
            $.each(obj.d, function (index, item) {
                $(combito).append($("<option></option>").val(item.CodProductoPresentacion).html(item.UnidadMedida + " (" + item.Factor + ")"));
            });
...
var CtlgvLinea;
var HfgvLinea;
function F_MarcasProductos(Control) {
    CtlgvLinea = Control;
    var Src = $(Control).attr('src');
    ...
    if (Src.indexOf('plus') >= 0) {
        ...
        var grid = $(Control).next();
        F_LlenarGridDetalleLinea(grid.attr('id'));
    }
    else {
        $(Control).attr("src", "../Asset/images/plus.gif");
        $(Control).closest("tr").next().remove();
...
function F_LlenarGridDetalleLinea(Fila) {
    try {
        var nmrow = 'MainContent_grvConsulta_pnlOrdersMarcaProducto_0';
        var Col = Fila.split('_')[3];
        var Codigo = $('#' + Fila.replace('pnlOrdersMarcaProducto', 'lblcodproducto')).val();
        var grvNombre = 'MainContent_grvConsultaArticulo_grvDetalleMarcaProducto_' + Col;
        HfgvLinea = '#' + Fila.replace('pnlOrdersMarcaProducto', 'hfDetalleCargado');

            {
                ...
                MostrarEspera(true);
                $(CtlgvLinea).attr('src', '../Asset/images/loading.gif');
                F_LlenarGridDetalle_MarcaProducto_NET(arg, function (result) {

                    $(CtlgvLinea).attr('src', '../Asset/images/minus.gif');
                    MostrarEspera(false);
                    ...
                    if (str_resultado_operacion === "0") {
                        ...
                        $(CtlgvLinea).closest('tr').after('<tr><td></td><td colspan = "999">' + $(CtlgvLinea).next().html() + '</td></tr>');
                        $(HfgvLinea).val('1');
                    }
                    else {
                        alertify.log(str_mensaje_operacion);
...
//agregado para limpiar la grilla al cambiar minorista o calle
function F_NuevoCLiente(){
...
function F_ActualizarDescuento(Fila) {
    var imgID = Fila; //MainContent_txtPrecioDisplay 
    var descuento = Fila.replace('imgAgregar', 'hfDescuento');
    if($('#hfCodCategoria').val()=='1'){
    if($('#descuento').val()<$('#MainContent_txtDescuentoFamiliaMarca').val()){
    alertify.log('El descuento no puede ser más de'+$('#descuento').val());
    }else{
    $('#MainContent_txtDescuentoFamiliaMarca').val($('#descuento').val());
    }
    }
