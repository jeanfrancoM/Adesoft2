var AppSession = "../Ventas/RegistroCotizacionesFundidora.aspx";
...
//                        $('#MainContent_ddlVendedor').val(result.split('~')[13]);
                        ...
                        $("#MainContent_txtPrecioDisplay").prop('disabled', true);
...
function F_Buscar_Productos() {
    ...
    var CodCategoria = 1;
    try {
        if ($('#MainContent_chkMateriaPrima').is(':checked'))
            CodCategoria = 2;
...
function F_AgregarArticulo(ControlID, DirectoBoton) {
    ...
    $('#MainContent_txtCantidad').val(1);
    $("#MainContent_txtPEstimado").val(0);
    $("#MainContent_txtPxKilo").val(0);
    $("#MainContent_txtMaterial").val('');
...
//    if($('#MainContent_chkMateriaPrima').is(":checked")){
//            $("#MainContent_txtPrecioDisplay").prop("disabled",false);
//             $("#MainContent_txtPEstimado").prop("disabled",true);
//              $("#MainContent_txtPxKilo").prop("disabled",true);
//        }else{
//                $("#MainContent_txtPrecioDisplay").prop("disabled",true);
//             $("#MainContent_txtPEstimado").prop("disabled",false);
//              $("#MainContent_txtPxKilo").prop("disabled",false);
//        }
 ...
 try 
        ...
        if ($("#MainContent_txtPrecioDisplay").val() == '')
            cadena = cadena + "CAMPO PRECIO ARTICULO <p></p> ";
             ...
             if ($("#MainContent_txtMaterial").val() == '' && parseFloat($("#MainContent_txtPEstimado").val()) > 0 &&  parseFloat($("#MainContent_txtPxKilo").val()) > 0)
            cadena = cadena + "MATERIAL <p></p> ";
...
function F_AgregarTemporal() {
    ...
    try {
       ...
       Precio2=0;
        ...
        Cantidad173: 0,            
        Cantidad250: 0,
        Pestimado:$('#MainContent_txtPEstimado').val(),
        PxKilo:$('#MainContent_txtPxKilo').val(),
        Material:$('#MainContent_txtMaterial').val()
        };
        arrDetalle.push(objDetalle);
        ...
        AgregandoProducto = false;
            ...
            if (str_resultado_operacion == "1") {
               ...
               // $("#MainContent_lblNumRegistros").text(F_Numerar_Grilla("grvDetallePedido", "lblCodigoProducto")); //Numerando grilla de pedidos
                ...
                $('#MainContent_txtCantidad173').val('');
                $('#MainContent_txtCantidad250').val('');
                $('#MainContent_txtPEstimado').val('');
                $('#MainContent_txtPxKilo').val('');
                $('#MainContent_txtMaterial').val('');
                $("#hfMenorPrecioAgregar").val(0);
                $('#lblCantidadRegistro').text(F_Numerar_Grilla("grvDetalleArticulo", "lblimporte"));
                F_TablaDown(); 
                F_ValidarPrecioMinimoRojo();
                  ...
                  $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
                     chkDel = '#' + this.id;
                     txtPestimado = chkDel.replace('chkEliminar', 'txtPestimado');
                     txtPxKilo = chkDel.replace('chkEliminar', 'txtPxKilo');
                     txtPrecio = chkDel.replace('chkEliminar', 'txtPrecio');
                                                 
                         if ($(txtPestimado).val()==0 && $(txtPxKilo).val()==0 ){
                         $(txtPestimado).prop('disabled',true);
                         $(txtPxKilo).prop('disabled',true);
                         }

                            if (parseFloat($(txtPestimado).val())>0 && parseFloat($(txtPxKilo).val())>0 ){
...
function F_ActualizarPrecio(Fila) {
    try {
        ...
        var txtMaterial = txtPrecio.replace('txtPrecio', 'txtMaterial');
        var FlagIgv = 0;
...
//        }
        ...
        var objParams = {
            ...
            Filtro_Flag: 0,
            Filtro_TasaIgvDscto: parseFloat($("#MainContent_ddlIgv option:selected").text()) + parseFloat(1),
            Filtro_Material: $(txtMaterial).val()   
        };
...
function F_ActualizarCantidad(Fila) {
    try {
        ...
        var txtMaterial = txtCantidad.replace('txtCantidad', 'txtMaterial');
        var Flag = 0;
        var FlagIgv = 0;
...
function F_InicializarCajaTexto()
{
    ...
    $("#MainContent_txtMaterial").css('background', '#FFFFE0');

    $("#MainContent_txtPEstimado").css('background', '#FFFFE0');

    $("#MainContent_txtPxKilo").css('background', '#FFFFE0');
 ...
 $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
        ...
        if (parseFloat($(hfExclusivo).val())>0)
            PrecioLimite = $(hfExclusivo).val();
               ...
               if ($(hfCodDetalleOC).val()==0)
               {
                     //if (parseFloat($(txtPrecio).val()) < parseFloat($(hfP1).val()) & parseFloat($(hfP1).val()) > 0 & C==0)
...
function F_PrecioUnitario(Fila) {
//if (!F_SesionRedireccionar(AppSession)) return false;
//    if (F_PermisoOpcion(CodigoMenu, 4000502, '') === "0")  return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    try {
        
        var Pestimado = '#MainContent_txtPEstimado';
        var PxKilo = '#MainContent_txtPxKilo';
        var Pdisplay = '#MainContent_txtPrecioDisplay';
        
         $("#MainContent_txtPrecioDisplay").prop('disabled', true);

       if($(Pestimado).val() ==0 && $(PxKilo).val() ==0){
            $("#MainContent_txtPrecioDisplay").prop('disabled', false);
            $(Pdisplay).val('0');
       }else {
            $(Pdisplay).val(parseFloat( Number($(Pestimado).val())*Number($(PxKilo).val())).toFixed(2));
       }
    }
    catch (e) {
        MostrarEspera(false);
...
function F_ActualizarMaterial(Fila) {
    try {
        var txtMaterial = '#' + Fila;
        var hfCodDetalle = txtMaterial.replace('txtMaterial', 'hfCodDetalle');
        var hfPrecio = txtMaterial.replace('txtMaterial', 'hfPrecio');
        var hfCantidad = txtMaterial.replace('txtMaterial', 'hfCantidad');
        var txtCantidad = txtMaterial.replace('txtMaterial', 'txtCantidad');
        var txtPrecio = txtMaterial.replace('txtMaterial', 'txtPrecio');
        var txtDescripcion = txtMaterial.replace('txtMaterial', 'txtDescripcion');
        var hfCodDetalleOC = txtMaterial.replace('txtMaterial', 'hfCodDetalleOC');
        var lblAcuenta = txtMaterial.replace('txtMaterial', 'lblAcuenta');
        var txtPestimado = txtMaterial.replace('txtMaterial', 'txtPestimado');
        var txtPxKilo = txtMaterial.replace('txtMaterial', 'txtPxKilo');
        var hfMaterial = txtMaterial.replace('txtMaterial', 'hfMaterial');
        var Flag = 0;
        var FlagIgv = 0;

        if (parseFloat($(txtPestimado).val()) > 0 && parseFloat($(txtPxKilo).val()) > 0  && $(txtMaterial).val()=='') {
            $(txtMaterial).val($(hfMaterial).val());
            return false;
        }
        ...
        if ($(txtMaterial).val()==$(hfMaterial).val()){
            $(txtMaterial).val($(hfMaterial).val());
            return false
        }
...
function F_ActualizarPestimado(Fila) {
    try {
        var txtPestimado = '#' + Fila;
        var hfCodDetalle = txtPestimado.replace('txtPestimado', 'hfCodDetalle');
        var txtPxKilo = txtPestimado.replace('txtPestimado', 'txtPxKilo');
        var hfpestimado = txtPestimado.replace('txtPestimado', 'hfpestimado');
        var hfpxkilo = txtPestimado.replace('txtPestimado', 'hfpxkilo');
        var Flag = 0;
        var FlagIgv = 0;

 if (parseFloat($(txtPxKilo).val()) == parseFloat($(hfpxkilo).val()) & parseFloat($(txtPestimado).val()) == parseFloat($(hfpestimado).val())) {
            $(txtPxKilo).val(parseFloat($(hfpxkilo).val()).toFixed(2));
            $(txtPestimado).val(parseFloat($(hfpestimado).val()).toFixed(2));
            return false;
        }
        ...
        var objParams = {
            Filtro_PxKilo:$(txtPxKilo).val(),
            Filtro_PEstimado: $(txtPestimado).val(),
            Filtro_CodDetDocumentoVenta: $(hfCodDetalle).val(),
            Filtro_TasaIgv: tasaigv,
             Filtro_NotaPedido: 0,
             Filtro_FlagIgv: FlagIgv,
        ...
        var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
        MostrarEspera(true);
        F_ActualizarPrecioXkilo_Net(arg, function (result) {
...
function F_ActualizarPxKilo(Fila) {
    try {
        var txtPxKilo = '#' + Fila;
        var hfCodDetalle = txtPxKilo.replace('txtPxKilo', 'hfCodDetalle');
        var txtPestimado = txtPxKilo.replace('txtPxKilo', 'txtPestimado');
        var hfpestimado = txtPxKilo.replace('txtPxKilo', 'hfpestimado');
        var hfpxkilo = txtPxKilo.replace('txtPxKilo', 'hfpxkilo');
        var Flag = 0;
        var FlagIgv = 0;
