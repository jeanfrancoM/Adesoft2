var AppSession = "../Ventas/RegistroCotizacionesConvergente.aspx";
                ...
                success: function (data) {
                    response($.map(data.d, function (item) {
                        return {
                            ...
                            Chasis: item.split(',')[10],
                            CodTipoPlan: item.split(',')[11]
                        }
                    }))
                },
                error: function (response) {
 ...
 if ($("#MainContent_txtPlaca").val() != $("#hfPlaca").val() & Buscando == false & $("#hfPlaca").val() != '') {
    ...
    $('#MainContent_btnAgregar').click(function () {
     if (!F_SesionRedireccionar(AppSession)) return false;
      ...
      $('#MainContent_btnPlan').click(function () {
     if (!F_SesionRedireccionar(AppSession)) return false;

        try {
            if (!F_ValidarAplicarPlanMantenimiento()){
                return false;
            }
              F_AplicarPlanMantenimiento();  
               return false;
        }
...
function F_ValidarAplicarPlanMantenimiento(){
...
try 
        {        
            ...
            */
              ...
              if ($(Cuerpo + 'txtPlaca').val()=='' | $('#hfPlaca').val()=='' | $('#hfCodVehiculo').val()=='' )
                    Cadena=Cadena + '<p></p>' + 'Datos del vehiculo'; 

            //preventivo 1
             if ( $("#MainContent_ddlTipoUso option:selected").text()!='PREVENTIVO' ){
              Cadena=Cadena + '<p></p>' + 'TIPO MANTENIMIENTO DEBE SER PREVENTIVO';
             }
             
             if ($("#MainContent_ddlTipoUso option:selected").text()=='' )
                    Cadena=Cadena + '<p></p>' + 'Tipo Mantenimiento';

             if ($(Cuerpo + 'txtKM').val()=='' | $(Cuerpo + 'txtKM').val()=='0' )
                    Cadena=Cadena + '<p></p>' + 'Kilometraje';


               if ( $('#hfCodCliente').val()=='' | $('#hfCodCliente').val()=='0' )
                    Cadena=Cadena + '<p></p>' + 'Cliente';   

//            
//              if ($(Cuerpo + 'txtHorometro').val()=='' | $(Cuerpo + 'txtHorometro').val()=='0')
//                    Cadena=Cadena + '<p></p>' + 'Horometro';   
//            
            
           

          // if ($(Cuerpo + 'txtHorometro').val()=='')
          //             Cadena=Cadena + '<p></p>' + 'Horometro';
          // 
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") 
                    {
                        ...
                        F_Update_Division_HTML('div_TipoCotizacionVehiculo', result.split('~')[17]);
                       ...
                       // $('#MainContent_ddlVendedor').val(result.split('~')[13]);
                        ...
                        $('#MainContent_ddlEmpleadoConsulta').css('background', '#FFFFE0');
                         $('#MainContent_ddlTipoUso').css('background', '#FFFFE0');
                          $('#MainContent_ddlTipoUso').val(-1);
                         ...
                         $('#MainContent_txtHorometro').css('background', '#FFFFE0');
...
AgregandoProducto = false;
function F_AplicarPlanMantenimiento() {
    if (AgregandoProducto === true)
        return true;
    try {
        ...
        var Contenedor = '#MainContent_';
         ...
         var objParams = {
            ...
            Filtro_Kilometros: $(Contenedor + 'txtKM').val(),
            Filtro_CodVehiculo: $('#hfCodVehiculo').val()

        };

        var arg = Sys.Serialization.JavaScriptSerializer.serialize(objParams);
        F_AplicarPlanTemporal_NET(arg, function (result) {
        AgregandoProducto = false;
            ...
            if (str_resultado_operacion == "1") {
                ...
                $('.ccsestilo').css('background', '#FFFFE0');
                F_LimpiarGrillaConsulta();
                if (result.split('~')[2] == 'Se aplico el plan con exito')
                    alertify.log('Se aplico el plan con exito');
                $('#MainContent_chkDescripcion').focus();
    ...
    try 
        {        
            ...
            var Cadena = 'Ingresar los sgtes. Datos:'; 
             ...
             if ($(Cuerpo + 'txtKM').val()=='0' | $(Cuerpo + 'txtKM').val()=='')
                    Cadena=Cadena + '<p></p>' + 'Kilometros';

                  if ($(Cuerpo + 'txtHorometro').val()=='0' | $(Cuerpo + 'txtHorometro').val()=='')
                    Cadena=Cadena + '<p></p>' + 'Horometro';      

                    if ($('hfCodCliente').val()=='0' | $(Cuerpo + 'txtCliente').val()=='')
               Cadena=Cadena + '<p></p>' + 'Cliente';   
...
//            NroRuc = '';
        ...
        var objParams = {
            ...
            Filtro_CodCliente: $('#hfCodCliente').val(),

            Filtro_Kilometraje: $('#MainContent_txtKM').val(),
            Filtro_Horometro: $('#MainContent_txtHorometro').val(),
            Filtro_CodVehiculo: $('#hfCodVehiculo').val(),
            ...
            Filtro_FechaIngreso: $(Contenedor + 'txtIngreso').val(),
            Filtro_FechaSalida: $(Contenedor + 'txtSalida').val(),
            ...
            Filtro_ObservacionPermiso: ObservacionPermiso,
             ...
             Filtro_CodTipoMantenimiento: $('#MainContent_ddlTipoUso').val()
         };
        ...
        success: function (dbObject) {
            ...
            try {
                    ...
                    var objParams = {
                        ...
                        Filtro_Horometro: data.Horometro,

                        Filtro_CodTipoPlan:data.CodTipoPlan,
                        Filtro_CodVehiculo: data.CodVehiculo,
                        Filtro_CodTipoMantenimiento: data.CodTipoMantenimiento,
                        Filtro_Color: data.Color,
                        Filtro_Anio: data.Anio
                    };
                ...
                F_AgregarTemporal_NET(arg, function (result) {
                    ...
                    if (result.split('~')[2] == "Los Producto(s) se han agregado con exito") {                        
                        ...
                        F_Update_Division_HTML('div_grvDetalleArticulo', result.split('~')[4]);
                        $('#lblCantidadRegistro').text(F_Numerar_Grilla("grvDetalleArticulo", "lblimporte"));
                        $('#hfCodCliente').val(data.CodCtaCte);

                        $('#hfCodTipoPlan').val(data.CodTipoPlan);

                         $('#MainContent_txtColor').val(data.Color);
                         $('#MainContent_txtAnio').val(data.Anio);

                        $('#MainContent_txtHorometro').val(data.Horometro);
                        $('#MainContent_txtKM').val(data.Kilometraje);
                        $('#hfCodCliente').val(data.CodCtaCte);

                        $('#hfCodTipoPlan').val(data.CodTipoPlan);
                        $('#hfCodVehiculo').val(data.CodVehiculo);

                        $('#MainContent_ddlTipoUso').val(data.CodTipoMantenimiento);
                        ...
                        $('#MainContent_txtIngreso').val(data.FechaIngresoStr);
                        $('#MainContent_txtSalida').val(data.FechaSalidaStr);
                        $('.ccsestilo').css('background', '#FFFFE0');
                        $('#hfNotaPedido').val(result.split('~')[9]);
                            if ($('#hfNotaPedido').val() == '5')
                                $('#hfCodCtaCteNP').val($('#hfCodCtaCte').val());
