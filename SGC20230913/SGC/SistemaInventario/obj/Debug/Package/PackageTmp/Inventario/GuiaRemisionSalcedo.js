var AppSession = "../Inventario/GuiaRemisionSalcedo.aspx";
...
//            $('#hfDireccionTransportista').val(i.item.Direccion+' - '+i.item.Distrito);
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#hfCodEmpresa').val(result.split('~')[17]);
                        $('#hfCodAlmacen').val(result.split('~')[18]);
...
function F_ValidarGrabarDocumento(){
    try 
        ...
        if (($(Cuerpo + 'txtProveedor').val()=='' || $('#hfCodCtaCte').val()==0) & $(Cuerpo + 'ddlMotivoTrabajo').val()=='9' )
        ...
        if ($('#MainContent_ddlDireccion').val() == null & $(Cuerpo + 'ddlMotivoTrabajo').val()=='9')
        ...
        if ($('#MainContent_ddlDestino').val() == null & $(Cuerpo + 'ddlMotivoTrabajo').val()=='7')
            Cadena = Cadena + '<p></p>' + 'Destino';

              if ($('#MainContent_txtPartida').val() == $("#MainContent_ddlDestino option:selected").text())
            Cadena = Cadena + '<p></p>' + 'PARTIDA Y DESTINO NO PUEDEN SER IGUALES';

             if ($('#MainContent_ddlMotivoTrabajo').val() == 14)
            Cadena = Cadena + '<p></p>' + 'NO PUEDE SER INGRESO DE MERCADERIA';
...
function F_GrabarDocumento(){
    ...
    try 
        ...
        var Destino = $("#MainContent_ddlDestino option:selected").text();
        var CodAlmacenLlegada = $(Contenedor + 'ddlDestino').val();
        var CodDireccion = $(Contenedor + 'ddlDireccion').val();
...
//           if ($(Contenedor + 'ddlMotivoTrabajo').val()== '9'){
//            CodCtaCte = $('#hfCodCtaCte').val();
//            Destino = $("#MainContent_ddlDireccion option:selected").text();
//            CodAlmacenLlegada = $(Contenedor + 'ddlDireccion').val();
//             CodDireccion = $(Contenedor + 'ddlDireccion').val();
//            }

             if ($(Contenedor + 'ddlMotivoTrabajo').val()== '7'){
            CodCtaCte = $('#hfCodCtaCte').val();
            Destino = $("#MainContent_ddlDestino option:selected").text();
            CodAlmacenLlegada = $(Contenedor + 'ddlDestino').val();
             CodDireccion = $(Contenedor + 'ddlDireccion').val();
            }
        ...
        var objParams = {
                ...
                Filtro_NumeroDoc: $(Contenedor + 'txtNumero').val(),
                Filtro_CodMotivoInterno: $(Contenedor + 'ddlMotivoTrabajo').val(),
                Filtro_CodAlmacenLlegada: CodAlmacenLlegada,
                Filtro_Partida: $(Contenedor + 'txtPartida').val(),
                Filtro_Destino:  Destino,               
                Filtro_CodCtaCte: CodCtaCte,
                ...
                Filtro_CodDireccionTrans: $(Contenedor + 'ddlDireccionTransportista').val(),              
...
function F_BuscarDireccionesTransportista() {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        url: '../Servicios/Servicios.asmx/F_TCDireccion_ListarXCliente_AutoComplete',
        data: "{'CodCtaCte':'" + $('#hfCodCtaCteTransportista').val() + "'}",
        dataType: "json",
        async: false,
        success: function (dbObject) {
            var data = dbObject.d;
            try {
                ...
                var CodDireccionPrimera = 0; var ccc = 0;
                $.each(data.rows, function (index, item) {
                    $('#MainContent_ddlDireccionTransportista').append($("<option></option>").val(item.CodDireccion).html(item.Direccion));
                    if (ccc === 0) CodDireccionPrimera = item.CodDireccion;  ccc++;
                });


                if ($('#hfDireccionTransportista').val() != '')
                {
                    var CodDir = 0;
                    $('#MainContent_ddlDireccionTransportista > option').each(function() {
                        //alert($(this).text() + ' ' + $(this).val());
                        if ($('#hfDireccionTransportista').val().trim() === $(this).text().trim())
                            CodDir = $(this).val()
                    });
                    $('#MainContent_ddlDireccionTransportista').val(CodDir);
                }

              

            if ($('#MainContent_ddlDireccionTransportista').val() == null)
                $('#MainContent_ddlDireccionTransportista').val(CodDireccionPrimera);
        ...
        try {
            ...
            F_ReemplazarFactura_NET(arg, function (result) {
                ...
                if (str_resultado_operacion == "1") {
                    ...
                    $('#MainContent_txtObservacion').val(result.split('~')[28]);
                    F_BuscarDireccionesCliente();
                    ...
                    $('.ccsestilo').css('background', '#FFFFE0');
                    $('#hfCodCtaCteTransportista').val();
                    $("#divTabs").tabs("option", "active", $("#liRegistro").index());
                    $('#hfCodTransportista').val(result.split('~')[19]),
                    $('#hfCodCtaCteTransportista').val(result.split('~')[19]);
                    $('#hfCodDireccionTransportista').val(result.split('~')[20]);
                    $('#MainContent_txtPlaca').val(result.split('~')[21]);
                    $('#MainContent_txtMarcaGuia').val(result.split('~')[22]);
                    $('#MainContent_txtLicenciaGuia').val(result.split('~')[23]);
                    $('#MainContent_txtNuBultos').val(result.split('~')[24]);
                    $('#MainContent_txtPeso').val(result.split('~')[25]);
                    $('#MainContent_txtTransportista').val(result.split('~')[26]);
                    F_BuscarDireccionesTransportista();
                    $('#MainContent_ddlDireccionTransportista').val(result.split('~')[20]);
...
if (!F_SesionRedireccionar(AppSession)) return false;
        ...
        if ($(lblEstado).text() == 'FACTURADO' || $(lblEstado).text() == 'ANULADO')
...
function F_Efecto_MotivoTrabajo() {
     try {

     switch($('#MainContent_ddlTipoDocumento').val()) 
     {
        case "4":
            $('#transportista1').css('display', 'none');
                   $('#transportista2').css('display', 'none');
             
        break;
        case "10":
            $('#transportista1').css('display', '');
              $('#transportista2').css('display', '');
