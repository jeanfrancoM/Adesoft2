var AppSession = "../Maestros/ProcedenciaSalcedo.aspx";


var CodigoMenu = 1000;
var CodigoInterno = 25;
...
$(document).ready(function () {
    ...
    $('#MainContent_btnGrabar').click(function () {
        ...
        try {
            ...
            if (confirm("ESTA SEGURO DE GRABAR LA PROCEDENCIA"))
                F_GrabarDocumento();
    ...
    $('#MainContent_btnEdicion').click(function () {
        ...
        try {
            ...
            if (confirm("ESTA SEGURO DE ACTUALIZAR LOS DATOS DE LA PROCEDENCIA"))
                F_EdicionRegistro();
...
function F_GrabarDocumento() {
    try {
        ...
        var objParams = {
            Filtro_Codigo: $(Contenedor + 'txtCodigo').val(),
...
function F_Buscar() {
    ...
    try {
        var objParams = {
            Filtro_DscProcedencia: $("#MainContent_txtDescripcionConsulta").val()
        ...
        F_Buscar_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                $('#lblNumeroConsulta').text(F_Numerar_Grilla("grvConsulta", 'lblCodigoProcedencia'));
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        var imgID = Fila.id;
        var hfCodProcedencia = '#' + imgID.replace('imgAnularDocumento', 'hfCodProcedencia');
        var lblDescripcion = '#' + imgID.replace('imgAnularDocumento', 'lblDescripcion');
        var lblCodigoProcedencia = '#' + imgID.replace('imgAnularDocumento', 'lblCodigoProcedencia');

        if (!confirm("ESTA SEGURO DE ELIMINAR LA PROCEDENCIA " + $(lblCodigoProcedencia).text() + ' - ' + $(lblDescripcion).text().toUpperCase()))
            return false;

        var objParams = {
            Filtro_CodProcedencia: $(hfCodProcedencia).val(),
...
function F_EditarRegistro(Fila) {
    ...
    try {
        var imgID = Fila.id;
        var hfCodProcedencia = '#' + imgID.replace('imgEditarRegistro', 'hfCodProcedencia');
        var lblCodigoProcedencia = '#' + imgID.replace('imgEditarRegistro', 'lblCodigoProcedencia');
        ...
        $('#hfCodProcedencia').val($(hfCodProcedencia).val());
        $(Cuerpo + 'txtCodigoEdicion').val($(lblCodigoProcedencia).text());
        $(Cuerpo + 'txtDescripcionEdicion').val($(lblDescripcion).text());

        $("#divEdicionRegistro").dialog({
            resizable: false,
            modal: true,
            title: "Edicion de Procedencia",
            title_html: true,
            height: 160,
            width: 530,
            autoOpen: false
...
function F_EdicionRegistro() {
    try {
        ...
        var objParams = {
            ...
            Filtro_DscProcedencia: $(Contenedor + 'txtDescripcionConsulta').val(),
            Filtro_CodigoEdicion: $(Contenedor + 'txtCodigoEdicion').val()
        };
