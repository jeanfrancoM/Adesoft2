...
function F_AgregarTemporal() {
    try {
        ...
        $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
            ...
            hfCostoUniOriginal = chkSi.replace('chkOK', 'hfCostoUniOriginal');
          
            if ($(chkSi).is(':checked')) {
                var objDetalle = {
                        CodProducto: $(hfCodProducto).val(),
                        Precio1:$(hfCostoUniOriginal).val()*(1+( $(txtM1).val()/100)),
                        Precio2:$(hfCostoUniOriginal).val()*(1+( $(txtM2).val()/100)),
                        Precio3:$(hfCostoUniOriginal).val()*(1+( $(txtM3).val()/100)),
                        Precio4:$(hfCostoUniOriginal).val()*(1+( $(txtM4).val()/100)),
                        Precio5: $(txtPrecioLista5).val(),
                        M1:$(txtM1).val(),
                        M2:$(txtM2).val(),
                        M3:$(txtM3).val(),
                        M4:$(txtM4).val(),
