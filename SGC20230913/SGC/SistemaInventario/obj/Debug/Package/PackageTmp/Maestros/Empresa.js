...
function F_ValidarEdicionDocumento() {
    ...
    try {
        ...
        if (!ValidarRuc($(Cuerpo + 'txtRucEmpresaEdicion').val())) Cadena = Cadena + '<p></p>' + 'Ruc INVALIDO';
    ...
    myDropzone = new Dropzone("#mydropzone", {
        ...
        init: function () {
            ...
            $.ajax({
                ...
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                url: "../Digitalizacion/FileUpLoadDB.ashx?IdFile=" + str_id + "&Flag=3" + "&tipo=" + 0 + "&otro=" + Math.round(Math.random()) * 100,
    ...
    mydropzone_Edit = new Dropzone("#mydropzone_Edit", {
        ...
        init: function () {
            ...
            $.ajax({
                ...
                type: 'POST',
                contentType: "application/json; charset=utf-8",
                url: "../Digitalizacion/FileUpLoadDB.ashx?IdFile=" + $('#hfCodEmpresaEdicion').val() + "&nameimg=" + nameimg + "&Flag=3" + "&tipo=1" + "&otro=" + Math.round(Math.random()) * 100,
...
function F_VisualizarRegistro(Fila) {
    ...
    $.ajax({
        ...
        type: 'POST',
        contentType: "application/json; charset=utf-8",
        url: "../Digitalizacion/FileDocDB.ashx?IdFile=" + str_id + "&Flag=3&tipo=1" + "&otro=" + Math.round(Math.random()) * 100,
        success: function (data) {
            MostrarEspera(true);
            try
            {
