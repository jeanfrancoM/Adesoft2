        ...
        success: function (dbObject) {
            ...
            try {
                ...
                $.each(data.lUsuario, function (index, item) {
                    if (item.CodEstado === 1)
                        $('#MainContent_ddlUsuario').append($("<option></option>").val(item.CodUsuario).html(item.NombreUsuario));
                });
            }
            ...
            try {
                ...
                $.each(data.lUsuarioCopiar, function (index, item) {
                    if (item.CodEstado === 1)
                        $('#MainContent_ddlUsuarioCopiar').append($("<option></option>").val(item.CodUsuario).html(item.NombreUsuario));
                });
            }
            catch (x) { alertify.log('ERROR AL CARGAR LOS USUARIOS'); }
...
var TipoOperacion = '';
var divEdicionRegistro_height = 430;
            ...
            try {
                ...
                $.each(dbObject.d, function (index, item) {
                    ...
                    if (item.CodEstado == 1 && TipoOperacion == "Editar") {
                        $('#MainContent_grvAlmacenes_chkAlmacen_' + (item.CodAlmacen - 2)).prop("checked", true);
                    }
                    else if (TipoOperacion == "Agregar")
                        $('#MainContent_grvAlmacenes_chkAlmacen_' + (item.CodAlmacen -2)).prop("checked", false);
                });
