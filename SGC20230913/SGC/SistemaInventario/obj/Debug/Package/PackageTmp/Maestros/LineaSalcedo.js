var AppSession = "../Maestros/LineaSalcedo.aspx";
...
function F_Buscar() {
    ...
    try {
        var objParams = {
            Filtro_DscLinea: $("#MainContent_txtDescripcionConsulta").val()
        ...
        F_Buscar_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                $('#lblNumeroConsulta').text(F_Numerar_Grilla("grvConsulta", 'lblCodigoLinea'));
...
function F_EliminarRegistro(Fila) {
    ...
    try {
        ...
        var lblCodigoLinea = '#' + imgID.replace('imgAnularDocumento', 'lblCodigoLinea');

        if (!confirm("ESTA SEGURO DE ELIMINAR LA LINEA " + $(lblCodigoLinea).text() + ' - ' + $(lblDescripcion).text().toUpperCase()))
            return false;
...
function F_EditarRegistro(Fila) {
    ...
    try {
        ...
        $(Cuerpo + 'txtCodigoEdicion').val($(lblCodigoLinea).text());
...
function F_EdicionRegistro() {
    try {
        ...
        var objParams = {
            ...
            Filtro_DscLinea: $(Contenedor + 'txtDescripcionConsulta').val(),
            Filtro_CodigoLinea: $(Contenedor + 'txtCodigoEdicion').val()
        };
