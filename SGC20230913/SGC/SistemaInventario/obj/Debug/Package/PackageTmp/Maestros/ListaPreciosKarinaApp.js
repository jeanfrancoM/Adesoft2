var AppSession = "../Maestros/ListaPreciosKarinaApp.aspx";

var CodigoMenu = 1000;
var CodigoInterno = 20;
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Insertar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informac
    ...
    $('#MainContent_ddlTipoSincronizacion').css('background', '#FFFFE0');
        ...
        var objParams =
            {
                ...
                Filtro_SeVendeEnApp: $('#MainContent_ddlTipoSincronizacion').val(),
                Filtro_SoloConImagenes: ($('#MainContent_chkSoloConImagenes').is(':checked'))? 1 : 0,
                Filtro_SoloYaEnviados: ($('#MainContent_chkSoloYaEnviados').is(':checked'))? 1 : 0,
            };
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        var lblPrecioApp = '';
                        var hfCodProducto = '';
                        var lblCantidadImagenes = '';
                        var lblPublicadoApp = '';
                        $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
                            var chkSi = '#' + this.id;
                            lblPrecioApp = chkSi.replace('chkOK', 'lblPrecioApp');
                            hfCodProducto = chkSi.replace('chkOK', 'hfCodProducto');
                            lblCantidadImagenes = chkSi.replace('chkOK', 'lblCantidadImagenes');
                            lblPublicadoApp = chkSi.replace('chkOK', 'lblPublicadoApp');

                            if ($(lblPrecioApp).text() === "" && $(hfCodProducto).val() != "") {
                                $(lblPrecioApp).closest('td').css('background-color', 'red');
                            }

                            if ($(lblCantidadImagenes).text() === "0" && $(hfCodProducto).val() != "") {
                                $(lblCantidadImagenes).closest('td').css('background-color', 'orange');
                            }

                            if ($(lblPublicadoApp).text() === "SI" && $(hfCodProducto).val() != "") {
                                $(lblPublicadoApp).closest('td').css('background-color', 'green');
                            }
...
function checkAll(objRef)
{
    ...
    if ($(checkallid).is(':checked')) {
            ...
            $('#MainContent_grvConsultaArticulo .ccsestilo').prop('disabled', false);
        }
        else {
        $('#MainContent_grvConsultaArticulo input:checkbox').prop('checked', false);
            $('#MainContent_grvConsultaArticulo .ccsestilo').prop('disabled', true);
...
function F_Text_To_Array(texto, separador) {
var array = new Array();


    var x = texto.split(separador);
...
$(document).ready(function () {
    ...
    var storage = firebase.app().storage("gs://appmod1-93c45.appspot.com");
...
var arrArticulosData = new Array();
function F_Enviar_Productos() {

    arrArticulos = new Array();
    arrArticulosData = new Array();

    MostrarEspera(true);

    $('#MainContent_grvConsultaArticulo .chkSi :checkbox').each(function () {
        var chkSi = '#' + this.id;
        var imgProceso = chkSi.replace('chkOK', 'imgProceso');
        var hfEmpresaRuc = chkSi.replace('chkOK', 'hfEmpresaRuc');
        var lblPrecioApp = chkSi.replace('chkOK', 'lblPrecioApp');
        var hfCodProducto = chkSi.replace('chkOK', 'hfCodProducto');
        var hfCodAlterno = chkSi.replace('chkOK', 'hfCodAlterno');
        var lblCantidadImagenes = chkSi.replace('chkOK', 'lblCantidadImagenes');
        var lblPublicadoApp = chkSi.replace('chkOK', 'lblPublicadoApp');
        var hfCodEstado = chkSi.replace('chkOK', 'hfCodEstado');
        var lblDescripcion = chkSi.replace('chkOK', 'lblDescripcion');
        var hfDescripcionDetalladaApp = chkSi.replace('chkOK', 'hfDescripcionDetalladaApp');
        var lblFamilia = chkSi.replace('chkOK', 'lblFamilia');
        var lblPrecioApp = chkSi.replace('chkOK', 'lblPrecioApp');
        var hfStock = chkSi.replace('chkOK', 'hfStock');
        var hfTipoValorUM = chkSi.replace('chkOK', 'hfTipoValorUM');
        var hfUM = chkSi.replace('chkOK', 'hfUM');
        var hfTags = chkSi.replace('chkOK', 'hfTags');
        var hfTagsMarcaModelo = chkSi.replace('chkOK', 'hfTagsMarcaModelo');
        var hfTagsMarcas = chkSi.replace('chkOK', 'hfTagsMarcas');
        var hfTagsModelos = chkSi.replace('chkOK', 'hfTagsModelos');
        var hfTagsModelos = chkSi.replace('chkOK', 'hfTagsModelos');
        var hfCodigoApp = chkSi.replace('chkOK', 'hfCodigoApp');
        

        if ($(lblPrecioApp).text() != "" && $(hfCodProducto).val() != "" && $(chkSi).is(':checked')) {

                var Articulo = {};

                Articulo["CodEstado"] = Number($(hfCodEstado).val());
                Articulo["Codigo"] = $(hfCodAlterno).val();
                Articulo["Denominacion"] = $(lblDescripcion).text();
                Articulo["Descripcion"] = $(hfDescripcionDetalladaApp).val();
                Articulo["Familia"] =  $(lblFamilia).text(); 
                //Articulo["Imagenes"] = []; 
                Articulo["MostrarPrecio"] = 1; //: 1
                Articulo["MostrarStock"] = 1; //: 1
                Articulo["Precio"] = Number($(lblPrecioApp).text()); //: 44.56
                Articulo["Stock"] = Number($(hfStock).val()); //: 0
                Articulo["Tags"] = F_Text_To_Array($(hfTags).val(), '||||'); // : (6) ["", "ESTABILIZADOR", "JFP", "MAZDA", "3", "POST"]
                Articulo["TagsMarcaModelo"] = F_Text_To_Array($(hfTagsMarcaModelo).val(), '||||'); //: []
                Articulo["TagsMarcas"] = F_Text_To_Array($(hfTagsMarcas).val(), '||||'); //: []
                Articulo["TagsModelos"] = F_Text_To_Array($(hfTagsModelos).val(), '||||'); //: []
                Articulo["TagsText"] = ""; //: ""
                Articulo["TipoValorUM"] = $(hfTipoValorUM).val(); //: "ENTERO"
                Articulo["UM"] = $(hfUM).val(); //: "UND"
                Articulo["fua"] = ""; //: ""
                Articulo["uua"] = ""; //: "ADMIN"
                ...
                ArtContenedor["codigoapp"] = $(hfCodigoApp).val();
                ArtContenedor["ruc"] = $(hfEmpresaRuc).val();
                ArtContenedor["casilla"] = chkSi;
                ArtContenedor["imgproceso"] = imgProceso;
                ArtContenedor["data"] = Articulo;
                ArtContenedor["codproducto"] = $(hfCodProducto).val();
                ArtContenedor["imagenesb64"] = [];
                

                arrArticulosData.push(ArtContenedor);
    ...
    if (arrArticulos.length > 0) {
            if (!confirm("¿SEGURO DE SINCRONIZAR " + arrArticulos.length.toString() + " ARTICULOS A LA APP? " ))
                return false;
            else
                F_Enviar_Firestore();
...
function F_Lista_Img_Base64(CodProducto) {
    var valor;

        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '../Servicios/APP_Procesos.asmx/F_APP_LGProductos_ListaImagenes',
            ...
            error: function (response) {
                ...
                $(productoOnFire.imgproceso).attr("src","../Asset/images/cancelarx16.png");
            },
...
function F_Grabar_Firestore() {
    ...
    productoOnFire  = arrArticulosData[fireActual];


    if (productoOnFire.codigoapp === "") {
    //--ADD NUEVO 100%

        // Add a new document with a generated id.
        dbFirestore.collection("Articulos").doc(productoOnFire.ruc).collection("Articulos").add(productoOnFire.data)
        .then((docRef) => {
            productoOnFire.codigoapp = docRef.id;
            arrArticulosData[fireActual].codigoapp = docRef.id;

            F_Actualizar_Producto(productoOnFire.codproducto, docRef.id);
            $(productoOnFire.imgproceso).attr("src","../Asset/images/ok.gif");


            if (fireActual + 1 === arrArticulosData.length) {
                MostrarEspera(false);
                toastr.success("SE ACTUALIZARON LOS PRODUCTOS");
                    if ($('#MainContent_chkEnviarImagenes').is(':checked')) {
                        toastr.success("AHORA SE SUBIRAN LAS IMAGENES");
                        F_Grabar_Cloud();
                    }
                return false;
                }
    ...
    //--UPDATE YA EXISTE
        ...
        var actualizacion = dbFirestore.collection("Articulos").doc(productoOnFire.ruc).collection("Articulos").doc(productoOnFire.codigoapp);

        // Set the "capital" field of the city 'DC'
        return actualizacion.update(productoOnFire.data)
        .then(() => {

            F_Actualizar_Producto(productoOnFire.codproducto, productoOnFire.codigoapp);
            $(productoOnFire.imgproceso).attr("src","../Asset/images/ok.gif");
...
function F_Actualizar_Producto(CodProducto, CodigoApp) {

        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '../Servicios/APP_Procesos.asmx/F_APP_ActualizarProducto',
            data: "{'CodProducto':'" + CodProducto + "','CodigoApp':'" + CodigoApp + "'}",
            dataType: "json",
            async: false,
            success: function (data) {
...
function F_Grabar_Cloud() {
    MostrarEspera(true);
    fireActual = 0;
    F_Recorrido_Producto();
return false;
}

var arrImagenesProcesar = new Array();
var arrImgsFIRESTORE = new Array();
function F_Recorrido_Producto() {

    //RECORRIDO PARA BUSCAR LAS IMAGENES QUE LES CORRESPONDERIA
    arrArticulosData[fireActual].imagenesb64 = F_Lista_Img_Base64(arrArticulosData[fireActual].codproducto);
    productoOnFire  = arrArticulosData[fireActual];
    
    if (productoOnFire.imagenesb64.length === 0) { //si no hay nada, paso al siguiente
        
        arrImgsFIRESTORE = new Array();
        F_Actualizar_Imagen_Firestore();

        if (fireActual + 1 === arrArticulosData.length) { //si no hay nadie mas termino
            MostrarEspera(false);
            toastr.success("SE SUBIERON TODAS LAS IMAGENES");
            return false;
        }

        ////sumo el consecutivo al proximo index
        //fireActual = fireActual + 1;
        //F_Recorrido_Producto();
        return false;
    }

    arrImagenesProcesar = productoOnFire.imagenesb64;

    //asigno los nombres de como se van a llamar en el storage
    arrImagenesProcesar.forEach( function(valor, indice, array) {
        arrImagenesProcesar[indice].name = productoOnFire.codigoapp + '-' + productoOnFire.imagenesb64[indice].IdImagen + ".jpg";
        productoOnFire.imagenesb64[indice].name = productoOnFire.codigoapp + '-' + productoOnFire.imagenesb64[indice].IdImagen + ".jpg";
    });
...
function F_Subir_Imagen()  {

var filename = arrImagenesProcesar[indexIMG_SUBIR].name;
var file = arrImagenesProcesar[indexIMG_SUBIR].imageB64;
...
var uploadTask = storageRef.child('/APP/Productos/' + filename).putString(file, 'base64');
...
uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
  ...
  uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
    ...
    arrImgsFIRESTORE.push(downloadURL);


    //si ya es la ultima imagen.
    if (indexIMG_SUBIR == (arrImagenesProcesar.length - 1)){
       F_Actualizar_Imagen_Firestore();
       return false;
    }

    indexIMG_SUBIR = indexIMG_SUBIR + 1;
    F_Subir_Imagen();
...
function F_Actualizar_Imagen_Firestore() {

    

    var ArtImagenes = {};
    ArtImagenes["Imagenes"] = arrImgsFIRESTORE;

    var actualizacion = dbFirestore.collection("Articulos").doc(productoOnFire.ruc).collection("Articulos").doc(productoOnFire.codigoapp);

    // Set the "capital" field of the city 'DC'
    return actualizacion.update(ArtImagenes)
    .then(() => {
        ...
        toastr.success("SUBIDAS IMAGENES " + fireActual.toString() + " DE " + arrArticulosData.length.toString() + " PRODUCTOS");
