...
$(document).ready(function () {
    ...
    $('.Jq-ui-dtp2').datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd/mm/yy'
    });    

    $('.Jq-ui-dtp2').datepicker($.datepicker.regional['es']);
    $('.Jq-ui-dtp2').datepicker('setDate', new Date());
...
var C=0;          
 ...
 $('#MainContent_grvDetalleEdicionComisionTarjeta .ccsestilo ').each(function () {
        txtTotalTarjeta='#MainContent_grvDetalleEdicionComisionTarjeta_txtTotalTarjeta_' + H
        lblTotalFactura = txtTotalTarjeta.replace('txtTotalTarjeta', 'lblTotalFactura');
        lblNumero = txtTotalTarjeta.replace('txtTotalTarjeta', 'lblNumero');
                               
        if ($('#MainContent_ddlMedioPagoEdicion').val()==10 & parseFloat($(txtTotalTarjeta).val()) < parseFloat($(lblTotalFactura).text()))
        {           
            toastr.warning($(lblNumero).text() +    " EL CT NO PUEDE SER MENOR AL MONTO, VERIFICAR");  
            C++;
            return false;        
        }           
        H++
...
//          Cadena=Cadena + '<p></p>' + "Cliente Varios";
   ...
   toastr.warning('LA COMISION TARJETA NO PUEDE SER MENOR AL VALOR ACUENTA');
   }
    return true;
});
...
//    }
    ...
    $('#MainContent_txtComisionTarjetaEdicion').css('background', '#FFFFE0');
            ...
            (
                ...
                function (result) {
                    ...
                    {                        
                        ...
                        $('#MainContent_lblCT').text("TOTAL TARJETA (+"+result.split('~')[32]+"%)");
                        $('#hfComisionTarjeta').val( result.split('~')[32]);
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacion
        try {
            ...
            var lblComisionTarjeta = '#' + imgID.replace('imgEditar', 'lblComisionTarjeta');
            ...
            $('#MainContent_txtComisionTarjetaEdicion').val($(lblComisionTarjeta).text());
            $('#hfComisionTarjetaEdicion').val($(lblComisionTarjeta).text());
            ...
            if (str_resultado_operacion === "1")
            {
                    ...
                    $('#MainContent_txtComisionTarjetaEdicion').prop('disabled', true); 
     ...
     txtTotalTarjeta = '#MainContent_grvDetalleEdicionComisionTarjeta_txtTotalTarjeta_'+F;
      ...
      var objDetalle = {
        ...
        var objParams = {   
            ...
            Filtro_ComisionTarjeta:        $('#MainContent_txtComisionTarjetaEdicion').val(),
            Filtro_XmlDetalle: Sys.Serialization.JavaScriptSerializer.serialize(arrDetalle)
                         };
...
function F_EdicionDetalle(Fila, Tipo) {
    ...
    var TipoCambioDetalle = $('#MainContent_txtTC').val();

    if ($(hfCodTipoDoc).val()==8)
        TipoCambioDetalle = $(lblTC).text();
...
function F_EditarDetalle (){
    ...
    if (Number($('#MainContent_txtNuevoSaldoEdicion').val()) < 0 & SaldoActualReal > 0) {
      ...
      if ($('#MainContent_ddlMedioPago').val()==10 & parseFloat($('#MainContent_txtCT').val()) < parseFloat($('#MainContent_txtAcuentaEdicion').val())) {
        toastr.warning('COMISION TARJETA NO PUEDE SER MENOR Q EL ACUENTA');
        return false;
    }
...
var CT = 0;
 ...
 $('#MainContent_grvFacturaCobranzas .chkDelete :checkbox').each(function () {
        chkDel = '#' + this.id;
        lblCT = chkDel.replace('chkEditar', 'lblCT');
        lblAcuenta = chkDel.replace('chkEditar', 'lblAcuenta');
                         
        if ($('#MainContent_ddlMedioPago').val()==10)
        {
            $(lblCT).text(parseFloat($(lblAcuenta).text().replace('S/ ','')) * ((parseFloat($('#hfComisionTarjeta').val())/100)+1));
            CT+=parseFloat($(lblAcuenta).text().replace('S/ ',''));
        }            
        else
        {
             $(lblCT).text('0.00');
    ...
    $('#MainContent_txtComisionTarjeta').val(CT);
    return false;
}

function F_MedioPagoTarjetaEdicion(Fila){
    var txtTotalTarjeta = '#' + Fila;
    var lblTotalFactura = txtTotalTarjeta.replace('txtTotalTarjeta', 'lblTotalFactura');
    
    if (parseFloat($(txtTotalTarjeta).val()) < parseFloat($(lblTotalFactura).text()) & $('#MainContent_ddlMedioPagoEdicion').val() == 10)
    {
        $(txtTotalTarjeta).val(parseFloat($(lblTotalFactura).text())*((parseFloat($('#hfComisionTarjeta').val()) / 100)+1));
        toastr.warning("EL CT NO PUEDE SER MENOR AL MONTO");  
    }
    F_MedioPagoRecorridoEdicion();
    return false;
}

function F_MedioPagoRecorridoEdicion(){
var lblTotalFactura = ''
var txtTotalTarjeta = ''
var CT = 0;
var H=0;            
 ...
 $('#MainContent_grvDetalleEdicionComisionTarjeta .ccsestilo ').each(function () {
        ...
        if ($('#MainContent_ddlMedioPagoEdicion').val()==10)
        {           
            CT+= parseFloat($(txtTotalTarjeta).val().replace('S/ ',''));
            $(txtTotalTarjeta).prop('readonly', false);           
        }            
        else
        {
             $(txtTotalTarjeta).val('0.00');  
             $(txtTotalTarjeta).prop('readonly', true);          
             CT=0;                   
        }
            H++
       });

 $('#MainContent_txtComisionTarjetaEdicion').val(CT);
