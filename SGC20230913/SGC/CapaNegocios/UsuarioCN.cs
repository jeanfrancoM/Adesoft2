...
namespace CapaNegocios
{
    public class UsuarioCN
    {
        ...
        public List<UsuarioCE> F_UsuarioCopia_Listar_NET(int CodAlmacen, int CodEstado, string Usuario,int flag)
        {
            try
            {
                DataTable dtDatos = obj.F_UsuarioCopia_Listar_NET(CodAlmacen, CodEstado, Usuario);
                List<UsuarioCE> lDatos = new List<UsuarioCE>();
                if (flag == 0)
                    lDatos.Add(new UsuarioCE()
                    {
